                    ...
                    uint16_t flags)
{
    ...
    assert(queue->conn != NULL); /* just in case lwIP is trolling */
...
int sock_tcp_get_local(sock_tcp_t *sock, sock_tcp_ep_t *ep)
{
    ...
    if (sock->conn == NULL) {
        ...
        res = (lwip_sock_get_addr(sock->conn, (struct _sock_tl_ep *)ep,
                                  1)) ? -EADDRNOTAVAIL : 0;
    }
    mutex_unlock(&sock->mutex);
    return res;
...
int sock_tcp_queue_get_local(sock_tcp_queue_t *queue, sock_tcp_ep_t *ep)
{
    ...
    if (queue->conn == NULL) {
        ...
        res = lwip_sock_get_addr(queue->conn, (struct _sock_tl_ep *)ep,
                                 1) ? -EADDRNOTAVAIL : 0;
    }
    mutex_unlock(&queue->mutex);
    return res;
                    ...
                    uint32_t timeout)
{
    ...
    if (queue->used < queue->len) {
        ...
        if ((timeout != 0) && (timeout != SOCK_NO_TIMEOUT)) {
            netconn_set_recvtimeout(queue->conn, timeout / MS_IN_USEC);
        }
        else
#endif
...
#if LWIP_SO_RCVTIMEO
    if ((timeout != 0) && (timeout != SOCK_NO_TIMEOUT)) {
        netconn_set_recvtimeout(sock->conn, timeout / MS_IN_USEC);
    }
    else
#endif
