...
void lwip_bootstrap(void)
{
    ...
    if (LWIP_NETIF_NUMOF > 0) {
        ...
        for (unsigned i = 0; i < LWIP_NETIF_NUMOF_NETDEV_TAP; i++) {
            ...
            if (_netif_add(&netif[netif_idx++], &netdev_taps[i],
        ...
        for (unsigned i = 0; i < LWIP_NETIF_NUMOF_MRF24J40; i++) {
            ...
            if (netif_add(&netif[netif_idx++], &mrf24j40_devs[i],
                          lwip_netdev_init, tcpip_6lowpan_input) == NULL) {
        ...
        for (unsigned i = 0; i < LWIP_NETIF_NUMOF_AT86RF2XX; i++) {
            ...
            if (netif_add(&netif[netif_idx++], &at86rf2xx_devs[i],
                          lwip_netdev_init, tcpip_6lowpan_input) == NULL) {
        ...
        for (unsigned i = 0; i < LWIP_NETIF_NUMOF_ATWINC15X0; i++) {
            ...
            if (_netif_add(&netif[netif_idx++], &atwinc15x0_devs[i],
                           lwip_netdev_init, tcpip_input) == NULL) {
        ...
        for (unsigned i = 0; i < LWIP_NETIF_NUMOF_ENC28J60; i++) {
            ...
            if (_netif_add(&netif[netif_idx++], &enc28j60_devs[i],
                           lwip_netdev_init, tcpip_input) == NULL) {
        ...
        for (unsigned i = 0; i < LWIP_NETIF_NUMOF_SOCKET_ZEP; i++) {
            ...
            if (netif_add(&netif[netif_idx++], &socket_zep_devs[i],
                          lwip_netdev_init, tcpip_6lowpan_input) == NULL) {
        ...
        if (_netif_add(&netif[netif_idx++], &_esp_eth_dev, lwip_netdev_init,
                       tcpip_input) == NULL) {
        ...
        if (_netif_add(&netif[netif_idx++], &_esp_wifi_dev,
                       lwip_netdev_init, tcpip_input) == NULL) {
        ...
        if (_netif_add(&netif[netif_idx++], &stm32_eth,
                       lwip_netdev_init, tcpip_input) == NULL) {
        ...
        if (netif_add(&netif[netif_idx++], &nrf802154_dev,
                      lwip_netdev_init, tcpip_6lowpan_input) == NULL) {
        ...
        assert(netif_idx == LWIP_NETIF_NUMOF);
        ...
        if (netif[LWIP_NETIF_DEFAULT].state != NULL) {
            ...
            netif_set_default(&netif[LWIP_NETIF_DEFAULT]);
        }
