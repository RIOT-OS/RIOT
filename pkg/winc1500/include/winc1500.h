...
enum {
    ...
    WINC1500_ERR_FW_VER_MISMATCH = (-4), /**< did not detect the correct version of WINC1500 */
} ;
 ...
 */
typedef struct winc1500 winc1500_t; // Forward declaration for winc1500 headers

#include "winc1500_internal_types.h" // Include Atmel internal structs
...
struct winc1500 {
    ...
    char event_handler_stack[WINC1500_STACKSIZE]; /**< Stack for the event handler thread */
#endif
    mbox_t event_mbox;            /**< Message from the event handler */
    msg_t event_mbox_queue[WINC1500_EVENT_MBOX_LEN]; /**< Queue storing messages for event_mbox */
    char event_ssid[WINC1500_MAX_SSID_LEN + 1]; /**< SSID received from the event handler */
    winc1500_ap_t event_ap;       /**< AP information received from the event handler */
    uint8_t *event_mac_addr;      /**< MAC address message received from the event handler */
 ...
 * It does not initialize the device itself. In case GNRC is not used,
 * this function doesn't need to be called to initialize the device.
 *
 * @param[in]  dev      the handle of the device to initialize
 ...
 * GNRC module is used this function won't have any effect, since the
 * device is automatically initialized on startup.
 *
 * @param[in]  dev      	pointer to winc1500 device descriptor
 * @param[in]  params       parameters for device initialization
 *
 * @return                  WINC1500_OK on success
 * @return                  WINC1500_ERR
 * @return                  WINC1500_ERR_SPI
 * @return                  WINC1500_ERR_NODEV
 * @return                  WINC1500_ERR_FW_VER_MISMATCH
 */
 ...
 * @return      -1      on error
 */
int winc1500_read_ap(winc1500_t *dev, winc1500_ap_t *ap_result, uint8_t ap_idx);
