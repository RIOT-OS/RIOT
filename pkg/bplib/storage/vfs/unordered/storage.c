...
static int _open_next_free(void)
{
    int fd;
    ...
    while (1) {
        ...
        if (fd == -EEXIST) {
             ...
             * A different / better strategy could be used to find a free slot.
            ...
            /* Return here both in failure and success case.
             * On the next call of this function the direct successor is tried, instead
             * of the search above. A collision of this will then trigger the search again. */
            next_id++;
            return fd;
        }
    }
...
free_bundle:
     ...
     * This means repetitive starting / stopping has to rebuild the cache (queue) every
 ...
 *
 * @param[inout] iterator Iterator which will hold the next result and the current state.
 * @retval 0 if iterator has been updated
 * @retval 1 if search is completed
 * @retval -ERRNO other negative errno errors
 */
 ...
 *
 * @note mutex of all queues is acquired before this function is invoked
 *
 * @param[in] dest_eids EID filter
 * @param num_eids EID filter count
 * @param[inout] queue queue to insert to
 */
static void _fill_bundle_queue(BPLib_EID_Pattern_t* dest_eids,
    size_t num_eids, bundle_queue_t* queue)
{
    ...
    while (queue->length < CONFIG_BPLIB_EGRESS_CACHE_LEN) {
        res = _next_bundle_path(&iterator);
 ...
 * @param[in] queue bundle storage queue (cache)
 * @param[out] path path of the bundle on vfs, to unlink outside of the function.
 *                  This is expected to begin with CONFIG_BPLIB_STOR_BASE "/"
    ...
    bool local_delivery, size_t* num_egressed)
{
    ...
    if (!queue->all_bundles_queued && queue->length == 0) {
        _fill_bundle_queue(dest_eids, num_eids, queue);
    }

    char path[BPLIB_STOR_PATHLEN] = CONFIG_BPLIB_STOR_BASE "/";
    while (_load_next_bundle(inst, &curr_bundle, queue, path) == BPLIB_SUCCESS) {
