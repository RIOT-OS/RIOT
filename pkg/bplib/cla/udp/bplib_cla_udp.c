...
static void* cla_udp_in(bplib_cla_udp_t * cla)
{
    ...
    while (cla->running && bplib_instance_data.running) {
        int rv = sock_udp_recv(&cla->sock, buffer, CONFIG_BPLIB_CLA_UDP_BUFLEN,
            CONFIG_BPLIB_CLA_UDP_TIMEOUT, NULL);

        if (rv > 0) {
            bp_status = BPLib_CLA_Ingress(&bplib_instance_data.BPLibInst, cla->contact_id,
                                          buffer, rv, 0);
...
int bplib_cla_udp_start(bplib_cla_udp_t* cla, uint32_t contact_id)
{
    ...
    local_ep.port = bplib_instance_data.ConfigPtrs.
                        ContactsConfigPtr->ContactSet[contact_id].ClaInPort;
