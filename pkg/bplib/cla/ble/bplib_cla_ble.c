...
/* Contact types for _change_bplib_contact_state() */
#define CONTACT_STATE_START 1
#define CONTACT_STATE_STOP  2
...
static volatile bool _manage_contact_state = false;
...
static bool _is_cla_running(void)
{
    return _cla_running && bplib_instance_data.running;
}

static void _change_bplib_contact_state(int state)
{
...
static int _on_gap_evt(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        _coc = NULL;
        _handle = 0;
        _change_bplib_contact_state(CONTACT_STATE_STOP);
        if (!_cla_running) {
            break;
        }
        try_connect_gap();
...
static int _on_l2cap_evt(struct ble_l2cap_event *event, void *arg)
{
    ...
    case BLE_L2CAP_EVENT_COC_CONNECTED: {
        ...
        _change_bplib_contact_state(CONTACT_STATE_START);
        /* On the initial connection, if this is a reconnection, the send loop
            * might be trapped, waiting for FLAG_TX_UNSTALLED. */
        thread_flags_set(_tx_thread, FLAG_UP | FLAG_TX_UNSTALLED);
        break;
    }
...
static void try_connect_l2cap(void)
{
    ...
    int rv = ble_l2cap_connect(_handle, CONFIG_BPLIB_CLA_BLE_CID, CONFIG_BPLIB_CLA_BLE_MTU,
        sdu_rx, _on_l2cap_evt, NULL);
    DEBUG("l2cap_connect() returned: %i\n", rv);

    if (rv == BLE_HS_ENOTCONN) {
        /* Handle is actually not valid, connect GAP again */
        _coc = NULL;
        _handle = 0;
        _change_bplib_contact_state(CONTACT_STATE_STOP);
        if (_cla_running) {
...
{
    _manage_contact_state = manage_state;
}
...
static void* cla_ble_out(void *arg)
{
    ...
    while (1) {
        if (_coc == NULL) {
            ...
            if (flags & FLAG_TERMINATE || !_is_cla_running()) {
                return NULL;
            }
        }
