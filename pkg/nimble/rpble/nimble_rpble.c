...
int nimble_rpble_param_update(const nimble_rpble_cfg_t *cfg)
{
    ...
    uint32_t itvl = random_uint32_range(cfg->eval_itvl_min, cfg->eval_itvl_max);
    ble_npl_time_ms_to_ticks(itvl, &_eval_itvl);

    _adv_params.conn_mode = BLE_GAP_CONN_MODE_UND;
    _adv_params.disc_mode = BLE_GAP_DISC_MODE_GEN;
    _adv_params.itvl_min = BLE_GAP_ADV_ITVL_MS(cfg->adv_itvl);
    _adv_params.itvl_max = _adv_params.itvl_min;

    _conn_params.scan_itvl = BLE_GAP_SCAN_ITVL_MS(cfg->conn_scan_itvl);
    _conn_params.scan_window = BLE_GAP_SCAN_WIN_MS(cfg->conn_scan_win);
    ...
    scan_params.itvl =  BLE_GAP_SCAN_ITVL_MS(cfg->scan_itvl);
    scan_params.window = BLE_GAP_SCAN_WIN_MS(cfg->scan_win);
