 ...
 * redefinition, so drop RIOT's definition and let MicroPython provide its
 * own. */
#undef NORETURN
...
/* MicroPython's long long big integer implementation only supports a 32 bit
 * word size, and it gains just a single bit over the small integers of a 64
 * bit target anyway, so use the arbitrary precision implementation there. */
