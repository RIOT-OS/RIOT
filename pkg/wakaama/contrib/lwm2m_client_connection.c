...
void *lwm2m_connect_server(uint16_t sec_obj_inst_id, void *user_data)
{
    ...
    DEBUG("[lwm2m_connect_server] Connecting to server in security instance %" PRIiSIZE "\n", sec_obj_inst_id);
                                                        ...
                                                        lwm2m_client_connection_type_t type)
{
    ...
    DEBUG("Looking for connection from [%s]:%" PRIiSIZE "\n", ip, remote->port);
                            ...
                            lwm2m_client_data_t *client_data)
{
    ...
    if (conn->type == LWM2M_CLIENT_CONN_UDP) {
        ...
        if (sent_bytes <= 0) {
            DEBUG("[_connection_send] Could not send DTLS packet: %" PRIiSIZE "\n", (int)sent_bytes);
            return -1;
        }
    }
                                                     ...
                                                     lwm2m_client_data_t *client_data)
{
    ...
    DEBUG("[_connection_create] Creating connection to Host: %.*s, Port: %" PRIiSIZE "\n",
    ...
    if (val == LWM2M_SECURITY_MODE_PRE_SHARED_KEY || val == LWM2M_SECURITY_MODE_RAW_PUBLIC_KEY) {
        ...
        if (res != -SOCK_DTLS_HANDSHAKE) {
            DEBUG("[lwm2m:client] error creating session: %" PRIiSIZE "\n", res);
            goto free_out;
        }
