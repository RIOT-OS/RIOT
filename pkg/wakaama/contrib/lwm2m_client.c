...
static credman_tag_t _get_credential(const sock_udp_ep_t *ep, uint8_t security_mode)
{
    ...
    while (instance) {
        ...
        if (res < 0) {
            DEBUG("[lwm2m:client] could not get security mode of %" PRIiSIZE "\n", instance->id);
            goto check_next;
        }
        ...
        if (res < 0) {
            DEBUG("[lwm2m:client] could not get URI of %" PRIiSIZE "\n", instance->id);
            goto check_next;
        }
        ...
        if (sock_udp_ep_equal(ep, &inst_ep)) {
            DEBUG("[lwm2m:client:PSK] found matching EP on instance %" PRIiSIZE "\n", instance->id);
            DEBUG("[lwm2m:client:PSK] tag: %" PRIiSIZE "\n",
                                       ...
                                       size_t len)
{
    ...
    if (conn) {
        DEBUG("[lwm2m:client] handle packet (%" PRIiSIZE " bytes)\n", (int)len);
        int result = lwm2m_connection_handle_packet(conn, buffer, len, _client_data);
        if (0 != result) {
            DEBUG("[lwm2m:client] error handling message %" PRIiSIZE "\n", result);
        }
    }
    else {
...
static void _udp_event_handler(sock_udp_t *sock, sock_async_flags_t type, void *arg)
{
    ...
    if (type & SOCK_ASYNC_MSG_RECV) {
        ...
        if (rcv_len <= 0) {
            DEBUG("[lwm2m:client] UDP receive failure: %" PRIiSIZE "\n", (int)rcv_len);
            return;
        }
...
static void _dtls_event_handler(sock_dtls_t *sock, sock_async_flags_t type, void *arg)
{
    ...
    if (type & SOCK_ASYNC_MSG_RECV) {
        ...
        if (rcv_len <= 0) {
            DEBUG("[lwm2m:client] DTLS receive failure: %" PRIiSIZE "\n", (int)rcv_len);
            return;
        }
...
void lwm2m_client_add_credential(credman_tag_t tag)
{
    ...
    DEBUG("[lwm2m:client] trying to add credential with tag %" PRIiSIZE "\n", tag);
...
{
    DEBUG("[lwm2m:client] removing credential with tag %" PRIiSIZE "\n", tag);
...
void lwm2m_client_refresh_dtls_credentials(void)
{
    ...
    while (instance) {
        ...
        if (res < 0) {
            DEBUG("[lwm2m:client:refresh_cred] could not get security mode of %" PRIiSIZE "\n", instance->id);
        }
        else {
