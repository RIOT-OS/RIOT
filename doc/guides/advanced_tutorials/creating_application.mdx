...
for the project. You can add application headers at the same level as your
`main.c` file or in an `include/` subdirectory to keep the application's root
folder cleaner, if many source files and/or headers are present.
The modules directory could contain extra modules for the applications.
