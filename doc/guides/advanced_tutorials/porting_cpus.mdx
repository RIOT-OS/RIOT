...
- validate one timer instance (for example with `tests/periph/timer`) so
  `ztimer` works (you can run `examples/basic/blinky` and check that `ztimer`
  is invoked during the compilation),
...
- `tests/periph/timer` for `periph_timer`,
- `tests/sys/ztimer_msg` or other `tests/sys/ztimer_*` for `ztimer`,
- `tests/periph/i2c` and `tests/periph/spi` for small SPI or I2C loopback /
  device testing once the bus drivers are added.
