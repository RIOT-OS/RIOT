...
`Makefile` or by setting `PKG_SOURCE_LOCAL_<PKG_NAME>` in your environment.
The package name has to be specified in capital letters and hypens have to be
replaced with underscores. For the `awesome-package` package, you will have to
use `PKG_SOURCE_LOCAL_AWESOME_PACKAGE`.

```shell
PKG_SOURCE_LOCAL_<PKG_NAME>=~/customized_package_folder make -C tests/pkg/<pkgname>
```
...
patch files as follows:
 ...
 - Commit your changes using `git commit -m "your commit message"`.
   Remember to add a detailed description in your commit, as this will be
   added to the patch file as well.
 - Create the patch files using `git format-patch -<n> riot-port`.
   Replace `<n>` with the number of commits you have made previously.
   A separate patch file will be created for each commit.
...
:::note
While it is possible to perform changes in the `build/` directory itself and
to create the patches there, keep in mind that a rebuild or `make clean` will
override or delete the changes you have made in that folder!
Use the `PKG_SOURCE_LOCAL` variable instead (as described above).
:::

Try to keep each patch a reasonable size to help with the review process.
...
you can remove that patch file individually, without having to modify the
other patches.
...
## Detecting RIOT Builds

External code that is targeting multiple embedded systems may need to make use
of conditional compilation to use the code compatible with the current target
OS. For this reason, RIOT adds `-DRIOT_OS=1` to `CFLAGS`. It can be used
like:
...
static void say_hi(void)
{
    ...
    puts("Hi from RIOT!");
#else
    puts("Hi from an OS that is not RIOT!");
#endif
}
```
