...
:::note
This tutorial explains how to use `gcoap`, RIOT's longstanding CoAP implementation.
Note that we are currently developing `unicoap`, a new CoAP implementation
that features a more streamlined API, additional functionality & modularity.

The `unicoap` tutorial can be found [HERE](/networking/unicoap/introduction).
:::
...
```bash
BOARD=adafruit-feather-nrf52840-sense make all flash term
```
