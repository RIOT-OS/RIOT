---
title: Using Message APIs
description: A demo of the `unicoap` message APIs.
---
...
[`unicoap_message_properties_t::rfc7252`](https://api.riot-os.org/structunicoap__message__properties__t.html)
member.
...
You use the
`unicoap_message_code_is_request`,
`unicoap_message_code_is_response`, and
`unicoap_message_code_is_signal`
...
`unicoap_message_t.method`,
`unicoap_message_t.status`, and
`unicoap_message_t.signal`.
...
without checking the message class first, use `unicoap_string_from_code`.
...
[`unicoap_options_get_content_format`](https://api.riot-os.org/group__net__unicoap__options.html).
...
`unicoap_options_t::unicoap_options_add_uri_queries` and
`unicoap_options_t::unicoap_options_add_uri_queries_string`, or
`unicoap_options_t::unicoap_options_add_uri_query` and
`unicoap_options_t::unicoap_options_add_uri_query_string`.
...
:::note
In this very simple scenario, we don't use a token. Very constrained nodes
