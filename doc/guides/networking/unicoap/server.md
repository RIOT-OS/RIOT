---
title: CoAP Server
description: Register CoAP resources and handle requests asynchronously.
---
...
2. Alternatively, you can create a new listener and manually add resources as follows.
   Listeners are groups of resources. Internally, all statically declared resources form a
   built-in listener.
...
resources. `CONFIG_UNICOAP_WELL_KNOWN_CORE` allows you to customize the default behavior.

You can also customize the link encoding per listener. By default, listeners with the
`unicoap_listener_t.link_encoder` property unset, use the built-in link encoder.
...
resource to a subset of transports, set `unicoap_resource_t::protocols` to a value produced
by `UNICOAP_PROTOCOLS`. Please note that `unicoap` considers an unset `protocols` property
to mean "All protocols". If you want to make this explicit, use `UNICOAP_PROTOCOLS_ALLOW_ALL` or
`UNICOAP_PROTOCOLS_ALLOW_NONE`.

The server then checks the path of the resource. If you supplied the
`UNICOAP_RESOURCE_FLAG_MATCH_SUBTREE` flag, a resource registered for `/foo/bar` will also match
...
To do this, set the `unicoap_listener_t.request_matcher` property. By default, if this property
...
1. **Return status code**: If you don't want to send a response payload and have no
   options you may want to set, you can just return a `unicoap_status_t`. Note that this requires
