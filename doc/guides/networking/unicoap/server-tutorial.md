---
title: Writing a Server Application
description: Learn to write a simple server application with `unicoap`.
---
...
In this tutorial, we choose
[Generic (GNRC) network stack](https://api.riot-os.org/group__net__gnrc.html).
...
host/domain and port in the CoAP URI. To assign a path, set the `unicoap_resource_t.path`
property to `UNICOAP_PATH_ROOT` or a custom path using `UNICOAP_PATH`.
...
as `UNICOAP_PATH("gelato", "flavours", "menu")` using `UNICOAP_PATH`. Never include a slash
...
all methods permitted as parameters to the `UNICOAP_METHODS` macro. You cannot leave
`unicoap_resource_t.methods` empty. If you later send a request that does not match the set of
...
To do this, we pass the `UNICOAP_RESOURCE_FLAG_RELIABLE` flag.
...
address, a context and an optional argument (`unicoap_resource_t.handler_arg`).
...
`unicoap_string_from_method` to get a string representation of the CoAP method, i.e., the CoAP
message code, and `unicoap_message_t::unicoap_message_payload_get_size`
...
To do this, we use `unicoap_options_t::unicoap_options_get_first_uri_query_by_name`.
...
client the query value was invalid by leveraging the `unicoap_send_response` function and a
...
we do that first.
We allocate options on the stack through `UNICOAP_OPTIONS_ALLOC` and set the format with
`unicoap_options_t::unicoap_options_set_content_format`.
...
using `unicoap_transport_dtls_get_socket`.
...
:::tip
`unicoap` will emit debug logs depending on `CONFIG_UNICOAP_DEBUG_LOGGING` and
`CONFIG_UNICOAP_ASSIST`. These settings are enabled in the sample code in `app.config`.
:::
