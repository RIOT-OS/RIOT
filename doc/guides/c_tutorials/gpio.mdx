...
So far we have been running all our code using the `adafruit-feather-nrf52840-sense` board.

In this tutorial, we will learn how to use the GPIO pins on a real board
to control LEDs or read button presses.

For this tutorial we will be using a the `adafruit-feather-nrf52840-sense` board
...
```bash
BOARD=adafruit-feather-nrf52840-sense make all
```
...
but in the case of the `adafruit-feather-nrf52840-sense` board,
...
:::tip
If you are using the `adafruit-feather-nrf52840-sense` board,
...
For example, the `adafruit-feather-nrf52840-sense` board has the `LED0_TOGGLE`
