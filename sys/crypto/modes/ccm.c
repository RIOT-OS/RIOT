...
int ccm_compute_adata_mac(cipher_t *cipher, uint8_t* auth_data,
        uint8_t auth_data_len, uint8_t X1[16])
{
    int len;
    uint8_t len_encoding = 0,
            auth_data_encoded[26]; // 16 octet block size + max. 10 len encoding

    if (auth_data_len > 0) {
        if ( auth_data_len < pow(2,16)) {       // length (0x0001 ... 0xFEFF)
            len_encoding = 2;

            auth_data_encoded[1] = auth_data_len & 0xFF;
            auth_data_encoded[0] = (auth_data_len>>8) & 0xFF;
        } else if(auth_data_len < pow(2,32)) {  // 0xFF 0xFE (length 4 octets)
            len_encoding = 6;
            printf("UNSUPPORTED Adata");
            return -1;
            
        } else if(auth_data_len < pow(2,64)) {  // 0xFF 0xFF (length 8 octets)
            len_encoding = 10;
            printf("UNSUPPORTED Adata");
            return -1;
        } else {
...
int cipher_encrypt_ccm(cipher_t* cipher, uint8_t *auth_data, uint8_t auth_data_len,
                  ...
                  uint8_t* output)
{
    ...
    if (length_encoding < 2 || length_encoding > 8 ||
          input_len - auth_data_len > pow(2, 8 * length_encoding))
