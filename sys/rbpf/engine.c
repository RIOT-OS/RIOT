...
static bool _check_mem(const rbpf_application_t *rbpf, const intptr_t addr, size_t size, uint8_t type)
{
...
//#define CONT_JUMP  return _rbpf_jump(rbpf, instr, regmap, jump_cond) /* Execute the jump and continue */
#define CONT_JUMP \
    if (jump_cond) {\
        return _rbpf_jump(rbpf, instr, regmap); \
    }\
    break;
...
int rbpf_engine_run(rbpf_application_t* rbpf, const void *ctx, int64_t *result)
{
    ...
    res = _rbpf_instruction(rbpf, instr+1, regmap);
    *result = regmap[0];
    return res;
}

static inline int _rbpf_max_jumps(const rbpf_application_t* rbpf)
{
    return !(rbpf->flags & RBPF_CONFIG_NO_RETURN) && rbpf->branches_remaining == 0;
}

/* Must be inlined to make the tail call optimization work */
static inline __attribute__((always_inline)) int _rbpf_jump(rbpf_application_t* rbpf, const bpf_instruction_t *instr, uint64_t regmap[11])
{
    instr += instr->offset;
    rbpf->branches_remaining--;
    if (_rbpf_max_jumps(rbpf)) {
        return RBPF_OUT_OF_BRANCHES;
    }
    return _rbpf_instruction(rbpf, instr, regmap);
}
