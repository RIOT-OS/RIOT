...
static uint8_t local_slot = FW_SLOT;
static ota_request_t ota_request;
static char update_name[FW_NAME_LENGTH];
    ...
    if (pdu->payload_len) {
        if (pdu->content_type == COAP_FORMAT_TEXT
            ...
            printf(", %u bytes\n%.*s\n", pdu->payload_len, pdu->payload_len,
                                                          (char *)pdu->payload);
            puts("Paylod is not in the right format!");
        }
        else {
            switch (ota_request) {
                case COAP_GET_VERSION:
                    remote_version = strtol((const char*)pdu->payload, NULL, 16);
                    printf("[ota_update] Got remote version %#x\n", remote_version);
                    sema_post(&sema_fw_version);
                    break;

                case COAP_GET_NAME:
                    strcpy(update_name, (char *)pdu->payload);
...
static void *ota_update_start(void *arg)
{
    ...
    if (fw_slots_get_int_slot_metadata(local_slot, &fw_metadata) == -1) {
        puts("[ota_update] Error! Cannot retrieve local metadata\n");
        return 0;
    }
    else {
        local_appid = fw_metadata.appid;
        local_version = fw_metadata.version;
