...
#ifdef MODULE_NETSTATS_NEIGHBOR
static int _netif_stats_nb(kernel_pid_t iface)
{
    netstats_nb_t *stats;
    int res = gnrc_netapi_get(iface, NETOPT_STATS_NEIGHBOR, 0, &stats, sizeof(&stats));
    if (res == -ENOTSUP) {
        return res;
    }
...
static int _netif_set_state(kernel_pid_t iface, char *state_str)
{
    ...
    printf("success: set state of interface %" PRIkernel_pid " to ", iface);
    _print_netopt_state(state);
    puts("");
