...
/**
 * @brief   allocates memory for a new list entry and appends it before the
 *          head. The new entry is the new head.
 *
 * @param   head    pointer to the list
 * @return  the new list entry, NULL if no new list entry could be allocated
 */
#define simple_list_add_head(head) \
    __simple_list_add_head((struct simple_list_elem**) \
            (head), calloc(1, sizeof **(head)))


/**
 * @brief   appends a preallocated element to the top of the list. The new
 ...
 */
#define simple_list_set_head(head, node) \
    __simple_list_add_head((struct simple_list_elem**) (head), (node))

/**
* @brief    allocates memory for a new list entry and appends it at the end of
*           the list.
*
...
* @return   the new list entry, NULL if no new list entry could be allocated
*/
#define simple_list_add_tail(head) \
    __simple_list_add_tail((struct simple_list_elem**) \
            (head), calloc(1, sizeof **(head)))
 ...
 * @return  the new list entry (node)
 */
#define simple_list_set_tail(head, node) \
    __simple_list_add_tail((struct simple_list_elem**) (head), (node))

/**
* @brief    allocates memory for a new list entry and adds it before an
*           existing entry. The new entry is added before the existing element
*           where old_entry->value > value
*           If no such entry could be found, the new entry will be added at the
*           end of the list
*
* @param    head    pointer to the list
* @param    value   value to compare list entries
*                   has to be the same name as the element in the list entry
*                   structure that is be used for comparison
*
* @return   pointer to the new element, NULL if no new list element could be
*           allocated
*/
...
*           The new entry is added before the existing element where
...
*                   has to be the same name as the value in the list element
*                   struct
*
...
/**
* @brief    searches for a list element by comparing a buffer in the list
*           element struct
*
* @param    head    pointer to the list
...
/**
* @brief    searches for a list element by applying a comparator function to
*           each list entry
*
* @param    head    pointer to the list
* @param    value   input to the comparator function
* @param    comperator  a function that takes (value, node) and returns 0 if
*                       they match
* @return   pointer the list entry if found, otherwise NULL
*/
 ...
 */
#define simple_list_remove(head, node) \
    __simple_list_remove((struct simple_list_elem**) \
            (head), (struct simple_list_elem*) (node), 0)

/**
 * @brief removes an entry from the list, doesn't free it's memory but returns
 *        the element
 *
 * @param   head    pointer to the list
 ...
 */
#define simple_list_extract(head, node) \
    __simple_list_remove((struct simple_list_elem**) \
            (head), (struct simple_list_elem*) (node), 1)
 ...
 * @param   head    pointer to the list
 */
#define simple_list_clear(head) \
    __simple_list_clear((struct simple_list_elem**) (head))

/**
 * @brief   starts a loop to iterate over all list entries. Read-only list
 *          access only. needs to be provided with a local loop variable
 *
 * @param   head    pointer to the list
 * @param   node    to the current entry (loop variable)
 */
#define simple_list_for_each(head, node)    \
    for ((node) = (head); (node); (node) = (node)->next)

/**
 * @brief   starts a loop to iterate over all list elements with the
 *          possibility to remove elements
 *          to remove an element, use simple_list_for_each_remove
 *          needs to be provided with a local loop variable as well as two
 *          local auxiliary variables
 *
 * @param   head    pointer to the list
 * @param   node    to the current entry (loop variable)
 * @param   prev    internal variable, pointer to previous list entry - do not
 *                  modify
