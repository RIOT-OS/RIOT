...
struct ieee802154_submac {
    ...
    ieee802154_csma_be_t be;            /**< CSMA-CA backoff params */
    ...
    uint8_t nb;                         /**< current number of CSMA-CA retries */
 ...
 *
 * This is a shortcut to @ref ieee802154_set_addresses
 *
 * @param[in] submac pointer to the SubMAC descriptor
 ...
 * @return negative errno on error
 */
static inline int ieee802154_set_short_addr(ieee802154_submac_t *submac, const network_uint16_t *short_addr)
{
    int res = ieee802154_radio_set_hw_addr_filter(submac->dev, short_addr, NULL, NULL);
    if (res >= 0) {
 ...
 * @return negative errno on error
 */
static inline int ieee802154_set_ext_addr(ieee802154_submac_t *submac, const eui64_t *ext_addr)
{
    int res = ieee802154_radio_set_hw_addr_filter(submac->dev, NULL, ext_addr, NULL);
    if (res >= 0) {
...
{
    int res = ieee802154_radio_set_hw_addr_filter(submac->dev, NULL, NULL, &panid);
    if (res >= 0) {
        submac->panid = panid;
    }
 ...
 * @return negative errno on error
 */
static inline int ieee802154_set_channel_number(ieee802154_submac_t *submac, uint16_t channel_num)
{
    return ieee802154_set_phy_conf(submac, channel_num, submac->channel_page, submac->tx_pow);
}
 ...
 * @return negative errno on error
 */
static inline int ieee802154_set_channel_page(ieee802154_submac_t *submac, uint16_t channel_page)
{
    return ieee802154_set_phy_conf(submac, submac->channel_num, channel_page, submac->tx_pow);
}
 ...
 * @return negative errno on error
 */
static inline int ieee802154_set_tx_power(ieee802154_submac_t *submac, int8_t tx_pow)
{
    return ieee802154_set_phy_conf(submac, submac->channel_num, submac->channel_page, tx_pow);
}
 ...
 * @return negative errno on error
 */
static inline int ieee802154_read_frame(ieee802154_submac_t *submac, void *buf, size_t len, ieee802154_rx_info_t *info)
{
 ...
 */
extern void ieee802154_submac_ack_timer_set(ieee802154_submac_t *submac, uint16_t us);
