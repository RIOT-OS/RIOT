...
{
    const coap_udp_hdr_t *hdr = (const coap_udp_hdr_t *)pkt->buf;
    ...
    const uint8_t *ext = pkt->buf + sizeof(coap_udp_hdr_t);
    switch (tkl) {
    case 13:
        return tkl + *ext;
    case 14:
...
{
    return coap_build_udp_hdr(hdr, sizeof(*hdr) + 8, type, token, token_len, code, id);
}
