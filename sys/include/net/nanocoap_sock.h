...
#  endif
    ...
    void *tcp_buf;              /**< recv buffer for TCP socket (output of malloc()) */
...
typedef struct {
    union {
#if MODULE_NANOCOAP_UDP || MODULE_NANOCOAP_DTLS || DOXYGEN
        sock_udp_ep_t remote_udp; /**< UDP remote to send response to   */
#endif
#if MODULE_NANOCOAP_TCP || DOXYGEN
        sock_tcp_ep_t remote_tcp; /**< TCP remote to send response to   */
#endif
    };
#if (defined(MODULE_SOCK_AUX_LOCAL) && MODULE_NANOCOAP_UDP) || DOXYGEN
 ...
 *
 * @pre The transport used is UDP/DTLS. (There are not separate responses in
 ...
 */
void nanocoap_server_prepare_separate(nanocoap_server_response_ctx_t *ctx, coap_pkt_t *pkt,
