...
/**
 * @ingroup     sys_registry_find
 * @brief       RIOT Registry Find module for finding specific nodes within the
 *              configuration tree
 * @{
 *
 * @file
    ...
    registry_node_t *node)
{
    ...
    if (schema != NULL) {
        /* Instance */
        const registry_schema_instance_t *instance = NULL;
        clist_node_t *instance_node = schema->instances.next;
        if (instance_node) {
            do {
                instance_node = instance_node->next;
                instance = container_of(instance_node, registry_schema_instance_t, node);
