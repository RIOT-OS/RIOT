...
int sixlowpan_legacy_lowpan_border_init(int if_id)
{
    ...
    while (net_if_iter_addresses(if_id, (net_if_addr_t **) &addr)) {
        if (!ipv6_legacy_addr_is_multicast(addr->addr_data) &&
            !ipv6_legacy_addr_is_link_local(addr->addr_data) &&
            !ipv6_legacy_addr_is_loopback(addr->addr_data) &&
            !ipv6_legacy_addr_is_unique_local_unicast(addr->addr_data)) {
...
void border_process_lowpan(void)
{
    ...
    while (1) {
        msg_receive(&m);
        ipv6_legacy_hdr_t *ipv6_legacy_buf = (ipv6_legacy_hdr_t *)m.content.ptr;

        if (ipv6_legacy_buf->nextheader == IPV6_LEGACY_PROTO_NUM_ICMPV6) {
            icmpv6_hdr_t *icmp_buf = (icmpv6_hdr_t *)(((uint8_t *)ipv6_legacy_buf) + IPV6_LEGACY_HDR_LEN);
        ...
        multiplex_send_ipv6_legacy_over_uart(ipv6_legacy_buf);
    }
}
