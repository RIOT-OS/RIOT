...
int net_if_get_eui64(net_if_eui64_t *eui64, int if_id, int force_generation)
{
    ...
    if (eui64->uint64 == 0 || force_generation) {
        ...
        if (pan_id >= 0) {
            // 16bit Pan-ID:16-zero-bits:16-bit-short-addr = 48bit (RFC 4944)
            eui64->uint16[0] = HTONS((uint16_t)pan_id);
        }
        else {
            // if transceiver has no PAN ID
            eui64->uint16[0] = 0;
        }
