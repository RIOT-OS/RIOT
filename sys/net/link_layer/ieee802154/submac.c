...
#define ACK_TIMEOUT                     (864U)

#define IEEE802154_CCA_THRESH_DEFAULT   (-70)

#if IS_USED(MODULE_IEEE802154_SUBMAC)

static void _handle_tx_no_ack(ieee802154_submac_t *submac);

static void _tx_end(ieee802154_submac_t *submac, int status, ieee802154_tx_info_t *info)
{
...
{
    return ieee802154_radio_has_frame_retrans(dev) || ieee802154_radio_has_irq_ack_timeout(dev);
}
...
int ieee802154_csma_ca_transmit(ieee802154_submac_t *submac)
{
    ...
    if (ieee802154_radio_has_auto_csma(dev) || ieee802154_radio_has_frame_retrans(dev)) {
...
void ieee802154_submac_rx_done_cb(ieee802154_submac_t *submac)
{
    ...
    if (!_does_handle_ack(dev) && submac->wait_for_ack) {
        ...
        if (ieee802154_radio_indication_rx(dev, ack, 3, NULL) &&
            ...
            ieee802154_radio_set_rx_mode(submac->dev, IEEE802154_RX_AACK_ENABLED);
            _tx_end(submac, fp ? TX_STATUS_FRAME_PENDING : TX_STATUS_SUCCESS, &tx_info);
        }
    }
    else {
...
static void _handle_tx_success(ieee802154_submac_t *submac, ieee802154_tx_info_t *info)
{
    ieee802154_dev_t *dev = submac->dev;
    ieee802154_radio_request_set_trx_state(dev, IEEE802154_TRX_STATE_RX_ON);
    while (ieee802154_radio_confirm_set_trx_state(dev) == -EAGAIN);

    if (ieee802154_radio_has_frame_retrans(dev) || ieee802154_radio_has_irq_ack_timeout(dev) || !submac->wait_for_ack) {
        ...
        ieee802154_submac_ack_timer_set(submac, ACK_TIMEOUT);
    }
}
...
{
    ieee802154_dev_t *dev = submac->dev;
    if (ieee802154_radio_has_frame_retrans(dev) || ieee802154_radio_has_auto_csma(dev)) {
        ieee802154_radio_request_set_trx_state(dev, IEEE802154_TRX_STATE_RX_ON);
        _tx_end(submac, TX_STATUS_MEDIUM_BUSY, NULL);
    }
    else {
...
int ieee802154_send(ieee802154_submac_t *submac, const iolist_t *iolist)
{
    ...
    if (submac->tx || ieee802154_radio_request_set_trx_state(dev, IEEE802154_TRX_STATE_TX_ON) < 0) {
        return -EBUSY;
    }
...
int ieee802154_submac_init(ieee802154_submac_t *submac)
{
    ...
    ieee802154_radio_set_hw_addr_filter(dev, &submac->short_addr, &submac->ext_addr, &submac->panid);

    /* Configure PHY settings (channel, TX power) */
    ieee802154_phy_conf_t conf = {.channel=CONFIG_IEEE802154_DEFAULT_CHANNEL, .page=CONFIG_IEEE802154_DEFAULT_CHANNEL, .pow=CONFIG_IEEE802154_DEFAULT_TXPOWER};

    ieee802154_radio_config_phy(dev, &conf);
    assert(ieee802154_radio_set_cca_threshold(dev, IEEE802154_CCA_THRESH_DEFAULT) >= 0);
...
{
    ieee802154_dev_t *dev = submac->dev;
    ieee802154_phy_conf_t conf = {.channel=channel_num, .page=channel_page, .pow=tx_pow};
