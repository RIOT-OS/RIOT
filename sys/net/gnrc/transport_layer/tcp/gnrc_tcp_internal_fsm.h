...
static int8_t _translate_to(gnrc_tcp_tcb_t *tcb, gnrc_tcp_states_t state){
    msg_t msg;
...
static int8_t _fsm(gnrc_tcp_tcb_t *tcb, gnrc_pktsnip_t *pkt, gnrc_tcp_event_t event)
{
    ...
    if(event == RCVD_PKT){
        ...
        switch(tcb->state){
            ...
            case LISTEN:
                ...
                LL_SEARCH_SCALAR(pkt, snp, type, GNRC_NETTYPE_IPV6);
                ...
                if(ctl & MSK_SYN){
                    ...
                    if(gnrc_netreg_num(GNRC_NETTYPE_TCP, _build_context(src, dst)) > 0){
                        break;
                    }
    ...
    if(event == TIME_RETRANSMIT){
        ...
        if(tcb->ret_queue[0].no_of_retries < GNRC_TCP_MAX_RETRANSMITS){
            _send_pkt(tcb, true);
        }else{
            _translate_to(tcb, CLOSED);
        }
