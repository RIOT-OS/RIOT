                                         ...
                                         gnrc_sixlowpan_frag_fb_t *fbuf)
{
    ...
    if (ipv6_addr_is_link_local(ipv6_dst)) {
        ...
        DEBUG("6lo minfwd: link-local address is not forwarded\n");
        fbuf->pkt = NULL;
        return -1;
    }
    if (fbuf->datagram_size <= fbuf->best_frag_size) {
        DEBUG("6lo minfwd: no fragmentation necessary (%u < %u)\n",
              fbuf->datagram_size, fbuf->best_frag_size);
        fbuf->pkt = NULL;
        return -1;
    }
