...
void gnrc_sixlowpan_frag_sfr_inter_frame_gap(gnrc_sixlowpan_frag_fb_t *fbuf)
{
    if (gnrc_sixlowpan_frag_sfr_congure_snd_has_inter_frame_gap()) {
        ...
        if (node != NULL) {
            _last_frame_sent = ztimer_now(ZTIMER_MSEC);
...
{
    uint32_t now;
    uint32_t if_gap = gnrc_sixlowpan_frag_sfr_congure_snd_inter_frame_gap(fbuf) / US_PER_MS;
...
static void _sched_next_frame(gnrc_sixlowpan_frag_fb_t *fbuf)
{
    ...
    uint32_t last_sent_since = (_last_frame_sent - ztimer_now(ZTIMER_MSEC));
    uint32_t if_gap = gnrc_sixlowpan_frag_sfr_congure_snd_inter_frame_gap(fbuf) / US_PER_MS;
