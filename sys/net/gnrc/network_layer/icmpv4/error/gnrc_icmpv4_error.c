...
#define ICMPV4_ERROR_SET_VALUE(data, value) \
    ((icmpv4_error_pkt_too_big_t *)(data))->mtu = byteorder_htonl(value)
 ...
 *
 * Get's the minimum size for an ICMPv4 error message packet, based on the
...
static inline gnrc_pktsnip_t *_pkt_too_big_build(uint32_t mtu,
                                                 const gnrc_pktsnip_t *orig_pkt)
{
    return _icmpv4_error_build(ICMPV4_PKT_TOO_BIG, 0, orig_pkt, mtu);
}
...
static gnrc_pktsnip_t *_param_prob_build(uint8_t code, void *ptr,
                                         const gnrc_pktsnip_t *orig_pkt)
{
    gnrc_pktsnip_t *pkt = gnrc_icmpv4_build(NULL, ICMPV4_PARAM_PROB, code,
                                            _fit(orig_pkt));
                                        ...
                                        const gnrc_pktsnip_t *orig_pkt)
{
    ...
    if (ipv4 != NULL) {
        ...
        DEBUG("gnrc_icmpv4_error: trying to send packet too big error\n");
        _send(pkt, orig_pkt, ipv4);
    }
}
