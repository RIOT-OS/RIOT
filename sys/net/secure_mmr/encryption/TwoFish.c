...
//prototype
static int TwoFish_set_key(TWOFISH_context *ctx, byte *key, uint8_t keylen);
...
int TwoFish_init(CipherContext *context, uint8_t blockSize, uint8_t keySize,
                 uint8_t *key)
{
    //printf("%-40s: Entry\r\n", __FUNCTION__);
    // 16 byte blocks only
...
int TwoFish_setup_key(CipherContext *context, uint8_t *key, uint8_t keysize)
{
    return TwoFish_init(context, TwoFishInfo_get_preferred_block_size(),
                        keysize, key);
}
...
int TwoFish_encrypt(CipherContext *context, uint8_t *in, uint8_t *out)
{
    ...
    TWOFISH_context *ctx = malloc(sizeof(TWOFISH_context));
...
uint8_t TwoFishInfo_get_preferred_block_size()
{
