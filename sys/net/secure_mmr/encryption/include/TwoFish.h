/*
 * TwoFish.h - The headers for thr TwoFish Cipher-Algorithm.
 *
 ...
 */
int TwoFish_init(CipherContext *context, uint8_t block_size, uint8_t key_size,
                 uint8_t *key);
 ...
 * @return SUCCESS
 */
int TwoFish_setup_key(CipherContext *context, uint8_t *key, uint8_t key_size);
 ...
 */
int TwoFish_encrypt(CipherContext *context, uint8_t *in, uint8_t *out);
 ...
 *          include not calling init()
 */
int TwoFish_decrypt(CipherContext *context, uint8_t *in, uint8_t *out);
 ...
 */
uint8_t TwoFishInfo_get_preferred_block_size(void);

/**
 * Interface to access the functions
 *
 */
block_cipher_interface_t TWOFISHInterface = {
    "TWOFISH\0",
    TwoFish_init,
    TwoFish_encrypt,
    TwoFish_decrypt,
    TwoFish_setup_key,
    TwoFishInfo_get_preferred_block_size
};
