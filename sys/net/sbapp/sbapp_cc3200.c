...
//#define STOP_IF_ERR(ERR_ID) if(ERR_ID < 0) PANIC(ERR_ID)
...
const char* sl_err_descr[] = {
        ...
        [WLAN_START_ERR] = "unable to start simplelink task",
 ...
 */
static void
_send(cd_t *cd, gnrc_pktsnip_t *pkt);

static void *
_event_loop(void *arg);
...
// todo: to be removed
void
SimpleLinkHttpServerCallback(SlHttpServerEvent_t *pSlHttpServerEvent,
 ...
 */
void
SimpleLinkWlanEventHandler(SlWlanEvent_t *pWlanEvent) {
    msg_t msg;
...
//*****************************************************************************
void
SimpleLinkNetAppEventHandler(SlNetAppEvent_t *ev) {
    msg_t msg;
...
sbapp_init(void) {
    ...
    if (sbapp.main_pid == KERNEL_PID_UNDEF) {
        ...
        THREAD_CREATE_STACKTEST,
                                       _event_loop, NULL, "sbapp");
    }
    return sbapp.main_pid;
}

int16_t
connect_to(const char* server, uint16_t port) {
    SlSockAddrIn_t addr;
    int iAddrSize;
...
static void
_receive(cd_t* cd, gnrc_pktsnip_t *pkt) {
}

static void *
_send_handler_task(void *arg) {
    msg_t msg, reply;
 ...
 */
static void *
_receive_handler_task(void* arg) {
...
sbh_t
sbapp_connect(const char* server, uint16_t port, kernel_pid_t pid) {
    ...
    GNRC_SBAPP_PRIO,
                                   THREAD_CREATE_STACKTEST, _send_handler_task,
                                   conn, server);
    ...
    GNRC_SBAPP_PRIO,
                                   THREAD_CREATE_STACKTEST,
                                   _receive_handler_task, conn, server);

    return conn;
}

void
sbapp_close(sbh_t fd) {
    cd_t *handle = (cd_t *) fd;
...
int
sbapp_send(sbh_t fd, void* data, size_t len) {
    msg_t msg;
    cd_t *handle = (cd_t *) fd;
