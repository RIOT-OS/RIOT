...
static int _request_done_cb(void *arg, coap_pkt_t *pkt)
{
    ...
    for (unsigned i = 0; i < pkt->options_len; i++) {
        if (pkt->options[i].opt_num & 0x1) {
            DEBUG_PUTS("[reverse proxy] No option handling for reply, but critical option detected");
            const char *msg = "opt in reply";
...
static void _forward_request_handler(event_t *ev)
{
    ...
    DEBUG("[reverse proxy] Forwarded request: %" PRIdSIZE "\n", err);
    if (err < 0) {
        char errnum[8];
        nanocoap_server_send_separate(&conn->response_ctx,
                                      COAP_CODE_INTERNAL_SERVER_ERROR,
                                      COAP_TYPE_NON, errnum,
                                      fmt_s32_dec(errnum, err));
    }
    _disconnect(conn);
}
                                ...
                                coap_request_ctx_t *ctx)
{
    ...
    const char *ep = uri + strlen(my_path);
    ...
    ep++;
    const char *uri_remainder = NULL;
    const char *ep_end = strchr(ep, '/');
    if (ep_end) {
        uri_remainder = ep_end + 1;
    }
    else {
        ep_end = ep + strlen(ep);
    }
