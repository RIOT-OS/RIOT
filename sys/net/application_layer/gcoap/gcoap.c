...
static void _listen(sock_udp_t *sock)
{
    ...
    case COAP_CLASS_SERVER_FAILURE:
        _find_req_memo(&memo, &pdu, &remote, GCOAP_FIND_REQ_TOKEN);
        if (memo) {
 ...
 * array. Matches on token or message ID based on match_type parameter, plus
 * remote endpoint.
 *
                           ...
                           const sock_udp_ep_t *remote, int match_type)
{
    *memo_ptr = NULL;
    ...
    for (int i = 0; i < GCOAP_REQ_WAITING_MAX; i++) {
        ...
        if (match_type == GCOAP_FIND_REQ_TOKEN) {
        ...
        else if (match_type == GCOAP_FIND_REQ_MSGID) {
