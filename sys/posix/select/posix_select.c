...
static int _set_timeout(ztimer_t *timeout_timer, struct timeval *timeout,
                        uint32_t offset, bool *wait)
{
    if (timeout != NULL) {
        ...
        if (t > UINT32_MAX) {
            ...
            ztimer_set_timeout_flag(ZTIMER_USEC, timeout_timer, (uint32_t)t);
        }
    }
    return 0;
}
...
{
    uint32_t start_time = ztimer_now(ZTIMER_USEC);
    fd_set ret_readfds;
    ...
    while (wait) {
        if (_set_timeout(&timeout_timer, timeout,
                         ztimer_now(ZTIMER_USEC) - start_time, &wait) < 0) {
            return -1;
        }
        if (!wait) {
            errno = EINTR;
