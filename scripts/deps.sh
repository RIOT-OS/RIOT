...
# true if the argument is a used module
used() {
    [ "${DEPS#* $1 }" != "$DEPS" ];
}

# true if the argument is the prefix of a used module name
used_prefix() {
    [ "${DEPS#* $1}" != "$DEPS" ];
}

# true if the first argument is a used prefix,
...
# negate a result, i.e. "not used foo" is true if "foo" is not used
not() {
    if "$@"; then
        false
    else
