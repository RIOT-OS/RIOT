...
class rssi_plot(object):
    ...
    def plot_rssi(self, port):
        ...
        fig = plt.figure()
        plt.hold(True)
        pcm = plt.pcolormesh(X, Y, Z, vmin=-128, vmax=0)
        ...
        while True:
            ...
            pkt_data = re.match(r"\[([-+]?\d+),\s*([-+]?\d+)\]\s*(.*)", line.decode())
            if pkt_data:
                now = time.time()
                try:
                ...
                raw = pkt_data.group(3)
                resize = False
                for pair in raw.split(","):
                    try:
                        ch, ed = [int(s) for s in pair.split(":")]
                        if ch < ch_min:
                            ch_min = ch
                            resize = True
                        if ch > ch_max:
...
def main(argv):
    ...
    try:
        ...
        port = Serial(args.tty, args.baudrate, dsrdtr=0, rtscts=0, timeout=1)
    except IOError:
