...
def make_prng(seed):
    ...
    return generator.pseudo_random_data
...
class RSA2048():
    def __init__(self, key, deterministic_signature_seed=None):
        """Construct an RSA2048 key with the given key data"""
        self.key = key
        self._signature_seed = deterministic_signature_seed

    def _reset_rng(self):
        """Set the randfunc of the key according the deterministic signature
        seed."""
        if self._signature_seed is not None:
            self.key._randfunc = make_prng(self._signature_seed)

    @staticmethod
    def generate(randfunc=None):
        return RSA2048(RSA.generate(2048, randfunc=randfunc))
...
class ECDSA256P1():
    def __init__(self, key, deterministic_signature_seed=None):
        ...
        self.key = key
        self._deterministic_sign = deterministic_signature_seed is not None

    @staticmethod
    def generate(randfunc=None):
        if randfunc:
            keystr = randfunc(NIST256p.baselen)
            signing_key = SigningKey.from_string(keystr, curve=NIST256p)
        else:
            signing_key = SigningKey.generate(curve=NIST256p)

        return ECDSA256P1(signing_key)
    ...
    def sign(self, payload):
        ...
        sign = (self.key.sign_deterministic if self._deterministic_sign
                else self.key.sign)

        sig = sign(payload, hashfunc=hashlib.sha256, sigencode=util.sigencode_der)
