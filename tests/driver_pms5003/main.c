...
int _read(int argc, char **argv)
{
    (void)argv;
    (void)argc;
    if (pms5003_read_measurement(&dev, &data) == 0) {
        _print_data(&data);
    }
    else {
...
int main(void)
{
    pms5003_init(&dev, &pms5003_params[0]);
    pms5003_set_callback(&dev, _data_ready_cb, &dev);
    /* run the shell */
