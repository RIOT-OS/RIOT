...
#define TX_RX_MAX_TIME (12 * SYMBOL_TIME)
...
static void *_dev = &cc2538_rf_dev;
#endif

#ifdef MODULE_NRF802154
extern ieee802154_dev_t nrf802154_hal_dev;
static void *_dev = &nrf802154_hal_dev;
#endif

mutex_t lock;
            
#if MODULE_AT86RF2XX
static event_t _irq_event;
...
void _rx_done_handler(event_t *event)
{
     ...
     */
    int size = ieee802154_radio_indication_rx(_dev, buffer, 127, &info);
    if (size > 0) {
...
static void _tx_finish_handler(event_t *event)
{
    ...
    assert(ieee802154_radio_confirm_transmit(_dev, &tx_info) >= 0);

    if (!ieee802154_radio_has_irq_ack_timeout(_dev) && !ieee802154_radio_has_frame_retrans(_dev)) {
    ...
    if (ieee802154_radio_has_frame_retrans_info(_dev)) {
...
static void _send(iolist_t *pkt)
{
    ...
    ieee802154_radio_request_set_trx_state(_dev, IEEE802154_TRX_STATE_TX_ON);

    /* Write the packet to the radio */
    ieee802154_radio_write(_dev, pkt);

    /* Block until th radio confirms the state change */
    while(ieee802154_radio_confirm_set_trx_state(_dev) == -EAGAIN);

    /* Trigger the transmit and wait for the mutex unlock (TX_DONE event) */
    ieee802154_radio_request_transmit(_dev);
    ieee802154_radio_set_rx_mode(_dev, IEEE802154_RX_WAIT_FOR_ACK);
    mutex_lock(&lock);
...
static int _init(void)
{
    ...
    ieee802154_radio_request_on(_dev);
    while(ieee802154_radio_confirm_on(_dev) == -EAGAIN) {}

    current_rx_mode = IEEE802154_RX_AACK_ENABLED;
    ieee802154_radio_set_rx_mode(_dev, current_rx_mode);

    /* Set all IEEE addresses */
    uint16_t panid = 0x23;
    ieee802154_radio_set_hw_addr_filter(_dev, &short_addr, &ext_addr, &panid);

    /* Set PHY configuration */
    ieee802154_phy_conf_t conf = {.channel=21, .page=0, .pow=0};
...
int _cca(int argc, char **argv)
{
    (void) argc;
    (void) argv;
    ieee802154_radio_request_cca(_dev);
    mutex_lock(&lock);
    int res = ieee802154_radio_confirm_cca(_dev);
    assert(res >= 0);
...
{
    xtimer_ticks32_t a;
    int res = ieee802154_radio_request_set_trx_state(_dev, state);
    if (verbose) {
        a = xtimer_now();
        if(res != 0) {
            printf("%i != 0 \n", res);
    ...
    while ((res = ieee802154_radio_confirm_set_trx_state(_dev)) == -EAGAIN) {}
    if (verbose) {
        if (res != 0) {
            printf("%i != 0 \n", res);
            assert(false);
...
static int rx_mode_cmd(int argc, char **argv)
{
    ...
    ieee802154_radio_set_rx_mode(_dev, conf);
    return 0;
}
...
static int _caps_cmd(int argc, char **argv)
{
    ...
    if (ieee802154_radio_has_frame_retrans(_dev)) {
       has_frame_retrans = true; 
    }

    if (ieee802154_radio_has_auto_csma(_dev)) {
        has_auto_csma = true;
    }

    if (ieee802154_radio_has_24_ghz(_dev)) {
        has_24_ghz = true;
    }

    if (ieee802154_radio_has_sub_ghz(_dev)) {
        has_sub_ghz = true;
    }

    if (ieee802154_radio_has_irq_tx_done(_dev)) {
        has_irq_tx_done = true;
    }

    if (ieee802154_radio_has_irq_rx_start(_dev)) {
        has_irq_rx_start = true;
    }

    if (ieee802154_radio_has_irq_ack_timeout(_dev)) {
        has_irq_ack_timeout = true;
    }

    if (ieee802154_radio_has_irq_cca_done(_dev)) {
