 ...
 *
 * This application demonstrates how to use the tasklet module.
 * For that purpose the main thread simulates interrupts
 * from 2 interrupt sources with different priorities with a period
 * of half a second. One interrupt source triggers a second interrupt before
 ...
 * are generated by the main thread. Therefore the tasklets thread
...
/* preallocated and initialized tasklet objects */
static tasklet_t _int1_event = TASKLET_INIT;
static tasklet_t _int2_event = TASKLET_INIT;
...
static void _int1_service (void *arg)
{
    ...
    if (tasklet_schedule(&_int1_event) == -EALREADY) {
...
static void _int2_service (void *arg)
{
    ...
    puts("int2 triggered");
    /* registers just the interrupt event and returns */
    if (tasklet_schedule(&_int2_event) == -EALREADY) {
...
int main(void)
{
    _int1_event.task = _int1_handler;
    _int1_event.ctx = &timer1a;

    _int2_event.task = _int2_handler;
