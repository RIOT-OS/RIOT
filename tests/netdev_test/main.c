...
static int test_receive(void)
{
    ...
    if (memcmp(gnrc_netif_hdr_get_src_addr(hdr->data), _test_src,
        ...
        puts("expected");
        puts("=================");
        puts(netif_addr_to_str(_test_src, ETHERNET_ADDR_LEN, addr_str));
        puts("=================");
        puts("received source");
        puts("=================");
        puts(netif_addr_to_str(gnrc_netif_hdr_get_src_addr(hdr->data),
    ...
    if (memcmp(gnrc_netif_hdr_get_dst_addr(hdr->data), _dev_addr,
        ...
        puts("expected");
        puts("=================");
        puts(netif_addr_to_str(_dev_addr, ETHERNET_ADDR_LEN, addr_str));
        puts("====================");
        puts("received destination");
        puts("====================");
        puts(netif_addr_to_str(gnrc_netif_hdr_get_dst_addr(hdr->data),
                                    ETHERNET_ADDR_LEN, addr_str));
        return 0;
    }
