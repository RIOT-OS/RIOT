...
static void _usage(void)
{
    ...
    printf("       dns query <0, 4 or 6 (Unspecified/IPv4/IPv6)> <name>\n");
}
...
static int _dns_server(int argc, char **argv)
{
    ...
    if (!IS_USED(MODULE_IPV4) && ip_version == 4) {
        puts("sock does not support IPv4");
        return 1;
    }

    if (!IS_USED(MODULE_IPV6) && ip_version == 6) {
        puts("sock does not support IPv6");
        return 1;
    }
...
static int _dns_query(int argc, char **argv)
{
    if (argc < 4) {
        _usage();
        return 1;
