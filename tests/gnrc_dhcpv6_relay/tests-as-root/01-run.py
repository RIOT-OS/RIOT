...
class TestDHCPv6RelayAgent(PexpectTestCase):
    ...
    LOGFILE = None
    tap = os.environ["TAP"]
    lower_headers = Ether(dst="33:33:00:01:00:02") / IPv6(dst="ff02::1:2")
    ...
    @classmethod
    def _sendp(cls, pkt, client=True):
        sendp(cls.lower_headers / cls._udp(client=client) / pkt,
              iface=cls.tap, verbose=0 if cls.LOGFILE is None else 2)
    ...
    @classmethod
    def send_and_exp_any_dhcp6(cls, send_pkt, client=True):
        sniffer = AsyncSniffer(iface=cls.tap)
        sniffer.start()
        time.sleep(cls.pre_sniffer_wait)
        cls._sendp(send_pkt, client=client)
    ...
    def assert_pkt_ignored(self, pkt, client_pkt=True):
