...
static void test_psa_aes_128_cbc_encrypt_decrypt(void)
{
    ...
    TEST_ASSERT_PSA_SUCCESS(psa_cipher_encrypt(key_id, PSA_ALG_CBC_NO_PADDING, PLAINTEXT,
                                sizeof(PLAINTEXT), cipher_out, encr_output_size, &output_len));

    TEST_ASSERT_PSA_SUCCESS(psa_cipher_decrypt(key_id, PSA_ALG_CBC_NO_PADDING, cipher_out,
                                sizeof(cipher_out), plain_out, sizeof(plain_out), &output_len));

    TEST_ASSERT_MESSAGE(memcmp(PLAINTEXT, plain_out, sizeof(plain_out)) == 0, "Cipher Failed");

    psa_destroy_key(key_id);
}

/**
 * @brief   Example function to perform an AES-128 ECB encryption and decryption
 *          with the PSA Crypto API.
 */
static void test_psa_aes_128_ecb_encrypt_decrypt(void)
{
    ...
    TEST_ASSERT_PSA_SUCCESS(psa_cipher_encrypt(key_id, PSA_ALG_ECB_NO_PADDING, PLAINTEXT,
                                sizeof(PLAINTEXT), cipher_out, encr_output_size, &output_len));

    TEST_ASSERT_PSA_SUCCESS(psa_cipher_decrypt(key_id, PSA_ALG_ECB_NO_PADDING, cipher_out,
                                sizeof(cipher_out), plain_out, sizeof(plain_out), &output_len));
