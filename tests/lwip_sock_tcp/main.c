...
{
    const sock_tcp_ep_t remote = { .addr = { .ipv4 = HTONL(_TEST_ADDR4_REMOTE) },
                                          .family = AF_INET,
...
/* ETIMEDOUT not testable for IPv4 */
...
static void test_tcp_connect4__success_without_port(void)
{
    ...
    assert(HTONL(_TEST_ADDR4_REMOTE) == ep.addr.ipv4);
...
{
    const sock_tcp_ep_t local = { .addr = { .ipv4 = HTONL(_TEST_ADDR4_LOCAL) },
                                         .family = AF_INET,
...
static void test_tcp_listen4__success_any_netif(void)
{
    ...
    assert(AF_INET == ep.family);
    assert(HTONL(_TEST_ADDR4_LOCAL) == ep.addr.ipv4);
    assert(SOCK_ADDR_ANY_NETIF == ep.netif);
    assert(_TEST_PORT_LOCAL == ep.port);
}
...
static void test_tcp_accept4__success(void)
{
    ...
    _server_addr.addr.ipv4 = HTONL(_TEST_ADDR4_REMOTE);  /* loopback */
    _server_addr.family = AF_INET;
    _server_addr.port = _TEST_PORT_LOCAL;
    _server_addr.netif = SOCK_ADDR_ANY_NETIF;
...
static void test_tcp_connect6__ETIMEDOUT(void)
{
    static const sock_tcp_ep_t remote = { .addr = { .ipv6 = _TEST_ADDR6_WRONG },
                                          .family = AF_INET6,
                                          .port = _TEST_PORT_REMOTE,
                                          .netif = _TEST_NETIF };
    msg_t msg = { .type = _SERVER_MSG_START };
    ...
    assert(-ETIMEDOUT == sock_tcp_connect(&_sock, &remote, 0, SOCK_FLAGS_REUSE_EP));
}
...
int main(void)
{
    ...
    /* ENETUNREACH not testable in given loopback setup */
    CALL(test_tcp_connect6__ETIMEDOUT());
