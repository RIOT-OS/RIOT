...
static ztimer_tracker_t usec_tracker = { .base.callback = track, .base.arg = &usec_tracker, .exp = 0 };
...
int main(void)
{
    ...
    while(ztimer_spin(ZTIMER_USEC, 300* US_PER_MS),1){
        ...
        printf( "time now:\t%" PRIu32 "\t time track:\t%" PRIu64 "\t upper32:\t%" PRIu32 "\t lower32:\t%" PRIu32 "\n",
