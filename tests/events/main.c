...
static uint32_t timeout1_start;
static uint32_t timeout2_start;

static void callback(event_t *arg);
static void custom_callback(event_t *event);
static void timed_callback1(void *arg);
static void timed_callback2(void *arg);
...
static event_callback_t event_callback1 = EVENT_CALLBACK_INIT(timed_callback1, 0x12345678);
static event_callback_t event_callback2 = EVENT_CALLBACK_INIT(timed_callback2, 0x12348765);
...
static void timed_callback1(void *arg)
{
    order++;
    assert(order == 4);
    assert(arg == event_callback1.arg);
    uint32_t now = xtimer_now_usec();
    assert((now - timeout1_start >= 100000LU));
    printf("triggered timed callback with arg 0x%08x after %" PRIu32 "us\n",
           (unsigned)arg, now - timeout1_start);
}
...
static void timed_callback2(void *arg)
{
    order++;
    assert(order == 5);
    assert(arg == event_callback2.arg);
    uint32_t now = xtimer_now_usec();
    assert((now - timeout2_start >= 100000LU));
    printf("triggered timed callback with arg 0x%08x after %" PRIu32 "us\n",
           (unsigned)arg, now - timeout2_start);
    puts("[SUCCESS]");
}
...
static void *claiming_thread(void *arg)
{
    ...
    /* this tests that the non-blocking event wait can be used with the blocking
     * wait (called from event_loop) */
    while(!(ev = event_get(dq))) {}
    ev->handler(ev);
...
int main(void)
{
    ...
    printf("posting %p to detached event queue\n", (void *)&delayed_event1);
    event_post(&dq, &delayed_event1);

    event_timeout_t event_timeout1;

    puts("posting timed callback with timeout 1sec to detached event queue");
    event_timeout_init(&event_timeout1, &dq, (event_t *)&event_callback1);
    timeout1_start = xtimer_now_usec();
    event_timeout_set(&event_timeout1, (1 * US_PER_SEC));
    ...
    event_timeout_t event_timeout2;

    puts("posting timed callback with timeout 1sec");
    event_timeout_init(&event_timeout2, &queue, (event_t *)&event_callback2);
    timeout2_start = xtimer_now_usec();
    event_timeout_set(&event_timeout2, (1 * US_PER_SEC));
