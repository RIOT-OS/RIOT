...
static void test_gcoap__client_get_req(void)
{
    ...
    len = gcoap_request(&pdu, buf, CONFIG_GCOAP_PDU_BUF_SIZE,
...
static void test_gcoap__client_get_resp(void)
{
    ...
    res = coap_parse_udp(&pdu, buf, sizeof(pdu_data));
...
static void test_gcoap__server_get_req(void)
{
    ...
    ssize_t res = _read_cli_stats_req(&pdu, buf);
...
static void test_gcoap__server_get_resp(void)
{
    ...
    char resp_payload[]  = "2";
    memcpy(pdu.payload, resp_payload, strlen(resp_payload));
...
static void test_gcoap__server_con_req(void)
{
    ...
    ssize_t res = _read_cli_stats_req_con(&pdu, buf);
