...
#include "tests-crypto-blake2b.h"

static int gen_testhash(char *input, char *key, uint16_t input_length, char *test)
{
    uint8_t hash[BLAKE2B_OUTBYTES];

    blake2b(hash, input, key, BLAKE2B_OUTBYTES, input_length, BLAKE2B_KEYBYTES);
...
static void test_crypto_blake2b_hash(void)
{
...
Test *tests_crypto_blake2b_tests(void)
{
    EMB_UNIT_TESTFIXTURES(fixtures) {
        new_TestFixture(test_crypto_blake2b_hash),
    };

    EMB_UNIT_TESTCALLER(crypto_blake2b_tests, NULL, NULL, fixtures);

    return (Test *)&crypto_blake2b_tests;
}
