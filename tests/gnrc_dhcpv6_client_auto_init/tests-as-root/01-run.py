...
from scapy.all import DHCP6OptClientId, DHCP6OptServerId, DHCP6OptUnknown
from scapy.all import DUID_LL, DHCP6OptIA_NA, DHCP6OptIAAddress
...
def testfunc(child):
    ...
    # and it is asking for a non-temporary address
    assert DHCP6OptIA_NA in pkt

    # reply to solicit with advertise and an address provided
    ...
    sendp(build_reply_headers(pkt) / DHCP6_Advertise(trid=trid) /
          ia_na / cli_id / srv_id, iface=iface, verbose=False)
    # wait for request
    ...
    # reply to request with reply and an address provided
    ...
    child.expect("inet6 addr: b8cb:32d0:2013:ef2a:c6e4:508d:452c:e21f")
    print("SUCCESS")
