 ...
 * Picture the following scenario using condition variables:
 *
 * ```
 * static uint8_t critical_event_happened = false;
 * mutex_t cond_lock = MUTEX_INIT;
 * cond_t cond = COND_INIT;
 *
 * void critical_irq(void)
 * {
 *     critical_event_happened = true;
 *     cond_broadcast(&cond);
 * }
 *
 * void wait_for_critical_event(void)
 * {
 *     mutex_lock(&cond_lock);
 *     while (!atomic_load_u8(&critical_event_happened)) {
 ...
 * possible:
 *  1. thread sees critical_event_happened == false, and is about to call
 *     cond_wait()
 *  2. ISR fires, sets critical_event_happened = true and calls cond_signal()
 ...
 * ```
 *
 * The mutex_unlock() at the end of wait_for_critical_event() is there to wake
 ...
 * void wait_for_critical_event(void)
 * {
 *     queue_wait(&wq, atomic_load_u8(&critical_event_happened));
 * }
 * ```
 *
 * This is free of the the race condition above because queue_wait() is a macro
 * that checks the conditition (critical_event_happened) AFTER queueing the
 * current thread to be waken up:
 *
 * ```
 * queue_wait(wq, cond):
 *     loop:
 ...
 * setting the condition true, which is not.
 *
 * Limitations
 *
 ...
 *     the locking is left to the user.
 ...
 * Wait queues only make sense if the signaler (or one of multiple) is in ISR
 * context.
 *
 * @{
 *
...
static inline void _wait_enqueue(list_node_t *queue, thread_t *me)
{
    irq_disable();
    sched_set_status(me, STATUS_COND_BLOCKED);
    thread_add_to_list(queue, me);
    irq_enable();
}

static inline void _wait_dequeue(list_node_t *queue, thread_t *me)
{
    irq_disable();
    list_remove(queue, &me->rq_entry);
...
#define queue_wait(wq, cond) do {                   \
    ...
    while (1) {                                     \
        _wait_enqueue(&wq->cond.queue, me);         \
        if (cond) {                                 \
            break;                                  \
        }                                           \
        thread_yield_higher();                      \
    }                                               \
    _wait_dequeue(&wq->cond.queue, me);             \
} while (0)
