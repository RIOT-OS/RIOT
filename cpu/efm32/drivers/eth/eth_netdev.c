...
static int _init(netdev_t *netdev)
{
    ...
    int res = efm32_eth_init(hwaddr.uint8);
...
static int _get(netdev_t *netdev, netopt_t opt, void *val, size_t max_len)
{
    ...
    case NETOPT_ADDRESS:
        ...
        efm32_eth_get_mac((uint8_t *)val);
        return ETHERNET_ADDR_LEN;
    case NETOPT_LINK:
        assert(max_len == sizeof(netopt_enable_t));
        *(netopt_enable_t *)val = efm32_eth_link_up() ? NETOPT_ENABLE : NETOPT_DISABLE;
        return sizeof(netopt_enable_t);
    case NETOPT_PROMISCUOUSMODE:
        assert(max_len == sizeof(netopt_enable_t));
        *(netopt_enable_t *)val = efm32_eth_get_promiscuous() ? NETOPT_ENABLE : NETOPT_DISABLE;
        return sizeof(netopt_enable_t);
    default:
...
static int _set(netdev_t *netdev, netopt_t opt, const void *val, size_t len)
{
    ...
    case NETOPT_ADDRESS:
        ...
        efm32_eth_set_mac((const uint8_t *)val);
        return ETHERNET_ADDR_LEN;
    case NETOPT_PROMISCUOUSMODE:
        assert(len == sizeof(netopt_enable_t));
        efm32_eth_set_promiscuous(*(const netopt_enable_t *)val == NETOPT_ENABLE);
        return sizeof(netopt_enable_t);
    default:
...
void efm32_eth_netdev_setup(netdev_t *netdev)
{
    ...
    netdev_register(netdev, NETDEV_EFM32_ETH, 0);
}
