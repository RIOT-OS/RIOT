...
static inline volatile uint32_t* ccr(pwm_t dev, int ch)
{
    return &(tim(dev)->CCR[chan(dev, ch)->chan]);
}
...
void pwm_poweron(pwm_t dev)
{
    ...
    periph_clk_en(pwm_config[dev].apb, pwm_config[dev].rcc_mask);
}
...
void pwm_poweroff(pwm_t dev)
{
    ...
    periph_clk_dis(pwm_config[dev].apb, pwm_config[dev].rcc_mask);
}
