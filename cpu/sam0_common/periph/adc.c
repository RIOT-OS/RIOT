...
uint32_t adc_get_freq(void)
{
     ...
     * Here, nSAMPLING is the sampling duration in CLK_ADC cycles,
     * nOFFCOMP is the offset compensation duration in clock
     * cycles, and nDATA is the bit resolution. fCLK_ADC is the
     * ADC clock frequency from the internal prescaler:
     * fCLK_ADC = fGCLK_ADC / 2^(1 + CTRLA.PRESCALER)
     */
    return sam0_gclk_freq(ADC_GCLK_SRC) /
           ((uint32_t)1 << ((ADC_PRESCALER >> ADC_CTRLA_PRESCALER_Pos) + 1));
}
...
uint32_t adc_get_sample_rate(adc_t line, unsigned bits)
{
     ...
     * Setting this bit enables the offset compensation for each sampling period
     * to ensure low offset and immunity to temperature or voltage drift.
     * This compensation increases the sampling time by three clock cycles that results in a
     * fixed sampling duration of 4 CLK_ADC cycles.
     * This bit must be set to zero to validate the SAMPLEN value.
     * It’s not possible to use OFFCOMP=1 and SAMPLEN>0.
     */
    bool offcomp = dev->SAMPCTRL.reg & (1u << ADC_SAMPCTRL_OFFCOMP_Pos);
    unsigned n_sampling = ((dev->SAMPCTRL.reg & ADC_SAMPCTRL_SAMPLEN_Msk)
                          >> ADC_SAMPCTRL_SAMPLEN_Pos) + 1;
    if (offcomp) {
        n_sampling = 4;
    }
