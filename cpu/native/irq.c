...
#include "debug.h"
#define IRQ_DEBUG(...) DEBUG("[native] IRQ: " __VA_ARGS__)
...
unsigned irq_disable(void)
{
    ...
    IRQ_DEBUG("irq_disable(): _native_in_isr == %i\n", _native_in_isr);

    if (_native_in_isr == 1) {
        IRQ_DEBUG("irq_disable + _native_in_isr\n");
    }
...
#     else
        IRQ_DEBUG("irq_enable + _native_in_isr\n");
#     endif
    }

    _native_syscall_enter();
    IRQ_DEBUG("irq_enable()\n");
    ...
    if (_native_in_isr == 0 && sched_context_switch_request) {
        IRQ_DEBUG("irq_enable() deferred thread_yield_higher()\n");
        thread_yield_higher();
    }

    IRQ_DEBUG("irq_enable(): return\n");
...
void _native_call_sig_handlers_and_switch(void)
{
    ...
    while (_native_pending_signals > 0) {
        ...
        if (_native_irq_handlers[sig]) {
            IRQ_DEBUG("call sig handlers + switch: calling interrupt handler for %i\n", sig);
    ...
    IRQ_DEBUG("call sig handlers + switch: return\n");
...
{
    IRQ_DEBUG("native_register_interrupt\n");
...
{
    /* empty signal mask */
    IRQ_DEBUG("native_unregister_interrupt\n");
...
void native_interrupt_init(void)
{
    ...
    struct sigaction sa;
    IRQ_DEBUG("native_interrupt_init\n");
