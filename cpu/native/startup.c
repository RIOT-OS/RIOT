...
#include "debug.h"
#define STARTUP_DEBUG(...) DEBUG("[native] startup: " __VA_ARGS__)
...
__attribute__((constructor)) static void startup(int argc, char **argv, char **envp)
{
    ...
    while (init_array_ptr != &__init_array_end) {
        ...
        if ((*init_array_ptr) == startup) {
            ...
            STARTUP_DEBUG("%18p - myself\n", (void *)init_array_ptr);
            ++init_array_ptr;
            break;
        }
        STARTUP_DEBUG("%18p - skip\n", (void *)init_array_ptr);
        ++init_array_ptr;
    }
    while (init_array_ptr != &__init_array_end) {
        /* call all remaining constructors */
        STARTUP_DEBUG("%18p - call\n", (void *)init_array_ptr);
        (*init_array_ptr)(argc, argv, envp);
        ++init_array_ptr;
    }
    STARTUP_DEBUG("done, __init_array_end: %p\n", (void *)init_array_ptr);
