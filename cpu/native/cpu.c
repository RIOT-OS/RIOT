...
#include "debug.h"
#define CPU_DEBUG(...) DEBUG("[native] CPU: " __VA_ARGS__)
...
/* static */ void _isr_switch_to_user(void) {
    CPU_DEBUG("... ISR: switching to user thread, calling setcontext(PID %" PRIkernel_pid ")\n\n", thread_getpid());
...
/* static */ void _isr_context_switch_exit(void)
{
    CPU_DEBUG("_isr_schedule_and_switch\n");
...
# ifdef NATIVE_AUTO_EXIT
    if (sched_num_threads <= 1) {
        ...
        CPU_DEBUG("cpu_switch_context_exit: last task has ended. exiting.\n");
...
/* static */ void _isr_thread_yield(void)
{
    CPU_DEBUG("... ISR: switched to ISR context, scheduling\n");

    if (_native_pending_signals > 0) {
        CPU_DEBUG("... ISR: pending signals, handling signals\n\n");
...
void thread_yield_higher(void)
{
    ...
    if (_native_in_isr == 0 && _native_interrupts_enabled) {
        CPU_DEBUG("yielding higher priority thread, switching to ISR context ...\n");
...
void native_cpu_init(void)
{
    ...
    CPU_DEBUG("RIOT native cpu initialized.\n");
}
