...
static bool set_next_alarm(bool may_call)
{
    ...
    while (timers_start) {
        ...
        if (us_future <= START_MAX_US_PREMATURELY) {
            ...
            if (!may_call) {
                msg_t m;
                msg_send_int(&m, x86_hwtimer_pid);
                return true;
            }
            else {
        ...
        if (us_future > 5 * US_PER_SECOND) {
            ...
            rtc_alarm_ie = false;
            stop_alarms();
            x86_rtc_set_alarm(&rtc_now, 0, x86_hwtimer_pid, true);
            rtc_alarm_ie = true;
        }
            ...
            rtc_update_ie = false;
            stop_alarms();
            x86_rtc_set_update(0, x86_hwtimer_pid, true);
            rtc_update_ie = true;
        }
        else {
            ...
            if (hz != rtc_periodic_ie) {
                ...
                stop_alarms();
                x86_rtc_set_periodic(hz, 0, x86_hwtimer_pid, true);
                rtc_periodic_ie = hz;
            }
        }
...
void hwtimer_arch_init(void (*handler)(int), uint32_t fcpu)
{
    ...
    x86_hwtimer_pid = thread_create(hwtimer_stack,
