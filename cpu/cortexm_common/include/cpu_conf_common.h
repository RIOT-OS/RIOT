 ...
 *
 * Cortex-M CPUs allow priorities to be arranged in subgroups,
 ...
 * running the scheduler here.  However, in RIOT we don't want the
 * scheduler to be interrupted by any "normal" interrupts, which
 ...
 * By default, RIOT runs PendSV on the same interrupt priority as all
 * other interrupts.
 *
 ...
 * exactly trivial.
 *
 * An experimental way to to run PendSV as the last thing is to enable
 * Cortex-M sub-priorities @ref CPU_CORTEXM_PRIORITY_GROUPING and then
