...
static inline void isr_func(int num, Usart *dev)
{
    uint32_t status = dev->US_CSR;

    if (status & US_CSR_RXRDY) {
        char data = (char)dev->US_RHR;
        uart_config[num].rx_cb(uart_config[num].arg, data);
    }
    if ((status & US_CSR_TXRDY) && (dev->US_IMR & US_IMR_TXRDY)) {
        uint16_t data = uart_config[num].tx_cb(uart_config[num].arg);
        if (data) {
            dev->US_THR = (uint8_t)data;
        }
        else {
...
{
    isr_func(UART_0, (Usart *)UART_0_DEV);
}
#endif
...
{
    isr_func(UART_2, (Usart *)UART_2_DEV);
}
#endif
...
{
    isr_func(UART_3, (Usart *)UART_3_DEV);
}
#endif
