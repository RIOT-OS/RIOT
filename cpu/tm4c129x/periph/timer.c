...
{
    if (dev == TIMER_0) {
        TIMER0_TAILR_R = 0x00000000 | value;               /* period; Reload value */
...
unsigned int timer_read(tim_t dev)
{
    if (dev == TIMER_0) {
        ...
        currTimer0Val = (unsigned int)ROM_TimerValueGet(TIMER0_BASE, TIMER_A);
        loadTimer0Val = (unsigned int)ROM_TimerLoadGet(TIMER0_BASE, TIMER_A);
        DEBUG("TIMER0_TAILR_R=0x%lx\t currTimer0Val=0x%x\t loadTimer0Val=0x%x\n", TIMER0_TAILR_R, currTimer0Val, loadTimer0Val);
