...
/* --- local typedefs ---------------------------------------------------------------------------*/
/**
 * @brief Define a struct holding two function pointers for each GPIO pin
 * 
 * The function pointers are needed for 
 * a) reading the current state (different pointer depending if input or output)
 * b) callback when configured as external interrupt source
 */
typedef struct {
...
/**
 * @brief           Unified initializing function for the given GPIO pin
 * 
 * @param dev       the pin to initialize
 * @param pushpull  pull resistor configuration
 * @param config    a pre-configured init struct defined by the STM lib
 */
static void init(gpio_t dev, gpio_pp_t pushpull, GPIO_InitTypeDef *config);

/**
 * @brief           Unified interrupt handler for reducing the amount of code
 * 
 * @param dev       the GPIO pin that triggered the interrupt
 */
