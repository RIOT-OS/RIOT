 ...
 *
 * @file
 * @brief   Low level USB interface functions for the ESP32 FS devices
 *
...
/* define macros that are used for volatile keyword in STM32 header files */
#define __I  volatile
#define __O  volatile
...
/* at the moment FS is support only */
#define ESP32_USB_OTG_FS_ENABLED        1
 ...
 */
#define ESP32_USB_OTG_FS_NUM_EP         5
#define ESP32_USB_OTG_HS_NUM_EP         5

#define ESP32_USB_OTG_FS_RX_FIFO_SIZE   (128U)
#define ESP32_USB_OTG_HS_RX_FIFO_SIZE   (512U)

/* ESP32-S2 and ESP32-S3 use DWC2_CORE_REV_4_00a which corresponds to STM32 USB_OTG_CID_2x */
#define ESP32_USB_OTG_CID_2x            1

#define USBDEV_CPU_DMA_ALIGNMENT        (4)

#if defined(ESP32_USB_OTG_FS_ENABLED) && defined(ESP32_USB_OTG_HS_ENABLED)
#define _TOTAL_NUM_ENDPOINTS  (ESP32_USB_OTG_FS_NUM_EP + \
                               ESP32_USB_OTG_HS_NUM_EP)
#elif defined(ESP32_USB_OTG_FS_ENABLED)
#define _TOTAL_NUM_ENDPOINTS  (ESP32_USB_OTG_FS_NUM_EP)
#elif defined(ESP32_USB_OTG_HS_ENABLED)
#define _TOTAL_NUM_ENDPOINTS  (ESP32_USB_OTG_HS_NUM_EP)
#endif

/* Mask for the set of interrupts used */
#define ESP32_FSHS_USB_GINT_MASK    \
...
#define ESP32_PKTSTS_GONAK          0x01    /**< Rx fifo global out nak */
#define ESP32_PKTSTS_DATA_UPDT      0x02    /**< Rx fifo data update    */
#define ESP32_PKTSTS_XFER_COMP      0x03    /**< Rx fifo data complete  */
#define ESP32_PKTSTS_SETUP_COMP     0x04    /**< Rx fifo setup complete */
#define ESP32_PKTSTS_SETUP_UPDT     0x06    /**< Rx fifo setup update   */
...
#define ESP32_USB_OTG_FIFO_MIN_WORD_SIZE    (16U)
/* Offset for OUT endpoints in a shared IN/OUT endpoint bit flag register */
#define ESP32_USB_OTG_REG_EP_OUT_OFFSET      (16U)

/* Endpoint zero size values */
#define ESP32_USB_OTG_EP0_SIZE_64    (0x0)
#define ESP32_USB_OTG_EP0_SIZE_32    (0x1)
#define ESP32_USB_OTG_EP0_SIZE_16    (0x2)
#define ESP32_USB_OTG_EP0_SIZE_8     (0x3)

/* Endpoint type values */
#define ESP32_USB_OTG_EP_TYPE_CONTROL   (0x00 << USB_OTG_DOEPCTL_EPTYP_Pos)
#define ESP32_USB_OTG_EP_TYPE_ISO       (0x01 << USB_OTG_DOEPCTL_EPTYP_Pos)
#define ESP32_USB_OTG_EP_TYPE_BULK      (0x02 << USB_OTG_DOEPCTL_EPTYP_Pos)
#define ESP32_USB_OTG_EP_TYPE_INTERRUPT (0x03 << USB_OTG_DOEPCTL_EPTYP_Pos)

/**
 * @brief esp32 USB OTG peripheral device out endpoint struct
 */
typedef struct {
    usbdev_ep_t ep;     /**< Inherited usbdev endpoint struct */
    uint8_t *out_buf;   /**< Requested data output buffer */
} esp32_usb_otg_fshs_out_ep_t;
 ...
 */
typedef enum {
    ESP32_USB_OTG_FS = 0,   /**< Full speed peripheral */
    ESP32_USB_OTG_HS = 1,   /**< High speed peripheral */
} esp32_usb_otg_fshs_type_t;

/**
 * @brief esp32 USB OTG configuration
 */
typedef struct {
    uintptr_t periph;               /**< USB peripheral base address */
    esp32_usb_otg_fshs_type_t type; /**< FS or HS type */
} esp32_usb_otg_fshs_config_t;

/**
 * @brief esp32 USB OTG peripheral device context
 */
typedef struct {
    usbdev_t usbdev;                            /**< Inherited usbdev struct */
    const esp32_usb_otg_fshs_config_t *config;  /**< USB peripheral config   */
    size_t fifo_pos;                            /**< FIFO space occupied */
    usbdev_ep_t *in;                            /**< In endpoints */
    esp32_usb_otg_fshs_out_ep_t *out;           /**< Out endpoints */
    bool suspend;                               /**< Suspend status */
} esp32_usb_otg_fshs_t;

static const esp32_usb_otg_fshs_config_t esp32_usb_otg_fshs_config[] = {
    {
        .periph = USB_OTG_FS_PERIPH_BASE,
        .type = ESP32_USB_OTG_FS,
    }
};

#define USBDEV_NUMOF    ARRAY_SIZE(esp32_usb_otg_fshs_config)

/* List of instantiated USB peripherals */
static esp32_usb_otg_fshs_t _usbdevs[USBDEV_NUMOF] = { 0 };

static esp32_usb_otg_fshs_out_ep_t _out[_TOTAL_NUM_ENDPOINTS];
static usbdev_ep_t _in[_TOTAL_NUM_ENDPOINTS];

/* Forward declaration for the usb device driver */
const usbdev_driver_t driver;

static void _flush_tx_fifo(const esp32_usb_otg_fshs_config_t *conf,
                           uint8_t fifo_num);

static void isr_otg_fs(void *arg);

/*************************************************************************
* Conversion function from the base address to specific register blocks *
*************************************************************************/
static USB_OTG_GlobalTypeDef *_global_regs(
    const esp32_usb_otg_fshs_config_t *conf)
{
...
static __I uint32_t *_rx_fifo(const esp32_usb_otg_fshs_config_t *conf)
{
    return (__I uint32_t *)(conf->periph + USB_OTG_FIFO_BASE);
}

static __O uint32_t *_tx_fifo(const esp32_usb_otg_fshs_config_t *conf,
                              size_t num)
{
...
static __IO uint32_t *_pcgcctl_reg(const esp32_usb_otg_fshs_config_t *conf)
{
 ...
 */
static size_t _max_endpoints(const esp32_usb_otg_fshs_config_t *config)
{
    return (config->type == ESP32_USB_OTG_FS) ?
           ESP32_USB_OTG_FS_NUM_EP :
           ESP32_USB_OTG_HS_NUM_EP;
}

static bool _uses_dma(const esp32_usb_otg_fshs_config_t *config)
{
#if defined(ESP32_USB_OTG_HS_ENABLED) && ESP32_USB_OTG_HS_USE_DMA
    return config->type == ESP32_USB_OTG_HS;
#else
    (void)config;
    return false;
#endif
...
static size_t _setup(esp32_usb_otg_fshs_t *usbdev,
                     const esp32_usb_otg_fshs_config_t *config, size_t idx)
{
...
void usbdev_init_lowlevel(void)
{
    ...
    for (size_t i = 0; i < USBDEV_NUMOF; i++) {
        ep_idx += _setup(&_usbdevs[i], &esp32_usb_otg_fshs_config[i], ep_idx);
    }
#ifdef NDEBUG
    (void)ep_idx;
#endif
...
static void _enable_global_out_nak(const esp32_usb_otg_fshs_config_t *conf)
{
...
static void _disable_global_out_nak(const esp32_usb_otg_fshs_config_t *conf)
{
    if (!(_device_regs(conf)->DCTL & USB_OTG_DCTL_GONSTS)) {
        return;
    }
...
static void _enable_global_in_nak(const esp32_usb_otg_fshs_config_t *conf)
{
...
static void _disable_global_in_nak(const esp32_usb_otg_fshs_config_t *conf)
{
    if (!(_device_regs(conf)->DCTL & USB_OTG_DCTL_GINSTS)) {
        return;
    }
...
static void _disable_global_nak(const esp32_usb_otg_fshs_config_t *conf)
{
...
static uint32_t _type_to_reg(usb_ep_type_t type)
{
    switch (type) {
        case USB_EP_TYPE_CONTROL:
            return ESP32_USB_OTG_EP_TYPE_CONTROL;
        case USB_EP_TYPE_ISOCHRONOUS:
            return ESP32_USB_OTG_EP_TYPE_ISO;
        case USB_EP_TYPE_BULK:
            return ESP32_USB_OTG_EP_TYPE_BULK;
        case USB_EP_TYPE_INTERRUPT:
            return ESP32_USB_OTG_EP_TYPE_INTERRUPT;
        default:
            assert(false);
            return 0;
    }
...
static uint32_t _ep0_size(size_t size)
{
    switch (size) {
        case 64:
            return ESP32_USB_OTG_EP0_SIZE_64;
        case 32:
            return ESP32_USB_OTG_EP0_SIZE_32;
        case 16:
            return ESP32_USB_OTG_EP0_SIZE_16;
        case 8:
            return ESP32_USB_OTG_EP0_SIZE_8;
        default:
            assert(false);
            return 0x00;
    }
 ...
 */
static void _ep_in_disable(const esp32_usb_otg_fshs_config_t *conf, size_t num)
{
 ...
 * Endpoint is only deactivated if it was activated
 */
static void _ep_out_disable(const esp32_usb_otg_fshs_config_t *conf, size_t num)
{
...
{
    esp32_usb_otg_fshs_t *usbdev = (esp32_usb_otg_fshs_t *)ep->dev;
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
...
static void _ep_activate(usbdev_ep_t *ep)
{
    ...
    if (ep->dir == USB_EP_DIR_IN) {
        ...
        _device_regs(conf)->DAINTMSK |=
            1 << (ep->num + ESP32_USB_OTG_REG_EP_OUT_OFFSET);
...
static inline void _usb_attach(esp32_usb_otg_fshs_t *usbdev)
{
...
static inline void _usb_detach(esp32_usb_otg_fshs_t *usbdev)
{
...
static void _set_address(esp32_usb_otg_fshs_t *usbdev, uint8_t address)
{
...
static usbdev_ep_t *_get_ep(esp32_usb_otg_fshs_t *usbdev, unsigned num,
                            usb_ep_dir_t dir)
{
    if (num >= ESP32_USB_OTG_FS_NUM_EP) {
        return NULL;
    }
...
static size_t _total_fifo_size(const esp32_usb_otg_fshs_config_t *conf)
{
    if (conf->type == ESP32_USB_OTG_FS) {
#ifdef ESP32_USB_OTG_FS_ENABLED
...
#ifdef ESP32_USB_OTG_HS_ENABLED
...
static void _configure_tx_fifo(esp32_usb_otg_fshs_t *usbdev, size_t num,
                               size_t len)
{
    /* TX Fifo size must be at least 16 words long and must be word aligned */
    size_t wordlen = len < (ESP32_USB_OTG_FIFO_MIN_WORD_SIZE * sizeof(uint32_t))
                     ? ESP32_USB_OTG_FIFO_MIN_WORD_SIZE
...
static void _configure_fifo(esp32_usb_otg_fshs_t *usbdev)
{
    /* TODO: cleanup, more dynamic, etc */
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
    size_t rx_size = conf->type == ESP32_USB_OTG_FS
                     ? ESP32_USB_OTG_FS_RX_FIFO_SIZE
                     : ESP32_USB_OTG_HS_RX_FIFO_SIZE;

    _global_regs(conf)->GRXFSIZ =
        (_global_regs(conf)->GRXFSIZ & ~USB_OTG_GRXFSIZ_RXFD) |
        rx_size;
    _global_regs(conf)->DIEPTXF0_HNPTXFSIZ =
        (ESP32_USB_OTG_FIFO_MIN_WORD_SIZE << USB_OTG_TX0FD_Pos) |
        rx_size;
    usbdev->fifo_pos = (rx_size + ESP32_USB_OTG_FIFO_MIN_WORD_SIZE);
}

static usbdev_ep_t *_usbdev_new_ep(usbdev_t *dev, usb_ep_type_t type,
                                   usb_ep_dir_t dir, size_t len)
{
    esp32_usb_otg_fshs_t *usbdev = (esp32_usb_otg_fshs_t *)dev;
    usbdev_ep_t *ep = NULL;
    ...
    if (type == USB_EP_TYPE_CONTROL) {
        ...
        for (unsigned idx = 1; idx < ESP32_USB_OTG_FS_NUM_EP && !ep; idx++) {
...
static void _flush_rx_fifo(const esp32_usb_otg_fshs_config_t *conf)
{
    _global_regs(conf)->GRSTCTL |= USB_OTG_GRSTCTL_RXFFLSH;
    while (_global_regs(conf)->GRSTCTL & USB_OTG_GRSTCTL_RXFFLSH) {}
}

static void _sleep_periph(const esp32_usb_otg_fshs_config_t *conf)
{
    (void)conf;
#if 1
    *_pcgcctl_reg(conf) |= USB_OTG_PCGCCTL_STOPCLK;
    DEBUG("usbdev: _sleep_periph %08x\n", *_pcgcctl_reg(conf));
#ifdef ESP32_USB_OTG_CID_1x
    pm_unblock(ESP32_PM_STOP);
#endif
#endif
}

static void _wake_periph(const esp32_usb_otg_fshs_config_t *conf)
{
#if 1
#ifdef ESP32_USB_OTG_CID_1x
    pm_block(ESP32_PM_STOP);
#endif
    *_pcgcctl_reg(conf) &= ~USB_OTG_PCGCCTL_STOPCLK;
#endif
    DEBUG("usbdev: _wake_periph %08x\n", *_pcgcctl_reg(conf));
    _flush_rx_fifo(conf);
    _flush_tx_fifo(conf, 0x10);
}

static void _reset_eps(esp32_usb_otg_fshs_t *usbdev)
{
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
...
static void _reset_periph(esp32_usb_otg_fshs_t *usbdev)
{
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
...
static void _set_mode_device(esp32_usb_otg_fshs_t *usbdev)
{
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
...
static void _usbdev_init(usbdev_t *dev)
{
    ...
    *_pcgcctl_reg(conf) = 0;
    ...
    uint8_t trdt = conf->type == ESP32_USB_OTG_FS ? 0x06 : 0x09;
    _global_regs(conf)->GUSBCFG =
    ...
    _global_regs(conf)->GINTSTS |= ESP32_FSHS_USB_GINT_MASK;
    _global_regs(conf)->GINTMSK |= ESP32_FSHS_USB_GINT_MASK;
...
static int _usbdev_ep_xmit(usbdev_ep_t *ep, uint8_t *buf, size_t len)
{
    ...
    /* Assert the alignment required for the buffers if DMA is used */
    if (_uses_dma(conf)) {
        assert(HAS_ALIGNMENT_OF(buf, USBDEV_CPU_DMA_ALIGNMENT));
    }

    DEBUG("usbdev: transfer %d byte on EP%u-%s @%u\n",
          len, ep->num, ep->dir == USB_EP_DIR_IN ? "IN" : "OUT", ztimer_now(ZTIMER_MSEC));
    ...
    if (ep->dir == USB_EP_DIR_IN) {
        ...
        if (_uses_dma(conf)) {
            ...
            container_of(ep, esp32_usb_otg_fshs_out_ep_t, ep)->out_buf = buf;
        }
        ...
        doeptsiz |= (ep->num == 0) ? 3 << USB_OTG_DOEPTSIZ_STUPCNT_Pos : 0;
...
static void _copy_rxfifo(esp32_usb_otg_fshs_t *usbdev, uint8_t *buf, size_t len)
{
...
static void _read_packet(esp32_usb_otg_fshs_out_ep_t *esp_ep)
{
    esp32_usb_otg_fshs_t *usbdev = (esp32_usb_otg_fshs_t *)esp_ep->ep.dev;
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
     ...
     * complete status*/
    if (pkt_status == ESP32_PKTSTS_DATA_UPDT ||
        pkt_status == ESP32_PKTSTS_SETUP_UPDT) {
        DEBUG("I %s %d\n", pkt_status == ESP32_PKTSTS_SETUP_UPDT ? "STUP_UPD" : "DATA_UPD", len);
        _copy_rxfifo(usbdev, esp_ep->out_buf, len);
#ifdef ESP32_USB_OTG_CID_2x
        /* CID 2x doesn't signal SETUP_COMP on non-zero length packets, signal
         * the TR_COMPLETE event immediately */
        if (esp_ep->ep.num == 0 && len) {
            usbdev->usbdev.epcb(&esp_ep->ep, USBDEV_EVENT_TR_COMPLETE);
        }
#endif  /* ESP32_USB_OTG_CID_2x */
    }
    /* On zero length frames, only the COMP status is signalled and the UPDT
     * status is skipped */
    else if (pkt_status == ESP32_PKTSTS_XFER_COMP) {
        DEBUG("I XFER_CPL %d\n", len);
        usbdev->usbdev.epcb(&esp_ep->ep, USBDEV_EVENT_TR_COMPLETE);
    }
    else if (pkt_status == ESP32_PKTSTS_SETUP_COMP) {
        DEBUG("I STUP_CPL %d\n", len);
#ifndef ESP32_USB_OTG_CID_2x
        usbdev->usbdev.epcb(&esp_ep->ep, USBDEV_EVENT_TR_COMPLETE);
#endif  /* ESP32_USB_OTG_CID_2x */
    }
}
    ...
    if (ep->dir == USB_EP_DIR_IN) {
        ...
        if ((_global_regs(conf)->GINTSTS & USB_OTG_GINTSTS_RXFLVL) &&
            (_global_regs(conf)->GRXSTSR & USB_OTG_GRXSTSP_EPNUM_Msk) == ep->num &&
             !_uses_dma(conf)) {
            _read_packet(container_of(ep, esp32_usb_otg_fshs_out_ep_t, ep));
        }
...
static void _isr_ep(esp32_usb_otg_fshs_t *usbdev)
{
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
    ...
    if (active_ep) {
        ...
        if (epnum >= ESP32_USB_OTG_REG_EP_OUT_OFFSET) {
            usbdev->usbdev.epcb(&usbdev->out[epnum - ESP32_USB_OTG_REG_EP_OUT_OFFSET].ep,
                                USBDEV_EVENT_ESR);
        }
        else {
...
void _isr_common(esp32_usb_otg_fshs_t *usbdev)
{
    const esp32_usb_otg_fshs_config_t *conf = usbdev->config;
...
static void isr_otg_fs(void *arg)
{
    ...
    esp32_usb_otg_fshs_t *usbdev = &_usbdevs[0];
...
#ifdef ESP32_USB_OTG_HS_ENABLED
static void isr_otg_hs(void *arg)
{
    (void)arg;

    /* Take the last usbdev device from the list */
    esp32_usb_otg_fshs_t *usbdev = &_usbdevs[USBDEV_NUMOF - 1];
