...
struct _spi_bus_t {
    ...
    uint32_t clk_last;               /* SPI clock speed used last time in Hz */
...
{
    /* TODO */
#if IS_USED(MODULE_SPI_RECONFIGURE)
...
static const uint32_t _spi_clk_freq_hz[] = {
    100 * KHZ(1),   /* SPI_CLK_100KHZ */
    400 * KHZ(1),   /* SPI_CLK_400KHZ */
    1 * MHZ(1),     /* SPI_CLK_1KHZ */
    5 * MHZ(1),     /* SPI_CLK_5MHZ */
    10 * MHZ(1),    /* SPI_CLK_10MHZ */
};
...
void IRAM_ATTR spi_acquire(spi_t bus, spi_cs_t cs, spi_mode_t mode, spi_clk_t clk)
{
    ...
    assert(clk < ARRAY_SIZE(_spi_clk_freq_hz));
    ...
    if (_spi_clk_freq_hz[clk] != _spi[bus].clk_last) {
        uint32_t apb_clk = rtc_clk_apb_freq_get();
        uint32_t clk_reg;

        if (apb_clk / 5 < _spi_clk_freq_hz[clk]) {
            LOG_TAG_ERROR("spi", "APB clock rate (%"PRIu32" Hz) has to be at "
                          "least 5 times SPI clock rate (%"PRIu32" Hz)\n",
                          apb_clk, _spi_clk_freq_hz[clk]);
            assert(false);
        }

        /* duty cycle is measured in is 1/256th, 50% = 128 */
        int _clk = spi_ll_master_cal_clock(apb_clk, _spi_clk_freq_hz[clk],
                                           128, &clk_reg);

        _spi[bus].clk_last = _spi_clk_freq_hz[clk];
        ...
        DEBUG("%s bus %d: SPI clock frequency: clk=%"PRIu32" eff=%d "
              "reg=%08"PRIx32"\n",
              __func__, bus, _spi_clk_freq_hz[clk], _clk, clk_reg);
    }
