...
struct _hw_timer_t {
    ...
    bool  alarm_set;   /* indicates whether timer alarm is set for the timer */
...
void IRAM_ATTR _timer_int_handler(void* arg)
{
...
#ifdef CONFIG_ZTIMER_HW_ISR_DELAY
            /* FIXME:
             * In some cases we need a small delay here before calling the
             * callback function that might set an alarm again. Otherwise, an
             * interrupt is asserted immediately again, not really yet clear why.
             */
            esp_rom_delay_us(CONFIG_ZTIMER_HW_ISR_DELAY);
#endif
    ...
    /* check whether any timer alarm is still set */
    for (unsigned dev = 0; dev < HW_TIMER_NUMOF; dev++) {
        if (_timers[dev].alarm_set) {
            alarm_active = true;
            break;
        }
    }
    /* disable the interrupt if there is no further timer alarm pending */
    if (!alarm_active) {
        esp_cpu_intr_disable(BIT(CPU_INUM_TIMER));
    }
