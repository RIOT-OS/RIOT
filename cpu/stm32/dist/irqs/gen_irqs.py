...
# IRQ table. Map such a line to the line whose IRQ count it should reuse.
ALIASES = {
...
def main(args):
    ...
    # Inject aliased lines that have no CMSIS header of their own.
    for alias_line, target_line in ALIASES.items():
        if target_line in irq_numofs and alias_line not in irq_numofs:
            irq_numofs[alias_line] = irq_numofs[target_line]
            cpu_lines.append(alias_line)

    cpu_lines = sorted(cpu_lines)
    context = {
        "cpu_fam": args.cpu_fam,
        "cpu_lines": [
            {
                "line": cpu_line.upper().replace("X", "x"),
                "irq_numof": irq_numofs[cpu_line],
            }
            for cpu_line in cpu_lines
        ]
    }
