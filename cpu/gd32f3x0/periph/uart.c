 ...
 *
 * @file
 * @brief       GD32F3x0 UART peripheral implementation
 *
...
#include "cpu.h"
// #include "clic.h"
#include "gd32f3x0_periph.h"
#ifdef MODULE_PM_LAYERED
#include "pm_layered.h"
#endif

#define RXENABLE            (USART0_CTL0_REN_Msk | USART0_CTL0_RBNEIE_Msk)
...
int uart_init(uart_t uart, uint32_t baudrate, uart_rx_cb_t rx_cb, void *arg)
{
    ...
    if (rx_cb) {
        ...
        dev(uart)->CTL0 = (USART0_CTL0_UEN_Msk | USART0_CTL0_TEN_Msk | RXENABLE);
    }
    else {
        dev(uart)->CTL0 = (USART0_CTL0_UEN_Msk | USART0_CTL0_TEN_Msk);
    }
...
{
    while (!(dev(uart)->STAT & USART0_STAT_TBE_Msk)) {}
    dev(uart)->TDATA = byte;
}

static inline void wait_for_tx_complete(uart_t uart)
{
    while (!(dev(uart)->STAT & USART0_STAT_TC_Msk)) {}
}
...
void uart_write(uart_t uart, const uint8_t *data, size_t len)
{
    ...
    if (!(dev(uart)->CTL0 & USART0_CTL0_TEN_Msk)) {
        return;
    }
#endif
...
void uart_poweron(uart_t uart)
{
    ...
    dev(uart)->CTL0 |= (USART0_CTL0_UEN_Msk);
}
...
static inline void _irq_handler(uart_t uart)
{
    ...
    if (status & USART0_STAT_RBNE_Msk) {
        isr_ctx[uart].rx_cb(isr_ctx[uart].arg,
                            (uint8_t)dev(uart)->RDATA & isr_ctx[uart].data_mask);
    }
    if (status & USART0_STAT_ORERR_Msk) {
        dev(uart)->RDATA;
    }
}
