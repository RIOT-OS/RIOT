 ...
 *
 * @file
 * @brief       NewLib system calls implementations for PSoC5 LP
 *
 ...
 * manage the heap
 */
extern char __cy_heap_start;                 /* start of the heap */
extern char __cy_heap_end;                 /* end of the heap */
caddr_t heap_top = (caddr_t)&__cy_heap_start + 4;
...
caddr_t _sbrk_r(struct _reent *r, ptrdiff_t incr)
{
    ...
    if (((incr > 0) && ((heap_top + incr > &__cy_heap_end) || (heap_top + incr < res))) ||
        ((incr < 0) && ((heap_top + incr < &__cy_heap_start) || (heap_top + incr > res)))) {
        r->_errno = ENOMEM;
        res = (void *) -1;
    }
    else {
