...
```shell
BOARD=arduino-mega2560 make flash
```
...
```shell
BOARD=arduino-mega2560 make debug-server
```

starts an [avarice](https://github.com/avrdudes/avarice) (avarice needs to be installed)
server that `avr-gdb` can connect to.

```shell
BOARD=arduino-mega2560 make debug
```

starts an avarice server and connects `avr-gdb` to it.
...
```shell
BOARD=arduino-mega2560 PROGRAMMER=dragon_isp make clean all flash debug
```
