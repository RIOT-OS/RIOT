...
{
    uint8_t ie = 0;
    int res = _read_regs(dev, MAX31343_REG_INT_EN, &ie, 1);
    if (res != 0) {
        return res;
    }
    ...
    return _write_regs(dev, MAX31343_REG_INT_EN, &ie, 1);
}
...
int max31343_init(max31343_t *dev, const max31343_params_t *params)
{
    ...
    uint8_t status;
    i2c_acquire(dev->i2c);
    int res = _read_regs(dev, MAX31343_REG_STATUS, &status, 1);
    i2c_release(dev->i2c);
    if (res != 0) {
        return res;
    }
...
int max31343_set_alarm_int(const max31343_t *dev, bool enable)
{
    ...
    if (res == 0 && !enable) {
        uint8_t st;
        (void) _read_regs(dev, MAX31343_REG_STATUS, &st, 1);
    }
...
int max31343_poweron(const max31343_t *dev)
{
    ...
    int res = _read_regs(dev, MAX31343_REG_RTC_CFG1, &cfg1, 1);
    if (res == 0) {
        cfg1 |= MAX31343_RTC_CFG1_ENOSC;
        res = _write_regs(dev, MAX31343_REG_RTC_CFG1, &cfg1, 1);
    }
...
int max31343_set_sqw(const max31343_t *dev, max31343_sqw_freq_t freq)
{
    ...
    int res = _read_regs(dev, MAX31343_REG_RTC_CFG2, &cfg2, 1);
    if (res == 0) {
        cfg2 = (uint8_t)((cfg2 & ~MAX31343_RTC_CFG2_SQW_HZ_MASK) |
                          ((uint8_t)freq & MAX31343_RTC_CFG2_SQW_HZ_MASK));
        res = _write_regs(dev, MAX31343_REG_RTC_CFG2, &cfg2, 1);
    }
                                   ...
                                   max31343_trickle_res_t res)
{
    ...
    i2c_acquire(dev->i2c);
    int ret = _write_regs(dev, MAX31343_REG_TRICKLE, &reg, 1);
    i2c_release(dev->i2c);
    return ret;
}
...
int max31343_trickle_charge_disable(const max31343_t *dev)
{
    ...
    i2c_acquire(dev->i2c);
    uint8_t reg = 0x00;
    int res = _write_regs(dev, MAX31343_REG_TRICKLE, &reg, 1);
    i2c_release(dev->i2c);
    return res;
}
