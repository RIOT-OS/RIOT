...
void at86rf2xx_set_csma_max_retries(at86rf2xx_t *dev, int8_t retries)
{
    ...
    DEBUG("[at86rfr2] opt: Set CSMA retries to %u\n", retries);
...
void at86rf2xx_set_csma_backoff_exp(at86rf2xx_t *dev, uint8_t min, uint8_t max)
{
    max = (max > 8) ? 8 : max;
    min = (min > max) ? max : min;
    DEBUG("[at86rfr2] opt: Set min BE=%u, max BE=%u\n", min, max);
...
{
    if(entropy == NULL) {
        DEBUG("[at86rfr2] opt: CSMA seed entropy is nullpointer\n");
        return;
    }
    DEBUG("[at86rfr2] opt: Set CSMA seed to 0x%x 0x%x\n", entropy[0], entropy[1]);
    ...
    if (state) {
        ...
        switch (option) {
            case AT86RF2XX_OPT_CSMA:
                DEBUG("[at86rfr2] opt: enabling CSMA mode" \
            ...
            case AT86RF2XX_OPT_PROMISCUOUS:
                DEBUG("[at86rfr2] opt: enabling PROMISCUOUS mode\n");
            ...
            case AT86RF2XX_OPT_AUTOACK:
                DEBUG("[at86rfr2] opt: enabling auto ACKs\n");
                tmp = at86rf2xx_reg_read(dev, AT86RF2XX_REG__CSMA_SEED_1);
                tmp &= ~(AT86RF2XX_CSMA_SEED_1__AACK_DIS_ACK);
                at86rf2xx_reg_write(dev, AT86RF2XX_REG__CSMA_SEED_1, tmp);
                break;
            case AT86RF2XX_OPT_TELL_RX_START:
                DEBUG("[at86rfr2] opt: enabling SFD IRQ\n");
        ...
        switch (option) {
            case AT86RF2XX_OPT_CSMA:
                DEBUG("[at86rfr2] opt: disabling CSMA mode\n");
                /* setting retries to -1 means CSMA disabled */
                at86rf2xx_set_csma_max_retries(dev, -1);
                break;
            case AT86RF2XX_OPT_PROMISCUOUS:
                DEBUG("[at86rfr2] opt: disabling PROMISCUOUS mode\n");
                ...
                break;
            case AT86RF2XX_OPT_AUTOACK:
                DEBUG("[at86rfr2] opt: disabling auto ACKs\n");
                tmp = at86rf2xx_reg_read(dev, AT86RF2XX_REG__CSMA_SEED_1);
                tmp |= AT86RF2XX_CSMA_SEED_1__AACK_DIS_ACK;
                at86rf2xx_reg_write(dev, AT86RF2XX_REG__CSMA_SEED_1, tmp);
                break;
            case AT86RF2XX_OPT_TELL_RX_START:
                DEBUG("[at86rfr2] opt: disabling SFD IRQ\n");
                tmp = at86rf2xx_reg_read(dev, AT86RF2XX_REG__IRQ_MASK);
                tmp &= ~AT86RF2XX_IRQ_STATUS_MASK__RX_START;
                at86rf2xx_reg_write(dev, AT86RF2XX_REG__IRQ_MASK, tmp);
                break;
...
static inline void _set_state(at86rf2xx_t *dev, uint8_t state)
{
    ...
    /* If TRX_STATUS = 0x1F (STATE_TRANSITION_IN_PROGRESS)  the radio transceiver  is on a state transition.
...
void at86rf2xx_set_state(at86rf2xx_t *dev, uint8_t state)
{
    ...
    while (old_state == AT86RF2XX_STATE_BUSY_RX_AACK ||
    	...
    	DEBUG("wait to be ready to set state: 0x%x\n", old_state);
    }

    if (state == old_state) {
    	DEBUG("state was already: 0x%x\n", old_state);
        return;
    }
    ...
    if ((old_state == AT86RF2XX_STATE_RX_AACK_ON &&
    	...
    	DEBUG("set AT86RF2XX_STATE_PLL_ON\n");
    ...
    if (state == AT86RF2XX_STATE_SLEEP) {
    	DEBUG("set AT86RF2XX_STATE_SLEEP\n");
        /* First go to TRX_OFF */
