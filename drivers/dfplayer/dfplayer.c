...
int dfplayer_play_file(dfplayer_t *dev, uint8_t folder, uint8_t file)
{
    ...
    dev->folder = folder;
    dev->file = file;
    dev->flags |= DFPLAYER_FLAG_FOLDER_FILE;
    mutex_unlock(&dev->mutex);
    return 0;
}
...
int dfplayer_step(dfplayer_t *dev, int step)
{
    ...
    mutex_lock(&dev->mutex);
    uint8_t cmd, p1, p2;
    if (dev->flags & DFPLAYER_FLAG_FOLDER_FILE) {
        /* Currently using naming scheme <FOLDERNUM>/<FILENUM>.mp3 */
        if ((dev->file + step < 1) || (dev->file + step > UINT8_MAX)) {
            mutex_unlock(&dev->mutex);
            return -ERANGE;
        }
        cmd = DFPLAYER_CMD_FILE;
        p1 = dev->folder;
        p2 = (uint8_t)(dev->file + step);
    }
    else {
        /* Currently using naming scheme MP3/<FILENUM>.mp3 */
        if ((dev->track + step < 1) ||
            (dev->track + step > DFPLAYER_MAX_MP3_FILE))
        {
            mutex_unlock(&dev->mutex);
            return -ERANGE;
        }
        cmd = DFPLAYER_CMD_PLAY_FROM_MP3;
        p1 = (uint8_t)((dev->track + step) >> 8);
        p2 = (uint8_t)(dev->track + step);
    }
    ...
    if (dev->flags & DFPLAYER_FLAG_FOLDER_FILE) {
        /* Currently using naming scheme <FOLDERNUM>/<FILENUM>.mp3 */
        dev->file = (uint8_t)(dev->file + step);
    }
    else {
        /* Currently using naming scheme MP3/<FILENUM>.mp3 */
        dev->track = (uint16_t)(dev->track + step);
    }
