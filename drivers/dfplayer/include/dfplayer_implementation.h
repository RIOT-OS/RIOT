...
{
    return dfplayer_cmd(dev, DFPLAYER_CMD_NEXT);
}
...
{
    return dfplayer_cmd(dev, DFPLAYER_CMD_PREV);
}
...
{
    return dfplayer_cmd(dev, DFPLAYER_CMD_VOLUME_INC);
}

static inline int dfplayer_volume_decrease(dfplayer_t *dev)
{
    return dfplayer_cmd(dev, DFPLAYER_CMD_VOLUME_DEC);
}
