...
#define checkCRC(buffer, \
                 size) (ucrc16_calc_le(buffer, size, 0xA001, 0xFFFF) != 0)
...
static inline int prepare_request(modbus_rtu_t *modbus)
{
    ...
    switch (modbus->msg->func) {
        ...
        case MB_FC_WRITE_COIL:
            ...
            modbus->buffer[4] =
                ((modbus->msg->data[d.quot] >> d.rem) & 1u) ? 0xff : 0x00;
            modbus->buffer[5] = 0;
            /* id + func + addr + data */
            modbus->size_buffer = 6;
            break;
        ...
        case MB_FC_WRITE_REGISTERS:
            ...
            /* (id + func + addr + count + size) + data */
            modbus->size_buffer = 7 + size;
            copy_registers_to_buffer(modbus->buffer + 7,
    ...
    if (xtimer_msg_receive_timeout(&msg, modbus->timeout) < 0) {
        ...
        switch (modbus->buffer[FUNC]) {
            ...
            defined(MODBUS_RTU_USE_READ_INPUT_REGISTER)
                ...
                copy_buffer_to_registers(modbus->msg->data + modbus->msg->addr,
                                         modbus->buffer + 3,
                                         modbus->msg->count);
                mutex_unlock(&(modbus->mutex_buffer));
                break;
    ...
    while (1) {
        ...
        switch (modbus->buffer[FUNC]) {
            ...
            case MB_FC_WRITE_COILS:
                ...
                read_address(modbus);
                read_count(modbus);
                if ((modbus->msg->addr + modbus->msg->count) >
                    (modbus->msg->data_size * 8)) {
                    mutex_unlock(&(modbus->mutex_buffer));
                    err = MB_ER_ILLEGAL_ADDRESS;
                    goto exit;
                }
            ...
            case MB_FC_WRITE_REGISTERS:
                ...
                if ((modbus->msg->addr * 2 + modbus->buffer[BYTE_CNT]) >
                    modbus->msg->data_size) {
                    mutex_unlock(&(modbus->mutex_buffer));
                    err = MB_ER_ILLEGAL_ADDRESS;
                    goto exit;
                }
                             ...
                             modbus_rtu_message_t *message)
{
    ...
    switch (modbus->msg->func) {
        ...
        defined(MODBUS_RTU_USE_READ_INPUT_REGISTER)
            ...
            modbus->size_buffer = 3 + size;
            copy_registers_to_buffer(modbus->buffer + 3,
                                     modbus->msg->data + modbus->msg->addr,
                                     modbus->msg->count);
            break;
#endif  /* MODBUS_RTU_USE_READ_REGISTERS || MODBUS_RTU_USE_READ_INPUT_REGISTER */
