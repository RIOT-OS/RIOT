...
#include "at24mac_id_params.h"

#define CMD_READ_EUI48      (0x9A)
#define CMD_READ_EUI64      (0x98)
#define CMD_READ_ID128      (0x80)

static bool _is_valid(at24mac_id_type_t type, uint8_t reg)
{
...
static int _read_reg(at24mac_id_t dev, uint8_t reg, void *dst, size_t size)
{
    if (dev >= ARRAY_SIZE(at24mac_id_params)) {
        return -ERANGE;
    }

    int res = 0;
    const at24mac_id_params_t *params = &at24mac_id_params[dev];
...
int at24mac_id_get_eui48(at24mac_id_t dev, eui48_t *dst)
{
    return _read_reg(dev, CMD_READ_EUI48, dst, sizeof(*dst));
}

int at24mac_id_get_eui64(at24mac_id_t dev, eui64_t *dst)
{
...
at24mac_id_type_t at24mac_id_get_type(at24mac_id_t dev)
{
    ...
    return at24mac_id_params[dev].type;
}
