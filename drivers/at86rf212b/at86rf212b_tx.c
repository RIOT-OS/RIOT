...
static int16_t at86rf212b_load(at86rf212b_packet_t *packet);
static void at86rf212b_gen_pkt(uint8_t *buf, at86rf212b_packet_t *packet);
...
{
    int16_t result;
    result = at86rf212b_load(packet);
    if (result < 0) {
        return result;
    }
    at86rf212b_transmit_tx_buf(NULL);
    return result;
}
        ...
        unsigned int len)
{
    ...
    uint16_t src_pan = at86rf212b_get_pan();
    ...
    if (use_long_addr) {
        ...
        uint64_t src_long_addr = at86rf212b_get_address_long();
        ...
        uint16_t src_addr = at86rf212b_get_address();
    ...
    if (size > AT86RF212B_MAX_PKT_LENGTH) {
        DEBUG("at86rf212b: packet too long, dropped it.\n");
    ...
    at86rf212b_write_fifo(&size_byte, 1);
    at86rf212b_write_fifo(mhr, (radio_packet_length_t)index);
    if (upper_layer_hdrs) {
        do {
            at86rf212b_write_fifo(ptr->header,
                                (radio_packet_length_t)(ptr->header_len));
            netdev_hlist_advance(&ptr);
        } while (ptr != upper_layer_hdrs);
    }
    at86rf212b_write_fifo((uint8_t*)buf, len);
...
netdev_802154_tx_status_t at86rf212b_transmit_tx_buf(netdev_t *dev)
{
    ...
    /* will be freed in at86rf212b_rx_irq when TRX_END interrupt occurs */
    driver_state = AT_DRIVER_STATE_SENDING;

    /* Start TX */
    at86rf212b_reg_write(AT86RF212B_REG__TRX_STATE, AT86RF212B_TRX_STATE__TX_START);
    DEBUG("Started TX\n");

    if (!wait_for_ack) {
        DEBUG("at86rf212b: Don't wait for ACK, TX done.\n");
        return NETDEV_802154_TX_STATUS_OK;
    }

    uint8_t trac_status;
    do {
        trac_status = at86rf212b_reg_read(AT86RF212B_REG__TRX_STATE);
        trac_status &= AT86RF212B_TRX_STATE_MASK__TRAC;
    }
    while (trac_status == AT86RF212B_TRX_STATE__TRAC_INVALID);

    switch (trac_status) {
        case AT86RF212B_TRX_STATE__TRAC_CHANNEL_ACCESS_FAILURE:
            return NETDEV_802154_TX_STATUS_MEDIUM_BUSY;

        case AT86RF212B_TRX_STATE__TRAC_NO_ACK:
...
int16_t at86rf212b_load(at86rf212b_packet_t *packet)
{
    /* If previous transmission is still pending*/
    ...
    packet->frame.src_pan_id = at86rf212b_get_pan();
    ...
    if (packet->frame.fcf.src_addr_m == 2) {
        packet->frame.src_addr[0] = (uint8_t)(at86rf212b_get_address() >> 8);
        packet->frame.src_addr[1] = (uint8_t)(at86rf212b_get_address() & 0xFF);
    }
    else if (packet->frame.fcf.src_addr_m == 3) {
        packet->frame.src_addr[0] = (uint8_t)(at86rf212b_get_address_long() >> 56);
        packet->frame.src_addr[1] = (uint8_t)(at86rf212b_get_address_long() >> 48);
        packet->frame.src_addr[2] = (uint8_t)(at86rf212b_get_address_long() >> 40);
        packet->frame.src_addr[3] = (uint8_t)(at86rf212b_get_address_long() >> 32);
        packet->frame.src_addr[4] = (uint8_t)(at86rf212b_get_address_long() >> 24);
        packet->frame.src_addr[5] = (uint8_t)(at86rf212b_get_address_long() >> 16);
        packet->frame.src_addr[6] = (uint8_t)(at86rf212b_get_address_long() >> 8);
        packet->frame.src_addr[7] = (uint8_t)(at86rf212b_get_address_long() & 0xFF);
    }
    ...
    if (packet->length > AT86RF212B_MAX_PKT_LENGTH) {
        DEBUG("at86rf212b: ERROR: packet too long, dropped it.\n");
        return -1;
    }
    ...
    at86rf212b_reg_write(AT86RF212B_REG__TRX_STATE, AT86RF212B_TRX_STATE__TX_ARET_ON);

    do {
        int max_wait = _MAX_RETRIES;
        if (!--max_wait) {
            DEBUG("at86rf212b : ERROR : could not enter TX_ARET_ON mode\n");
            break;
        }
    } while (at86rf212b_get_status() != AT86RF212B_TRX_STATUS__TX_ARET_ON);

    
     at86rf212b_reg_read(AT86RF212B_REG__IRQ_STATUS);
   
    /* load packet into fifo */
    at86rf212b_write_fifo(pkt, packet->length);
    DEBUG("Wrote to FIFO\n");
...
/**
 * @brief Static function to generate byte array from at86rf212b packet.
 */
static void at86rf212b_gen_pkt(uint8_t *buf, at86rf212b_packet_t *packet)
{
