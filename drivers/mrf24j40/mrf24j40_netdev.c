...
// muss mal gucken wegen Partnummer
    /* test if the SPI is set up correctly and the device is responding */
   /* if (mrf24j40_reg_read(dev, AT86RF2XX_REG__PART_NUM) !=
        AT86RF2XX_PARTNUM) {
...
static int _recv(netdev2_t *netdev, char *buf, int len)
{
     ...
     * is called*/
    mrf24j40_fb_start(dev); // muss gucken wie das beim mrf24j40 geht
    /* get the size of the received packet */
    mrf24j40_fb_read(dev, &phr, 1); // muss gucken wie das beim mrf24j40 geht
