...
netdev_802154_tx_status_t cc2420_load_tx_buf(netdev_t *dev,
        ...
        unsigned int len)
{
    (void)dev;  /* TODO: see cc2420_initialize() in cc2420.c */
    ...
    /* total frame size */
    size_t size = idx + len + 2 + netdev_get_hlist_len(upper_layer_hdrs);
    ...
    cc2420_write_fifo(&size_byte, 1);
    cc2420_write_fifo(hdr, idx);

    if (upper_layer_hdrs) {
        do {
            cc2420_write_fifo(ptr->header,
...
netdev_802154_tx_status_t cc2420_transmit_tx_buf(netdev_t *dev)
{
    ...
    if (st & CC2420_STATUS_TX_UNDERFLOW) {
        return NETDEV_802154_TX_STATUS_UNDERFLOW;
    }
