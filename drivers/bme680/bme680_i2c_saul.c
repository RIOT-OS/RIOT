...
static bool _temp_valid[BME680_I2C_NUMOF] = { false };
static bool _press_valid[BME680_I2C_NUMOF] = { false };
static bool _hum_valid[BME680_I2C_NUMOF] = { false };
static bool _gas_valid[BME680_I2C_NUMOF] = { false };
static int16_t _temp[BME680_I2C_NUMOF];
static int16_t _press[BME680_I2C_NUMOF];
static int16_t _hum[BME680_I2C_NUMOF];
static uint32_t _gas[BME680_I2C_NUMOF];

extern bme680_i2c_t bme680_i2c_devs_saul[BME680_I2C_NUMOF];

static unsigned _dev2index (const bme680_i2c_t *dev)
{
    /*
     * returns the index of the device in bme680_i2c_devs_saul[] or BME680_I2C_NUMOF
     * if not found
     */
    for (unsigned i = 0; i < BME680_I2C_NUMOF; i++) {
        if (dev == &bme680_i2c_devs_saul[i]) {
            return i;
        }
    }
    return BME680_I2C_NUMOF;
}
...
static int _read(int dev)
{
    ...
    int res;
    bme680_data_t data;
    if ((res = bme680_i2c_read(&bme680_i2c_devs_saul[dev], &data)) != 0) {
        return res;
    }
...
static int read_temp(const void *dev, phydat_t *data)
{
    ...
    unsigned dev_index = _dev2index((const bme680_i2c_t *)dev);
    if (dev_index == BME680_I2C_NUMOF) {
...
const saul_driver_t bme680_i2c_saul_driver_temperature = {
    .read = read_temp,
    .write = saul_notsup,
    .type = SAUL_SENSE_TEMP
};

const saul_driver_t bme680_i2c_saul_driver_pressure = {
    .read = read_press,
    .write = saul_notsup,
    .type = SAUL_SENSE_PRESS
};

const saul_driver_t bme680_i2c_saul_driver_humidity = {
    .read = read_hum,
    .write = saul_notsup,
    .type = SAUL_SENSE_HUM
};

const saul_driver_t bme680_i2c_saul_driver_gas = {
    .read = read_gas,
    .write = saul_notsup,
    .type = SAUL_SENSE_GAS
};
