...
static int _qma6100p_run_init_seq(const qma6100p_t *dev)
{
    ...
    _reg_field_set(QMA6100P_PM_MCLK_MASK, QMA6100P_PM_MCLK_51K2, &pm);
    res = _write_reg(BUS, ADDR, QMA6100P_REG_PM, pm);
    if (res < 0) {
...
static int _qma6100p_set_range(const qma6100p_t *dev, qma6100p_range_t range)
{
    ...
    _reg_field_set(QMA6100P_RANGE_MASK, range, &range_reg);
...
static int _qma6100p_set_odr(const qma6100p_t *dev, qma6100p_odr_t odr)
{
    ...
    _reg_field_set(QMA6100P_ODR_MASK, odr, &odr_reg);
...
static int _qma6100p_set_mclk(const qma6100p_t *dev, qma6100p_mclk_t mclk)
{
    ...
    _reg_field_set(QMA6100P_PM_MCLK_MASK, mclk, &pm_reg);
...
static int _set_int_params(qma6100p_t *dev)
{
    ...
    _reg_field_set(QMA6100P_INT_CFG_SHADOW_MASK, dev->params.interrupt_shadow, &int_reg);
                            ...
                            uint8_t *map_reg)
{
    ...
    case QMA6100P_INT1:
        *map_reg = QMA6100P_REG_INT1_MAP1;
        _reg_field_set(QMA6100P_INTPIN_INT1_LVL_MASK, QMA6100P_PARAM_INT_ACTIVE_LEVEL, &reg);
        _reg_field_set(QMA6100P_INTPIN_INT1_OD_MASK, QMA6100P_PARAM_INT_PIN_MODE, &reg);
        break;
    case QMA6100P_INT2:
        *map_reg = QMA6100P_REG_INT2_MAP1;
        _reg_field_set(QMA6100P_INTPIN_INT2_LVL_MASK, QMA6100P_PARAM_INT_ACTIVE_LEVEL, &reg);
        _reg_field_set(QMA6100P_INTPIN_INT2_OD_MASK, QMA6100P_PARAM_INT_PIN_MODE, &reg);
        break;
    default:
...
static int _route_data_ready_int(const qma6100p_t *dev, uint8_t map_reg)
{
    ...
    _reg_field_set(QMA6100P_INT_MAP1_DATA_MASK, 1, &reg);
...
static int _enable_data_ready_int(const qma6100p_t *dev)
{
    ...
    _reg_field_set(QMA6100P_INT_EN1_DATA_MASK, 1, &reg);
