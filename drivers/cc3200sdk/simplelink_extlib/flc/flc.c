...
static _i32 _WriteBootInfo(sBootInfo_t *psBootInfo) {
    ...
    if (0
            == sl_FsOpen((_u8 *) IMG_BOOT_INFO, FS_MODE_OPEN_WRITE, &ulToken,
                    &lFileHandle)) {
        if (0
                < sl_FsWrite(lFileHandle, 0, (_u8 *) psBootInfo,
                        sizeof(sBootInfo_t))) {
...
static _i32 _ReadBootInfo(sBootInfo_t *psBootInfo) {
    ...
    if (0
            == sl_FsOpen((_u8 *) IMG_BOOT_INFO, FS_MODE_OPEN_READ, &ulToken,
                    &lFileHandle)) {
        if (0
                < sl_FsRead(lFileHandle, 0, (_u8 *) psBootInfo,
                        sizeof(sBootInfo_t))) {
            status = 0;
...
_i32 sl_extlib_FlcOpenFile(_u8 *file_name, _i32 file_size, _u32 *ulToken,
    ...
    if (open_flags == _FS_MODE_OPEN_READ)
        return sl_FsOpen((_u8 *) file_name, _FS_MODE_OPEN_READ, ulToken,
                lFileHandle);
    else
        return sl_FsOpen((_u8 *) file_name,
                FS_MODE_OPEN_CREATE(file_size, open_flags), ulToken,
                lFileHandle);
}
...
_i32 sl_extlib_FlcCloseFile(_i32 fileHandle, _u8 *pCeritificateFileName,
        _u8 *pSignature, _u32 SignatureLen) {
    if (SignatureLen == 0) {
        pSignature = 0;
    }
    return sl_FsClose(fileHandle, pCeritificateFileName, pSignature,
            SignatureLen);
}
