...
_i32 LogClient_ConnectServer(void *pvLogClient,
    ...
    pLogClient->serverSockId = http_connect_server(
            pOtaServerInfo->log_server_name, 0, SOCKET_PORT_DEFAULT,
            SOCKET_SECURED, SOCKET_BLOCKING);
...
_i32 LogClient_Print(void *pvLogClient, _u8 *pFilename,
    ...
    http_build_request(send_buf, "PUT ",
            pLogClient->pOtaServerInfo->log_server_name,
            pLogClient->pOtaServerInfo->rest_files_put, log_filename,
            pLogClient->pOtaServerInfo->rest_hdr,
            pLogClient->pOtaServerInfo->rest_hdr_val);

    /* prepare log buffer */
    sprintf((char *) log_buf,
            "start counter=%ld, access errors=%ld ---------------\n",
            pStatistics->startCount, pStatistics->continuousAccessErrorCount);
    sprintf((char *) &log_buf[strlen((const char *) log_buf)],
            "connect server done=%ld, error=%ld, update=%ld, metadata=%ld\n",
            pStatistics->ota_connect_done, pStatistics->ota_connect_error,
            pStatistics->ota_error_updatecheck,
            pStatistics->ota_error_metadata);
    sprintf((char *) &log_buf[strlen((const char *) log_buf)],
            "connect cdn    done=%ld, error=%ld\n",
            pStatistics->cdn_connect_done, pStatistics->cdn_connect_error);
    sprintf((char *) &log_buf[strlen((const char *) log_buf)],
            "download file  done=%ld, error=%ld, save_chunk=%ld, max_eagain=%ld\n",
            pStatistics->download_done, pStatistics->download_error,
    ...
    sprintf((char *) &send_buf[strlen((const char *) send_buf)],
            "Content-Length: %d", strlen((const char *) log_buf));
    /*strcat(send_buf, "Content-Length: 76"); */
    strcat((char *) send_buf, "\r\n");
    strcat((char *) send_buf, "\r\n");

    len = sl_Send(pLogClient->serverSockId, send_buf,
    ...
    len = sl_Send(pLogClient->serverSockId, log_buf,
            (_i16) strlen((const char *) log_buf), 0);
    if (len <= 0) {
        Report("LogClient_UpdateCheck: ERROR metadata sl_Send status=%ld\r\n", len);
        return OTA_STATUS_ERROR;
    }
...
_i32 LogClient_ConnectAndPrint(void *pvLogClient,
        OtaOptServerInfo_t *pOtaServerInfo, _u8 *vendorStr,
        OtaApp_statistics_t *pStatistics) {
    _i32 status;
    _u8 filename[40];
    ...
    sprintf((char *) filename, "%s_Mac%02X%02X%02X%02X%02X%02X", vendorStr,
