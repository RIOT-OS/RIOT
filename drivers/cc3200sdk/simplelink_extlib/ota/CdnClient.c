...
_i32 _OpenStorageFile(CdnClient_t *pCdnClient, _u8 *file_name, _i32 file_size,
        _u32 *ulToken, _i32 *lFileHandle);
...
_i32 _readSignature(CdnClient_t *pCdnClient, _u8 *signature_filename,
...
_i32 CdnClient_ConnectByUrl(void *pvCdnClient,
    ...
    if (strcmp((const char *) pCdnClient->cdn_server_name,
            (const char *) domain_name) != 0) {
        /* close previous CDN server socket */
        CdnClient_CloseServer(pCdnClient);
        strcpy((char *) pCdnClient->cdn_server_name,
                (const char *) domain_name);
        /* connect to the CDN server */
        pCdnClient->port_num = SOCKET_PORT_DEFAULT;
        pCdnClient->fileSockId = http_connect_server(
                pCdnClient->cdn_server_name, 0, pCdnClient->port_num,
                secured_connection, SOCKET_BLOCKING);
    ...
    status = _ReadFileHeaders(pCdnClient->fileSockId,
            pCdnClient->cdn_server_name, req_uri);
    if (status < 0) {
...
_i32 _readSignature(CdnClient_t *pCdnClient, _u8 *signature_filename,
    ...
    status = pFlcCb->pOpenFile((_u8 *) signature_filename, 0, NULL,
            &lFileHandle, _FS_MODE_OPEN_READ);
    if (status < 0) {
        Report("_readSignature: Error pOpenFile\r\n");
        return OTA_STATUS_ERROR;
    }

    len = pFlcCb->pReadFile(lFileHandle, 0, pMetadata->signature,
            sizeof(pMetadata->signature));
    if (len < 0) {
...
_i32 _OpenStorageFile(CdnClient_t *pCdnClient, _u8 *file_name, _i32 file_size,
    ...
    if (pMetadata->flags & METADATA_FLAGS_SECURED) {
        ...
        if ((pMetadata->flags & METADATA_FLAGS_SIGNATURE) == 0) {
            ...
            status = _readSignature(pCdnClient, pMetadata->signature_filename,
                    pMetadata);
            if (status < 0) {
    ...
    status = pCdnClient->pFlcCb->pOpenFile((_u8 *) file_name, file_size,
            ulToken, lFileHandle, fs_open_flags);
    if (status < 0) {
...
_i32 CdnClient_Run(void *pvCdnClient) {
    ...
    case CDN_STATE_CDN_SERVER_CONNECTED:
        ...
        /*  create a user file */
        status = _OpenStorageFile(pCdnClient, pCdnClient->p_file_name,
                pCdnClient->file_size, &pCdnClient->ulToken,
                &pCdnClient->lFileHandle);
        if (status < 0) {
            pCdnClient->state = CDN_STATE_IDLE;
    ...
    case CDN_STATE_FILE_DOWNLOAD_AND_SAVE:
        ...
        bytesReceived = _RecvFileChunk(pCdnClient->fileSockId,
                &pCdnClient->recv_buf[0], HTTP_RECV_BUF_LEN, 0);
        if (0 >= bytesReceived) {
        ...
        if ((pCdnClient->totalBytesReceived + bytesReceived)
                >= pCdnClient->file_size) {
            bytesReceived = pCdnClient->file_size
                    - pCdnClient->totalBytesReceived;
            endBuf = pCdnClient->recv_buf + 1; /* sign EOF  */
        } else {
            /* Force align length */
            if (bytesReceived & ALIGNMENT_READ_MASK) {
                status = _RecvFileChunk(pCdnClient->fileSockId,
                        &pCdnClient->recv_buf[bytesReceived],
                        (ALIGNMENT_READ_MASK + 1)
                                - (bytesReceived & ALIGNMENT_READ_MASK), 0);
                if (0 >= status) {
        ...
        Report("total %ld.\r\n", pCdnClient->totalBytesReceived );
        status = pFlcCb->pWriteFile((_i32) pCdnClient->lFileHandle,
                pCdnClient->totalBytesReceived, pCdnClient->recv_buf,
                bytesReceived);
        if (status < 0) {
        ...
        if (endBuf > pCdnClient->recv_buf) {
            ...
            status = pFlcCb->pCloseFile(pCdnClient->lFileHandle,
                    (_u8 *) pCdnClient->pResourceMetadata->p_cert_filename,
                    (_u8 *) pCdnClient->pResourceMetadata->p_signature,
