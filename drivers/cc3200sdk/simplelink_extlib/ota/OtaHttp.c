 ...
 */
_i32 http_build_request(_u8 *pHttpReq, _u8 *method, _u8 *host_name,
        _u8 *req_uri_prefix, _u8 *req_uri, _u8 *hdr_name, _u8 *hdr_val) {
...
_i32 sl_Recv_eagain(_i16 sockId, void *pBuf, _i32 Len, _i32 flags,
        _i32 max_eagain) {
    _i32 len;
 ...
 */
_i32 json_parse_dropbox_metadata(_i16 sockId, RsrcData_t *pRsrcData,
        _u8 *read_buf, _i32 size) {
...
_i32 json_parse_update_check_resp(_i16 sockId, RsrcData_t *pRsrcData,
        _u8 *read_buf, _i32 size) {
    RsrcData_t rsrcData;
    _u8 size_str[64];
    _u8 *pBuf;
...
/* ----------------------------------------------------- */
_i16 http_connect_server(_u8 *ServerName, _i32 IpAddr, _i32 port, _i32 secured,
        _i32 nonBlocking) {
    _i32 Status;
    SlSockAddrIn_t Addr;
    _i32 AddrSize;
    ...
    if (IpAddr == 0) {
        /* get host IP by name */
        Status = sl_NetAppDnsGetHostByName((_i8 *) ServerName,
                strlen((const char *) ServerName), &ServerIP, SL_AF_INET);
        if (0 > Status) {
            Report("http_connect_server: ERROR sl_NetAppDnsGetHostByName, status=%ld\r\n", Status);
            if (g_ConsecutiveNetAccessNetErrors++
                    >= MAX_CONSECUTIVE_NET_ACCESS_ERRORS) {
    ...
    SockID = sl_Socket(SL_AF_INET, SL_SOCK_STREAM,
            (secured ? SL_SEC_SOCKET : 0));
    if (SockID < 0) {
