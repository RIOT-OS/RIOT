...
_i32 OtaClient_ResourceNameConvert(void *pvOtaClient, _u8 *resource_file_name,
        OtaFileMetadata_t *pMetadata);
...
_i32 OtaClient_ConnectServer(void *pvOtaClient,
    ...
    pOtaClient->serverSockId = http_connect_server(
            pOtaServerInfo->server_domain, pOtaServerInfo->ip_address,
            SOCKET_PORT_DEFAULT, pOtaServerInfo->secured_connection,
            SOCKET_BLOCKING);
...
#else
    http_build_request(send_buf, "GET ", pOtaServerInfo->server_domain,
            pOtaServerInfo->rest_update_chk, pOtaClient->pVendorStr,
            pOtaServerInfo->rest_hdr, pOtaServerInfo->rest_hdr_val);
#endif

    len = sl_Send(pOtaClient->serverSockId, send_buf,
            (_i16) strlen((const char *) send_buf), 0);
    if (len <= 0) {
...
#else
    numUpdates = json_parse_dropbox_metadata(pOtaClient->serverSockId,
            pOtaClient->pOtaCheckUpdateResponse->rsrcList, response_buf, len);
#endif
...
_u8 *OtaClient_GetNextUpdate(void *pvOtaClient, _i32 *size) {
    ...
    *size =
            pOtaClient->pOtaCheckUpdateResponse->rsrcList[pOtaClient->currUpdateIndex].size;
 ...
 */
_i32 OtaClient_ResourceMetadata(void *pvOtaClient, _u8 *resource_file_name,
        OtaFileMetadata_t **paramResourceMetadata) {
    OtaClient_t *pOtaClient = (OtaClient_t *) pvOtaClient;
    OtaOptServerInfo_t *pOtaServerInfo = pOtaClient->pOtaServerInfo;
    ...
    *paramResourceMetadata = pMetadata;
    ...
    status = OtaClient_ResourceNameConvert(pvOtaClient, resource_file_name,
            pMetadata);
    if (status < 0) {
...
#else
    http_build_request(send_buf, "POST ", pOtaServerInfo->server_domain,
            pOtaServerInfo->rest_rsrc_metadata, resource_file_name,
            pOtaServerInfo->rest_hdr, pOtaServerInfo->rest_hdr_val);
#endif
...
_i32 OtaClient_ResourceNameConvert(void *pvOtaClient, _u8 *resource_file_name,
    ...
    if (pOtaClient->pVendorStr) {
        pMetadata->p_file_name = &pMetadata->p_file_name[strlen(
                (const char *) pOtaClient->pVendorStr) + 1]; /* skip "/Vid00_Pid00_Ver00" */
    }
    ...
    /* extract file flags */
    file_flags = 0;
    if ((pMetadata->p_file_name[2] >= 'a')
            && (pMetadata->p_file_name[2] <= 'f'))
        file_flags |= (0xa + (pMetadata->p_file_name[2] - 'a')) << 4;
    if ((pMetadata->p_file_name[2] >= '0')
            && (pMetadata->p_file_name[2] <= '9'))
        file_flags |= (0x0 + (pMetadata->p_file_name[2] - '0')) << 4;
    if ((pMetadata->p_file_name[3] >= 'a')
            && (pMetadata->p_file_name[2] <= 'f'))
        file_flags |= 0xa + (pMetadata->p_file_name[3] - 'a');
    if ((pMetadata->p_file_name[3] >= '0')
            && (pMetadata->p_file_name[2] <= '9'))
        file_flags |= 0x0 + (pMetadata->p_file_name[3] - '0');
    /*file_flags = ((pMetadata->p_file_name[2] - '0') << 4) + (pMetadata->p_file_name[3] - '0'); */
    if (file_flags & 0x01)
        pMetadata->flags |= METADATA_FLAGS_SECURED;
    if (file_flags & 0x02)
        pMetadata->flags |= METADATA_FLAGS_SIGNATURE;
    if (file_flags & 0x04)
        pMetadata->flags |= METADATA_FLAGS_CERTIFICATE;
    if (file_flags & 0x08)
        pMetadata->flags |= METADATA_FLAGS_NOT_CONVERT_SYS_DIR;
    if (file_flags & 0x10)
        pMetadata->flags |= METADATA_FLAGS_NOT_SFLASH_STORAGE;
    if (file_flags & 0x20)
        pMetadata->flags |= METADATA_FLAGS_RESEREVED_1;
    if (file_flags & 0x40)
    ...
    if (pMetadata->flags & METADATA_FLAGS_SIGNATURE) {
        ...
        pMetadata->flags |= METADATA_FLAGS_SIGNATURE;
        strcpy((char *) pMetadata->signature_filename,
                (const char *) pMetadata->p_file_name);
        strcpy(
                (char *) &pMetadata->signature_filename[strlen(
                        (const char *) pMetadata->signature_filename) - 4],
                ".sig");
    }
    ...
    if (pMetadata->flags & METADATA_FLAGS_CERTIFICATE) {
        ...
        strcpy((char *) pMetadata->cert2_filename,
                (const char *) pMetadata->p_file_name);
        strcpy(
                (char *) &pMetadata->cert2_filename[strlen(
                        (const char *) pMetadata->signature_filename) - 4],
                ".cer");
