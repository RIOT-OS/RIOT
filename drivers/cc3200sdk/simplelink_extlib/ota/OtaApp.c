...
_i32 sl_extLib_OtaRun(void *pvOtaApp) {
    ...
    case OTA_STATE_IDLE:
        ...
        /* Should read in IDLE */
        _ReadStatFile((_u8 *) pOtaApp->pStatistics,
                sizeof(g_OtaApp_statistics));
    ...
    case OTA_STATE_CONNECT_SERVER:
        ...
        pOtaApp->numUpdates = OtaClient_UpdateCheck(pOtaClient,
                pOtaApp->vendorStr);
    ...
    case OTA_STATE_RESOURCE_LIST:
        ...
        pOtaApp->file_path = OtaClient_GetNextUpdate(pOtaClient,
                &pOtaApp->file_size);
        ...
        if (pOtaApp->file_path == NULL) /* end of updates */
        {
            ...
            _WriteStatFile((_u8 *) pOtaApp->pStatistics,
                    sizeof(g_OtaApp_statistics));
            LogClient_ConnectAndPrint(pOtaApp->pvLogClient,
                    pOtaApp->pOtaServerInfo, pOtaApp->vendorStr,
                    pOtaApp->pStatistics);
        ...
        status = OtaClient_ResourceMetadata(pOtaClient, pOtaApp->file_path,
                &pOtaApp->pResourceMetadata);
        if (status < 0) {
    ...
    case OTA_STATE_METADATA:
        ...
        status = CdnClient_ConnectByUrl(pOtaApp->pvCdnClient,
                pOtaApp->pResourceMetadata,
                pOtaApp->pOtaServerInfo->secured_connection);
        if (status < 0) {
    ...
    case OTA_STATE_CONNECT_CDN:
        CdnClient_SetFileInfo(pOtaApp->pvCdnClient,
                pOtaApp->pResourceMetadata->sflash_file_size,
    ...
    case OTA_STATE_DOWNLOADING:
        ...
        if (status < 0) {
            ...
            if (NULL
                    != strstr((const char *) pOtaApp->file_path,
                            "mcuimgA.bin")) {
...
_i32 _WriteStatFile(_u8 *buf, _i32 buf_len) {
    ...
    status = sl_FsOpen((_u8 *) OTASTAT_FILENAME,
            FS_MODE_OPEN_CREATE(buf_len, 0), &ulToken, &lFileHandle);
    if (status < 0) {
...
_i32 _ReadStatFile(_u8 *buf, _i32 buf_len) {
    ...
    status = sl_FsOpen((_u8 *) OTASTAT_FILENAME, FS_MODE_OPEN_READ, &ulToken,
            &lFileHandle);
    if (status < 0) {
