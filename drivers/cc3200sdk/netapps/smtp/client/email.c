...
long sl_NetAppEmailSet(unsigned char AppId, unsigned char Option,
    ...
    case NETAPP_PASSWORD:
        ...
        memcpy(g_sSourcePass.Password, pSourcePassword->Password, OptionLen);
...
signed long _smtpStateMachine(void) {
    ...
    while (exit == 0) {
        ...
        case smtpINIT:
            ...
            if (g_cBuf[0] == cSmtpCodeReady[0] && g_cBuf[1] == cSmtpCodeReady[1]
        ...
        case smtpHELO:
            lRetVal = _sendSMTPCommand(g_lSmtpSocket, "HELO localhost", NULL,
                    g_cBuf);
            if (lRetVal < 0) {
                g_uiSmtpStatus = smtpERROR;
        ...
        case smtpAUTH:
            ...
            lRetVal = _generateBase64Key((char*) g_sEmail.Username,
                    g_cBasekey1);
            if (lRetVal < 0) {
                g_uiSmtpStatus = smtpERROR;
            ...
            lRetVal = _generateBase64Key((char*) g_sSourcePass.Password,
                    g_cBasekey2);
            if (lRetVal < 0) {
                g_uiSmtpStatus = smtpERROR;
                g_uiEmailErrorInfo = SL_EMAIL_ERROR_AUTH;
            ...
            lRetVal = _sendSMTPCommand(g_lSmtpSocket, "AUTH LOGIN", NULL,
                    g_cBuf);
            if (lRetVal < 0) {
                g_uiSmtpStatus = smtpERROR;
                g_uiEmailErrorInfo = SL_EMAIL_ERROR_AUTH;
            ...
            if (g_cBuf[0] == cSmtpCodeAuthReply[0]
                ...
                lRetVal = _sendSMTPCommand(g_lSmtpSocket, g_cBasekey1, NULL,
                        g_cBuf);
                if (lRetVal < 0) {
                    g_uiSmtpStatus = smtpERROR;
                    g_uiEmailErrorInfo = SL_EMAIL_ERROR_AUTH;
        ...
        case smtpFROM:
            ...
            if (g_cBuf[0] == cSmtpCodeOkReply[0]
                ...
                memcpy(&g_cUsernameRfc[1 + strlen((char*) g_sEmail.Username)],
                        ">", 1);
                lRetVal = _sendSMTPCommand(g_lSmtpSocket,
                        (char *) cSmtpMailFrom, g_cUsernameRfc, g_cBuf);
                if (lRetVal < 0) {
                    g_uiSmtpStatus = smtpERROR;
                    g_uiEmailErrorInfo = SL_EMAIL_ERROR_FROM;
                    break;
        ...
        case smtpMESSAGE:
            ...
            if (g_cBuf[0] == cSmtpCodeOkReply[0]
                ...
                lRetVal = sl_Send(g_lSmtpSocket, cSmtpQuit, strlen(cSmtpQuit),
                        0);
                if (lRetVal < 0) {
                    g_uiSmtpStatus = smtpERROR;
                    g_uiEmailErrorInfo = SL_EMAIL_ERROR_MESSAGE;
