...
static struct topic_node *alloc_node_subtop(const c8 *topstr,
        c8 const **next_subtop) {
    u16 len = 0;
...
struct topic_node *subtop_nhbr_node_find(const struct topic_node *root_nh,
        const c8 *topstr, c8 const **next_subtop) {
...
struct topic_node *leaf_node_find(const c8 *topstr) {
    ...
    bool flag_nh;
    struct topic_node *node = lowest_node_find(topstr, &flag_nh, &next_subtop);
...
static struct topic_node *node_delete(struct topic_node *node) {
    ...
    if ((NULL == node->up_nhbr) && (NULL == node->up_hier))
        root_node = NULL;
...
static bool topbuf_add(struct topbuf_desc *buf_desc,
        const struct topic_node *node) {
    const c8 *next_subtop;
...
static struct topic_node *pub_hier_search(const c8 *topSUB,
        const struct topic_node *base, struct topbuf_desc *mk_pubtop) {
...
static struct topic_node *SUB_leaf_search(const c8 *topPUB,
    ...
    SUB_leaf_search_exit1: return (struct topic_node*) node; // Bad
}
...
static void try_node_delete(struct topic_node *node) {
    while (node) {
        ...
        if (is_node_retain(node) || is_node_willed(node) || enrolls_plugin(node)
                || node->cl_map[0] || node->cl_map[1] || node->cl_map[2])
            break;
...
static struct topic_node *SUB_node_create(const c8 *topSUB, u8 qid,
        void *usr_cl) {
...
static u8 proc_pub_leaf(struct topic_node *leaf,
        const struct utf8_string *topic, enum mqtt_qos qos, void *usr_cl) {
    ...
    if (is_node_retain(leaf)) {
        ...
        pub_msg_send(topic, leaf->my_data, leaf->my_dlen,
                MAKE_FH_FLAGS(false, qid, true), cl_bmap_get(usr_cl));
    }
...
static u8 proc_pub_hier_no_top(struct topic_node *base,
        struct topbuf_desc *mk_pubtop, enum mqtt_qos qos, void *usr_cl) {
    ...
    if (NULL == node) {
        ...
        struct utf8_string topic = { mk_pubtop->buffer, mk_pubtop->offset };
...
static u8 proc_pub_tree_no_top(struct topic_node *base,
        struct topbuf_desc *mk_pubtop, enum mqtt_qos qos, void *usr_cl) {
...
static u8 proc_pub_tree_SUBtop(const c8 *topSUB, struct topic_node *base,
    ...
    while (stack_ref < stack_idx) {
        ...
        ack = proc_pub_hier_SUBtop((c8*) stack->val1, stack->node, mk_pubtop,
                qos, usr_cl);
...
static u8 proc_sub_ml_wc_hier(const c8 *grandpa_topSUB, c8 *parent_subtop,
    ...
    if (('\0' != subtop[0]) && ('+' != subtop[0]) && ('/' != subtop[0])) {
        ...
        if (base)
            base = topbuf_cpy(mk_pubtop, subtop) ? base->dn_hier : NULL;
...
static u8 proc_sub_ml_wc_tree(c8 *grandpa_topSUB, c8 *parent_subtop,
        struct topic_node *base, enum mqtt_qos qos, void *usr_cl) {
    ...
    while (stack_ref < stack_idx) {
        ...
        mk_pubtop.offset = stack->val2;
        ack = proc_sub_ml_wc_hier((c8*) stack->val1, parent_subtop, stack->node,
...
static u8 ml_wc_nodes_create(c8 *parent_topSUB, u16 toplen, u8 qid,
        void *usr_cl) {
...
static u8 proc_sub_sl_or_no_wc(const c8 *topSUB, enum mqtt_qos qos,
        void *usr_cl) {
    struct topbuf_desc mk_pubtop = { work_buf, WBUF_LEN, 0 /* offset */};
    u8 min = QOS_VALUE(qos);
...
static void leaf_msg_send(const struct topic_node *leaf,
        const struct utf8_string *topic, const u8 *data_buf, u32 data_len,
        bool dup, enum mqtt_qos qos,
        bool retain) {
    u8 qid = 0, fh_fgs = 0;
    ...
    if (enrolls_plugin(leaf))
        plugin_publish(leaf->pg_map, topic, data_buf, data_len, dup, qos,
                retain);
...
static bool node_data_update(struct topic_node *node, bool drop_qid0,
        const u8 *data_buf, u32 data_len, u8 qid, bool retain) {
...
static bool _proc_pub_msg_rx(void *usr_cl, const struct utf8_string *topic,
        const u8 *data_buf, u32 data_len, u8 msg_id, enum mqtt_qos qos,
        bool retain) {
    i32 err = -1;
    ...
    proc_sub_tree_topPUB(work_buf, topic, data_buf, data_len, qos, false);
    ...
    if (retain) {
        ...
        if ((NULL == leaf)
                || (false
                        == node_data_update(leaf, true, data_buf, data_len,
...
bool proc_pub_msg_rx_locked(void *usr_cl, const struct utf8_string *topic,
    ...
    return _proc_pub_msg_rx(usr_cl, topic, data_buf, data_len, msg_id, qos,
            retain);
}
...
static u16 proc_connect_rx(void *ctx_cl, u8 conn_flags,
    ...
    u16 rv = plugin_connect(MQC_UTF8_CLIENTID(utf8_vec),
            MQC_UTF8_USERNAME(utf8_vec), MQC_UTF8_PASSWORD(utf8_vec), &app_cl);
    if (rv)
        goto proc_connect_rx_exit1;
    /* Admin did not permit connection */
    ...
    proc_connect_rx_exit1: return rv;
}

static u16 proc_connect_rx_locked(void *ctx_cl, u8 conn_flags,
        struct utf8_string * const *utf8_vec, void **usr_cl) {
...
#define MK_QOS_ENUM(qid) ((enum mqtt_qos)(qid & QID_VMASK))
    ...
    proc_sub_tree_topPUB((c8*) topic.buffer, &topic, leaf->my_data,
            leaf->my_dlen, MK_QOS_ENUM(node_qid_get(leaf)),
            is_node_retain(leaf));
...
static i32 proc_topic_enroll_locked(u8 pg_id, const struct utf8_string *topic,
        enum mqtt_qos qos) {
    i32 rv = 0;
...
static i32 proc_topic_cancel_locked(u8 pg_id, const struct utf8_string *topic) {
    i32 rv = 0;
...
static i32 proc_app_pub_send_locked(const struct utf8_string *topic,
        const u8 *data_buf, u32 data_len, enum mqtt_qos qos, bool retain) {
    bool rv;

    MUTEX_LOCKIN();
    /* Received from application, process topic for distribution */
    rv = _proc_pub_msg_rx(NULL, topic, data_buf, data_len, 0x00, qos, retain);
    MUTEX_UNLOCK();
...
i32 mqtt_server_init(const struct mqtt_server_lib_cfg *lib_cfg,
    ...
    struct mqtt_server_msg_cbs pkts_cbs = { proc_connect_rx_locked,
            proc_sub_msg_rx_locked, proc_un_sub_msg_locked,
            proc_pub_msg_rx_locked, proc_notify_ack_locked,
            on_cl_net_close_locked, on_connack_send_locked };

    struct plugin_core_msg_cbs core_cbs = { proc_topic_enroll_locked,
            proc_topic_cancel_locked, proc_app_pub_send_locked };

    util_params_set(lib_cfg->debug_printf, lib_cfg->mutex,
            lib_cfg->mutex_lockin, lib_cfg->mutex_unlock);

    USR_INFO("Version: Server LIB %s, Common LIB %s.\n\r", MQTT_SERVER_VERSTR,
            MQTT_COMMON_VERSTR);
