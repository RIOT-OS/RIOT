...
struct mqtt_packet *mqp_server_copy(const struct mqtt_packet *mqp) {
    struct mqtt_packet *cpy = server_mqp_alloc(mqp->msg_type, mqp->maxlen, 0);
    if (NULL != cpy) {
...
void util_params_set(i32 (*dbg_prn)(const c8 *fmt, ...), void *mutex,
        void (*mutex_lockin)(void*), void (*mutex_unlock)(void*)) {
