...
u16 plugin_connect(const struct utf8_string *clientId,
        const struct utf8_string *username, const struct utf8_string *password,
        void **app_usr) {
    u16 rv = CONNACK_RC_REQ_ACCEPT; /* Accept everything from MQTT network */

    *app_usr = NULL;
    if (acl_pg)
        rv = acl_pg->app_cbs.connect(clientId, username, password, app_usr);
...
i32 plugin_publish(u8 pg_map, const struct utf8_string *topic,
    ...
    for (i = 0; i < MAX_PLUGINS; i++) {
        if (PG_MAP_HAS_VALUE(pg_map, i)) {
            ...
            plugin->app_cbs.publish(topic, payload, pay_len, dup, qos, retain);
        }
    }
...
i32 mqtt_server_topic_disenroll(const void *app_hnd,
    ...
    return app_hnd ? msg_cbs->topic_cancel(PLUGIN(app_hnd)->index, topic) : -1;
}
...
static void *server_app_register(const struct mqtt_server_app_cbs *cbs,
    ...
    if (NULL != plugin) {
        ...
        memcpy(&plugin->app_cbs, cbs, sizeof(struct mqtt_server_app_cbs));
