...
static i32 loopb_trigger(void) {
    ...
    if ((-1 != loopb_net) && (false == pending_trigs)) {
        rv = net_ops->send_dest(loopb_net, LOOP_DATA, LOOP_DLEN, loopback_port,
                ip_addr, 4);
        if (0 == rv)
            pending_trigs = true;
    }
...
static i32 sub_ack_send(struct client_ctx *cl_ctx, u8 *buf, u8 pay_ofs,
    ...
    ref -= mqp_buf_tail_wr_remlen(ref - MAX_REMLEN_BYTES, pay_len + buf - ref);

    ref -= 1;
    *ref = MAKE_FH_BYTE1(MQTT_SUBACK, MAKE_FH_FLAGS(false, MQTT_QOS0, false));
...
bool proc_sub_msg_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    if (usr_cbs->sub_msg_rx(cl_ctx->usr, qos_topics, n_topics, msg_id,
...
bool proc_pub_msg_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    rv = usr_cbs->pub_msg_rx(cl_ctx->usr, &topic, MQP_PUB_PAY_BUF(mqp_raw),
            MQP_PUB_PAY_LEN(mqp_raw), msg_id, BOOL_DUP(B), qos, BOOL_RETAIN(B));
    if (false == rv)
        return rv;
...
bool proc_ack_msg_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    if ((false == mqp_proc_msg_id_ack_rx(mqp_raw, false))
            || (false
                    == usr_cbs->ack_notify(cl_ctx->usr, mqp_raw->msg_type,
                            mqp_raw->msg_id)))
        return false;
...
static u16 proc_connect_vh_rx(struct mqtt_packet *mqp_raw, u8 *conn_flags,
        u16 *ka_secs) {
    struct utf8_string utf8;
...
u16 proc_connect_pl_rx(const u8 *buf, const u8 *end, u8 conn_flags,
        struct utf8_string *free_utf8s, struct utf8_string **used_refs) {
    struct utf8_string *utf8;
...
bool proc_connect_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    ack_vh16 = proc_connect_pl_rx(buf, end, conn_flags, free_utf8s, used_refs);
    if (ack_vh16)
        goto proc_connect_rx_exit1;

    clean_session = (conn_flags & CLEAN_START_VAL) ? true : false;
    ack_vh16 = (!used_refs[0]->length && !clean_session) ?
    CONNACK_RC_CLI_REJECT :
                                                           0; /* Validate 0 byte Client ID */
    if (ack_vh16)
        goto proc_connect_rx_exit1;
...
static bool accept_ctx(i32 net, u32 wait_secs) {
    ...
    cl_ctx->net = net_ops->accept(net, cl_ctx->remote_ip, &cl_ctx->ip_length);
    if (-1 == cl_ctx->net) {
        cl_ctx_free(cl_ctx);
...
i32 mqtt_server_run(u32 wait_secs)   // TBD break into two functions
{
    ...
    do {
        ...
        *r_hvec++ = listen_net;
        ...
        n_hnds = net_ops->io_mon(recv_hvec, &send_hvec, &rsvd_hvec, secs2wait);
        if (n_hnds < 0)
            return MQP_ERR_LIBQUIT;
...
i32 mqtt_server_register_net_svc(const struct device_net_services *net) {
    if (net && net->io_mon && net->close && net->send && net->recv && net->time
            && net->listen) {
        net_ops = net;
        return 0;
    }
...
i32 mqtt_server_lib_init(const struct mqtt_server_lib_cfg *lib_cfg,
    ...
    if ((NULL == lib_cfg) || (0 == lib_cfg->listener_port)
