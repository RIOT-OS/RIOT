...
bool cl_mgmt_qos2_pub_rx_update(void *usr_cl, u16 msg_id) {
    ...
    return cl_usr
            && (qos2_pub_rx_is_done(cl_usr, msg_id)
...
static inline i32 _pub_dispatch(struct client_usr *cl_usr,
...
i32 cl_pub_msg_send(void *usr_cl, const struct utf8_string *topic,
        const u8 *data_buf, u32 data_len, enum mqtt_qos qos, bool retain) {
    struct mqtt_packet *mqp = NULL;

    if ((NULL == topic) || ((data_len > 0) && (NULL == data_buf))
            || (NULL == usr_cl))
...
static void wl_remove(struct mqtt_ack_wlist *list, struct mqtt_packet *prev,
...
static struct mqtt_packet *wl_mqp_unmap_find(struct mqtt_ack_wlist *wl,
        struct client_usr *cl_usr, u16 msg_id, struct mqtt_packet **prev) {
    struct mqtt_packet *mqp = NULL;
...
bool cl_can_session_delete(void *usr_cl) {
    ...
    return cl_usr ?
            (has_clean_session(cl_usr) || !has_session_data(cl_usr)) : false;
}
...
static bool proc_pub_rel_rx(struct client_usr *cl_usr, u16 msg_id) {
    mqtt_vh_msg_send(cl_usr->ctx, MQTT_PUBCOMP, MQTT_QOS0, true, msg_id);
...
static bool proc_pub_rec_rx(struct client_usr *cl_usr, u16 msg_id) {
    ...
    if (mqp && ack2_msg_id_logup(cl_usr, msg_id)) {
        ...
        mqtt_vh_msg_send(cl_usr->ctx, MQTT_PUBREL, MQTT_QOS1, true, msg_id);
...
u16 cl_connect_rx(void *ctx_cl, bool clean_session, c8 *client_id, void *app_cl,
        void *will, void **usr_cl) {
