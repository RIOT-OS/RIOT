...
static const u8 *tx_part_buf_p(struct tx_part_pkt *tx_part) {
    ...
    return tx_mqp ? tx_part->buffer + offset : tx_part->vh_msg + offset;
}
...
enum module_state {
    ...
    WAIT_INIT_STATE, INIT_DONE_STATE = 0x01,
};
...
static i32 loopb_trigger(void) {
    ...
    return (-1 != loopb_net) ?
            net_ops->send_dest(loopb_net, LOOP_DATA, LOOP_DLEN, loopb_portid,
                    ip_addr, 4) :
            MQP_ERR_LIBQUIT;
}
...
static bool ack2_msg_id_unlog(struct client_ctx *cl_ctx, u16 msg_id) {
    ...
    if (qos2_pub_cq_unlog(&client->qos2_tx_cq, msg_id)) {
        ...
        if (ctx_cbs->ack_notify)
            ctx_cbs->ack_notify(client->app, MQTT_PUBCOMP, msg_id, NULL, 0);
        return true;
    }
...
static bool awaits_pkts(struct client_ctx *cl_ctx) {
    ...
    return client->qos_ack1_wl.head || qos2_pub_cq_count(&client->qos2_rx_cq)
...
static i32 wr_connect_pl(struct client_ctx *cl_ctx, u8 *buf, u32 fsz,
        u8 *conn_opts) {
...
static inline u16 get_connect_vh_len(struct client_ctx *cl_ctx) {
    return (IS_PROTO_VER31(cl_ctx) ? sizeof(mqtt310) : sizeof(mqtt311)) + 3;
}

static i32 wr_connect_vh(struct client_ctx *cl_ctx, u8 *buf, u16 ka_secs,
        u8 conn_opts) {
    u8 *ref = buf;
...
static i32 net_connect(struct client_ctx *cl_ctx) {
    ...
    cl_ctx->net = net_ops->open(client->nwconn_opts | DEV_NETCONN_OPT_TCP,
            client->server_addr, client->port_number, &client->nw_security);

    return (-1 == cl_ctx->net) ? MQP_ERR_NETWORK : 0;
}

static i32 cl_ctx_conn_state_try_locked(struct client_ctx *cl_ctx,
        const u8 *buf, u32 len, u16 ka_secs, bool clean_session,
        struct mqtt_packet *tx_mqp) {
    i32 rv;
    ...
    rv = MQP_ERR_BADCALL;
    if (false
            == ((INIT_DONE_STATE != cl_lib_state) || (AWAITS_CONNACK(cl_ctx))))
...
static i32 connect_msg_send(struct client_ctx *cl_ctx, bool clean_session,
        u16 ka_secs) {
    ...
    if (fsz < vhl)
        goto connect_msg_send_exit1;
    /* No space for VAR HDR */
    ...
    return cl_ctx_conn_state_try_locked(cl_ctx, ref, buf - ref, ka_secs,
            clean_session, mqp);
    connect_msg_send_exit1: if (mqp)
        mqp_free_locked(mqp);
...
i32 mqtt_connect_msg_send(void *ctx, bool clean_session, u16 ka_secs) {
    return ctx ? connect_msg_send(CL_CTX(ctx), clean_session, ka_secs) : -1;
}
...
static i32 msg_dispatch_no_free(struct client_ctx *cl_ctx,
        struct mqtt_packet *mqp, enum mqtt_qos qos, bool retain) {
...
static i32 utf8_array_send(struct client_ctx *cl_ctx,
        ...
        const struct utf8_string *unsub_vec, u32 n_elem) {
    ...
    for (i = 0; i < n_elem; i++) {
        ...
        rv = buf_utf8_wr_try(buf, end - buf, topic,
                subsc_vec ? (u8) subsc_vec[i].qosreq : QFL_VALUE);
        if (rv < 0) {
            mqp_free(mqp);
            return rv;
        }
...
i32 mqtt_sub_msg_send(void *ctx, const struct utf8_strqos *qos_topics,
        u32 count) {
...
bool _proc_ack_msg_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    if (ctx_cbs->ack_notify)
        ctx_cbs->ack_notify(client->app, mqp_raw->msg_type, msg_id,
                len ? MQP_PAYLOAD_BUF(mqp_raw) : NULL, len);
    return true;
}
...
bool proc_pub_msg_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    if (ctx_cbs->publish_rx) {
        ...
        if (ctx_cbs->publish_rx(CLIENT(cl_ctx)->app, BOOL_DUP(B), qos,
...
bool proc_connack_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    proc_connack_rx_exit1: if (ctx_cbs->ack_notify)
        ctx_cbs->ack_notify(CLIENT(cl_ctx)->app, mqp_raw->msg_type, 0, buf, 2);
...
bool proc_pingrsp_rx(struct client_ctx *cl_ctx, struct mqtt_packet *mqp_raw) {
    ...
    if (AWAITS_PINGRSP(cl_ctx)) {
        ...
        if (ctx_cbs->ack_notify)
            ctx_cbs->ack_notify(CLIENT(cl_ctx)->app, mqp_raw->msg_type, 0, NULL,
                    0);
        return true;
    }
...
static i32 single_ctx_ka_sequence(struct client_ctx *cl_ctx, u32 wait_secs) {
    ...
    if (is_connected(cl_ctx)) {
        ...
        if (AWAITS_KA_PING(cl_ctx))
            goto single_ctx_ka_sequence_exit1;
        /* No PINGRESP  */
...
static u32 single_ctx_adj_wait_secs_get(struct client_ctx *cl_ctx,
        u32 wait_secs) {
...
static i32 proc_ctx_data_recv(struct client_ctx *cl_ctx,
        struct mqtt_packet *mqp, u32 wait_secs, void **app) {
    i32 rv = MQP_ERR_NOTCONN;
    i32 net = cl_ctx->net;
    ...
    *app = cl_ctx->usr;
    ...
    if (((rv < 0) && (rv != MQP_ERR_TIMEOUT))
            || ((MQTT_CONNACK == mqp->msg_type) && MQP_CONNACK_RC(mqp)))
        do_net_close_rx(cl_ctx, rv);
...
static i32 cl_ctx_recv(struct client_ctx *cl_ctx, struct mqtt_packet *mqp,
    ...
    do {
        ...
        if (rv > 0)
            rv = mqp_setup_proc_ctx_data_recv(cl_ctx, mqp, wait_secs, &app);
...
i32 mqtt_client_ctx_await_msg(void *ctx, u8 msg_type, struct mqtt_packet *mqp,
    ...
    } while ((rv > 0) && (0 != msg_type) && (msg_type != mqp->msg_type));
...
static i32 group_ctxs_rx_prep(u32 wait_secs, void **app) {
    ...
    n_hnds = net_ops->io_mon(recv_hvec, &send_hvec, &rsvd_hvec, wait_secs);
    if (0 == n_hnds)
...
i32 mqtt_client_ctx_will_register(void *ctx, const struct utf8_string *will_top,
        const struct utf8_string *will_msg, enum mqtt_qos will_qos, bool retain) {
    u8 B = 0;
...
i32 mqtt_client_net_svc_register(const struct device_net_services *net) {
    if (net && net->open && net->send && net->recv && net->send_dest
            && net->recv_from && net->close && net->io_mon && net->time) {
        net_ops = net;
        return 0;
    }
...
const struct mqtt_client_ctx_cfg *ctx_cfg,
        const struct mqtt_client_ctx_cbs *ctx_cbs, void *app) {
    struct client_desc *client = CLIENT(cl_ctx);
    ...
    if (NULL != ctx_cfg->nw_security)
        buf_wr_nbytes((u8*) &client->nw_security, (u8*) ctx_cfg->nw_security,
...
i32 mqtt_client_ctx_create(const struct mqtt_client_ctx_cfg *ctx_cfg,
        const struct mqtt_client_ctx_cbs *ctx_cbs, void *app, void **ctx) {
    struct client_ctx *cl_ctx = NULL;

    if ((NULL == ctx_cfg) || (NULL == ctx_cfg->server_addr)
...
i32 mqtt_client_ctx_delete(void *ctx) {
    ...
    MUTEX_LOCKIN();
    if ((NULL == cl_ctx) || (-1 != cl_ctx->net) || (awaits_pkts(cl_ctx)))
