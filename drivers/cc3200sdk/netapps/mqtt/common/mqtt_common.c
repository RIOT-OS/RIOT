...
bool mqp_proc_msg_id_ack_rx(struct mqtt_packet *mqp_raw, bool has_pl) {
    if ((false == proc_vh_msg_id_rx(mqp_raw)) || (has_pl ^ (!!mqp_raw->pl_len)))
        return false;
...
i32 mqp_recv(i32 net, const struct device_net_services *net_ops,
        struct mqtt_packet *mqp, u32 wait_secs, bool *timed_out, void *ctx) {
    u8 *buf = MQP_FHEADER_BUF(mqp), *ref = buf, fh_len = mqp->fh_len;
    u32 pl_len = mqp->pl_len, remlen = 0;
    i32 rv;

    while (2 > fh_len) {
        RET_IF_ERR_IN_NET_RECV(net, buf + fh_len, 1, wait_secs, timed_out, ctx);
    ...
    while (pl_len < remlen) {
        RET_IF_ERR_IN_NET_RECV(net, buf + pl_len, remlen - pl_len, wait_secs,
                timed_out, ctx);
