...
struct device_net_services net = { comm_open, tcp_send, tcp_recv, send_dest,
        recv_from, comm_close, tcp_listen, tcp_accept, tcp_select, rtc_secs };
...
static void process_notify_ack_cb(void *app, u8 msg_type, u16 msg_id, u8 *buf,
    ...
    case MQTT_SUBACK:
        if (true == client_ctx->blocking_send) {
            ...
            client_ctx->app_cbs.sl_ExtLib_MqttEvent(client_ctx->app_hndl,
...
bool retain, struct mqtt_packet *mqp) {
    ...
    client_ctx->app_cbs.sl_ExtLib_MqttRecv(client_ctx->app_hndl,
            ...
            MQP_PUB_PAY_BUF(mqp), MQP_PUB_PAY_LEN(mqp), dup, MQP_PUB_QOS(mqp),
            retain);
    return true;
}
    ...
    do {
        if (false == g_multi_srvr_conn) {
            ...
            while (1) {
                ...
                retval = mqtt_client_ctx_run(sl_cli_ctx[0].cli_hndl,
                        g_wait_secs);
                if ((retval < 0) && (retval != MQP_ERR_TIMEOUT)) {
                    break;
                }
            }
        } else {
...
_const SlMqttClientCbs_t *msg_cbs, void *app_hndl) {
    ...
    client_ctx_ptr->app_cbs.sl_ExtLib_MqttRecv = msg_cbs->sl_ExtLib_MqttRecv;
    client_ctx_ptr->app_cbs.sl_ExtLib_MqttEvent = msg_cbs->sl_ExtLib_MqttEvent;
    ...
    if (ctx_cfg->server_info.netconn_flags & SL_MQTT_NETCONN_SEC) {
        ...
        lib_ctx_cfg.nw_security->n_file = ctx_cfg->server_info.n_files;
    ...
    retval = mqtt_client_ctx_create(&lib_ctx_cfg, &lib_cli_cbs, client_ctx_ptr,
...
void mutex_lockup(void *mqtt_lib_lock) {
    sl_LockObjLock((_SlLockObj_t*)mqtt_lib_lock, OSI_WAIT_FOREVER);
}

void mqtt_mutex_unlock(void *mqtt_lib_lock) {
...
_i32 sl_ExtLib_MqttClientInit(_const SlMqttClientLibCfg_t *cfg) {
    ...
    osi_TaskCreate(VMqttRecvTask, (_const signed char *) "MQTTRecv",
    OSI_STACK_SIZE,
    NULL, cfg->rx_tsk_priority, &g_rx_task_hndl);
...
//*****************************************************************************
_i32 sl_ExtLib_MqttClientSet(void *app, _i32 param, _const void *value,
        _u32 len) {
    struct sl_client_ctx *client_ctx = (struct sl_client_ctx *) app;
    switch (param) {
...
_i32 sl_ExtLib_MqttClientConnect(void *cli_ctx, bool clean,
    ...
    ret = mqtt_client_ctx_info_register(client_ctx->cli_hndl, clientid, usrname,
            usrpasswd);
    if (ret < 0) {
    ...
    if (NULL != client_ctx->mqtt_will.will_topic) {
        ...
        ret = mqtt_client_ctx_will_register(client_ctx->cli_hndl, &will_topic,
                &will_msg, qos[client_ctx->mqtt_will.will_qos],
    ...
    mqtt_connect_exit1: client_ctx->awaited_ack = 0;
    client_ctx->conn_ack = 0;
    return (ret < 0) ? -1 : ret;
}
...
//*****************************************************************************
_i32 sl_ExtLib_MqttClientSub(void *cli_ctx, char* _const *topics,
...
//*****************************************************************************
_i32 sl_ExtLib_MqttClientUnsub(void *cli_ctx, char* _const *topics, _i32 count)
{
...
//*****************************************************************************
_i32 sl_ExtLib_MqttClientSend(void *cli_ctx, _const char *topic,
_const void *data, _i32 len, char qos_level, bool retain) {
    _i32 ret = -1;
    ...
    /*publish the message*/
    ret = mqtt_client_pub_msg_send(client_ctx->cli_hndl, &topic_utf8, data, len,
