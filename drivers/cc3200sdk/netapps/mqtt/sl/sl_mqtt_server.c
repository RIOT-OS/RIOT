...
struct mqtt_server_lib_cfg server_cfg = { 0, 0, &MutexLockObj, mutex_lock,
        mutex_unlock,
        NULL /*Debug print*/
};
...
struct device_net_services net_ops = { comm_open, tcp_send, tcp_recv, send_dest,
...
_const struct utf8_string *password, void **app_usr) {
    return (cbs_obj.sl_ExtLib_MqttConn(
            (_const char*) MQ_CONN_UTF8_BUF(clientId),
            MQ_CONN_UTF8_LEN(clientId),
            (_const char*) MQ_CONN_UTF8_BUF(username),
            MQ_CONN_UTF8_LEN(username),
            (_const char*) MQ_CONN_UTF8_BUF(password),
...
struct mqtt_server_app_cbs server_appcallbacks = { sl_server_connect_cb,
        sl_server_publish_cb, sl_server_disconn_cb };
...
_const SlMqttServerCbs_t *app_cbs) {
    ...
    app_hndl.plugin_hndl = mqtt_server_app_register(&server_appcallbacks,
            "temp");
