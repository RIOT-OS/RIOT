...
int sl_NetAppXmppConnect() {
    ...
    Status = sl_SetSockOpt(g_SockID, SL_SOL_SOCKET, SO_SECMETHOD, &method,
            sizeof(method));
    if (Status < 0) {
        return -1;
    ...
    Status = sl_SetSockOpt(g_SockID, SL_SOL_SOCKET, SO_SECURE_MASK, &cipher,
            sizeof(cipher));
    if (Status < 0) {
        return -1;
...
int _SlValidateServerInfo(void) {
    ...
    if (strncmp(pServerStr + 6, (char*) g_XmppDomain.DomainName,
            g_XmppDomain.Length)) {
        return -1;
    }
...
int _SlValidateRoster(void) {
    ...
    if (!strstr(g_RecvBuf, "jabber:iq:roster")
            || !strstr(g_RecvBuf, "roster_1")) {
        return -1;
    }
...
signed short sl_NetAppXmppRecv(unsigned char* pRemoteJid, unsigned short Jidlen,
    ...
    Status = setsockopt(g_SockID, SOL_SOCKET, SO_NONBLOCKING, &nonBlocking,
            sizeof(nonBlocking));
...
long GenerateBase64Key(void) {
    ...
    lRetVal = ConvertToBase64(g_MyBaseKey, (void *) InputStr,
            g_XmppUserName.Length * 2 + 3 + g_XmppDomain.Length
                    + g_XmppPassword.Length);
...
//*****************************************************************************
            memcpy(ccPtr,
                    "<iq id='JAJSBind' type='set'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'><resource>",
                    86);
            ccPtr += 86;
