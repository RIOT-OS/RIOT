...
int OpenTCPServerSocket(unsigned int uiPortNum) {
    ...
    iRetVal = sl_SetSockOpt(iSockDesc, SOL_SOCKET, SL_SO_NONBLOCKING,
    ...
    if ( bind(iSockDesc, (struct sockaddr*) &sServerAddress,
...
int CreateTCPClientSocket(int iSockDesc) {
    ...
    if (sock >= 0) {
        ...
        if (sl_SetSockOpt(sock, SOL_SOCKET, SL_SO_NONBLOCKING, &enableOption,
                sizeof(enableOption)) < 0) {
        ...
        if ((sl_SetSockOpt(sock, SOL_SOCKET, SL_SO_RCVTIMEO, &timeVal,
                sizeof(timeVal))) < 0) {
            CloseTCPClientSocket(sock);
            return -1;
