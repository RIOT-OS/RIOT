...
static uint8_t write_byte(uint8_t value)
{
    ...
    clk_signal();
    printf("write byte value=%u ack=%u \n",value_temp,ack);
    return ack;
}
/*---------------------------------------------------------------------------*/
static uint8_t read_byte(uint8_t ack)
...
void sht11_init(void)
{
    ...
    transmission_start();
    uint8_t error = !SHT11_DATA;
    error += !write_byte(0x70);
   // error += !write_byte(0x5C);
    //error += !write_byte(0x24);
    error += !write_byte(0xEF);
    error += !write_byte(0xC8);

    xtimer_sleep(1);
    transmission_start();

    //error += !write_byte(0x71);

    uint8_t test1 = read_byte(1);
    uint8_t test2 = read_byte(1);
    uint8_t test3 = read_byte(1);
    uint8_t test4 = read_byte(1);
    uint8_t test5 = read_byte(1);
    uint8_t test6 = read_byte(1);

    printf("Init should be 0 : %u \n",(unsigned int) error);
    printf("Test1 : %u \n",(unsigned int) test1);
    printf("Test2 : %u \n",(unsigned int) test2);
    printf("Test3 : %u \n",(unsigned int) test3);
    printf("Test4 : %u \n",(unsigned int) test4);
