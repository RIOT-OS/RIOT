 ...
 * enabled.
 *
 * @pre     The caller is not calling `timer_init()` twice on the same timer
 *          while it is active. A timer must either not have been initialized
 *          before, or it must be stopped with @ref timer_stop before calling
 *          `timer_init()` again.
 *
