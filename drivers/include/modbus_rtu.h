 ...
 * request if valid.
 *
 * Read requests must ensure that @p message->data contains the requested data
 * so that the implementation can return it in the response. To support
 * zero-copy, the callback can also write directly to @p message->data, but it
 * is also valid set this to a different application buffer of sufficient size.
 * In that case, @p message->data_size must be set as well.
 *
 ...
 * the appropriate helpers in @ref driver_modbus.
 *
