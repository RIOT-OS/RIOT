...
/**
 * @defgroup    drivers_at86rf212b
 * @ingroup     drivers
 * @brief       Device driver for the Atmel AT86RF212B radio
 * @{
 *
 * @file
 * @brief       Interface definition for the AT86RF212B device driver
 *
...
#include "at86rf212b/at86rf212b_settings.h"
...
/**
 * @brief Maximum length of a frame on at86rf212b
 */
#define AT86RF212B_MAX_PKT_LENGTH    (127)
 ...
 */
#define AT86RF212B_MAX_DATA_LENGTH   (118)

/**
 * @brief Broadcast address
 */
#define AT86RF212B_BROADCAST_ADDRESS (0xFFFF)

/**
 * @brief at86rf212b's lowest supported channel
 */
#define AT86RF212B_MIN_CHANNEL       (0)

/**
 * @brief at86rf212b's highest supported channel
 */
#define AT86RF212B_MAX_CHANNEL       (10)

/**
 *  @brief Structure to represent a at86rf212b packet.
 */
...
extern netdev_t at86rf212b_netdev;   /**< netdev representation of this driver */
...
/**
 * @brief To keep state inside of at86rf212b driver
...
/**
 * @brief Initialize the at86rf212b transceiver
 */
int at86rf212b_initialize(netdev_t *dev);

#ifdef MODULE_TRANSCEIVER
/**
 * @brief Init the at86rf212b for use with RIOT's transceiver module.
 *
...
/**
 * @brief Turn at86rf212b off.
 */
void at86rf212b_off(void);

/**
 * @brief Indicate if the at86rf212b is on.
 *
...
/**
 * @brief Switches the at86rf212b into receive mode.
 */
...
/**
 * @brief Indicate if the at86rf212b is in monitor (promiscuous) mode.
 *
...
/**
 * @brief Set the channel of the at86rf212b.
 *
 * @param[in] chan The desired channel, valid channels are from 0 to 10.
 *
 * @return The tuned channel after calling, or -1 on error.
 */
int at86rf212b_set_channel(unsigned int chan);

/**
 * @brief Get the channel of the at86rf212b.
 *
...
/**
 * @brief Sets the short address of the at86rf212b.
 *
 * @param[in] addr The desired address.
 *
 * @return The set address after calling.
 */
uint16_t at86rf212b_set_address(uint16_t addr);

/**
 * @brief Gets the current short address of the at86rf212b.
 *
 * @return The current short address.
 */
uint16_t at86rf212b_get_address(void);

/**
 * @brief Sets the IEEE long address of the at86rf212b.
 *
 * @param[in] addr The desired address.
 *
 * @return The set address after calling.
 */
uint64_t at86rf212b_set_address_long(uint64_t addr);

/**
 * @brief Gets the current IEEE long address of the at86rf212b.
 *
...
/**
 * @brief Sets the pan ID of the at86rf212b.
 *
...
/**
 * @brief Sets the output (TX) power of the at86rf212b.
 *
 ...
 */
int at86rf212b_set_tx_power(int pow);

/**
 * @brief Gets the current output (TX) power of the at86rf212b.
 *
 * @return The current TX (output) power.
 */
int at86rf212b_get_tx_power(void);
...
/**
 * @brief Prepare the at86rf212b TX buffer to send with the given packet.
 *
 * @param[in] dev  The network device to operate on. (Currently not used)
 ...
 */
netdev_802154_tx_status_t at86rf212b_load_tx_buf(netdev_t *dev,
...
/**
 * @brief Transmit the data loaded into the at86rf212b TX buffer.
 *
 * @param[in] dev The network device to operate on. (Currently not used)
 *
 * @return @ref netdev_802154_tx_status_t
 */
netdev_802154_tx_status_t at86rf212b_transmit_tx_buf(netdev_t *dev);

/**
 * @brief Send function, sends a at86rf212b_packet_t over the air.
 *
 ...
 *
 */
int16_t at86rf212b_send(at86rf212b_packet_t *packet);

/**
 * RX Packet Buffer, read from the transceiver, filled by the at86rf212b_rx_handler.
 */
extern at86rf212b_packet_t at86rf212b_rx_buffer[AT86RF212B_RX_BUF_SIZE];

/**
 * Get at86rf212b's status byte
 */
uint8_t at86rf212b_get_status(void);

/**
 * Get at86rf212b's TRAC status byte
 */
uint8_t at86rf212b_get_trac_status(void);

/**
 * at86rf212b low-level radio driver definition.
 */
extern const netdev_802154_driver_t at86rf212b_driver;
