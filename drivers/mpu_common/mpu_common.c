 ...
 *
 * @file
 * @brief       Device driver implementation for the MPU Motion Sensors
 *
...
static const mpu_status_t DEFAULT_STATUS = {
    .accel_pwr = MPU_SENSOR_PWR_ON,
    .gyro_pwr = MPU_SENSOR_PWR_ON,
    .gyro_fsr = MPU_GYRO_FSR_250DPS,
    .accel_fsr = MPU_ACCEL_FSR_16G,
    .sample_rate = 0,
#ifdef MPU_WITH_COMPASS
    .compass_pwr = MPU_SENSOR_PWR_ON,
...
#ifdef MPU_WITH_COMPASS
static int compass_init(mpu_dev_t *dev);
static void conf_bypass(mpu_dev_t *dev, uint8_t bypass_enable);
#endif
static void conf_lpf(mpu_dev_t *dev, uint16_t rate);
...
#endif
{
    ...
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_1_REG, MPU_PWR_RESET);
    xtimer_usleep(MPU_RESET_SLEEP_US);
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_1_REG, MPU_PWR_WAKEUP);

    /* Release the bus, it is acquired again inside each function */
    i2c_release(dev->i2c_dev);

    /* Set default full scale ranges and sample rate */
    mpu_set_gyro_fsr(dev, MPU_GYRO_FSR_2000DPS);
    mpu_set_accel_fsr(dev, MPU_ACCEL_FSR_2G);
    mpu_set_sample_rate(dev, MPU_DEFAULT_SAMPLE_RATE);

    /* Disable interrupt generation */
    i2c_acquire(dev->i2c_dev);
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_INT_ENABLE_REG, REG_RESET);
    ...
    /* Release the bus, it is acquired again inside each function */
    i2c_release(dev->i2c_dev);
    mpu_set_compass_sample_rate(dev, 10);
#else
    /* Release the bus, it is acquired again inside each function */
    i2c_release(dev->i2c_dev);
#endif
    /* Enable all sensors */
    i2c_acquire(dev->i2c_dev);
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_1_REG, MPU_PWR_PLL);
    i2c_read_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_2_REG, &temp);
    temp &= ~(MPU_PWR_ACCEL | MPU_PWR_GYRO);
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_2_REG, temp);
    i2c_release(dev->i2c_dev);
    xtimer_usleep(MPU_PWR_CHANGE_SLEEP_US);
...
int mpu_set_accel_power(mpu_dev_t *dev, mpu_pwr_t pwr_conf)
{
    ...
    i2c_read_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_2_REG, &pwr_2_setting);
    /* Prepare power register settings */
    if (pwr_conf == MPU_SENSOR_PWR_ON) {
        pwr_1_setting = MPU_PWR_WAKEUP;
        pwr_2_setting &= ~(MPU_PWR_ACCEL);
    }
    else {
...
#ifdef MPU_WITH_COMPASS
    if ((dev->conf.gyro_pwr == MPU_SENSOR_PWR_OFF) &&
        (dev->conf.compass_pwr == MPU_SENSOR_PWR_OFF)) {
#else
    if (dev->conf.gyro_pwr == MPU_SENSOR_PWR_OFF) {
#endif
        i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_1_REG, pwr_1_setting);
    }
    /* Enable/disable accelerometer standby in power management 2 register */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_2_REG, pwr_2_setting);
...
int mpu_set_gyro_power(mpu_dev_t *dev, mpu_pwr_t pwr_conf)
{
    ...
    if (pwr_conf == MPU_SENSOR_PWR_ON) {
        /* Set clock to pll */
        i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_1_REG, MPU_PWR_PLL);
        pwr_2_setting &= ~(MPU_PWR_GYRO);
    }
    else {
        /* Configure power management 1 register */
#ifdef MPU_WITH_COMPASS
        if ((dev->conf.accel_pwr == MPU_SENSOR_PWR_OFF) &&
            (dev->conf.compass_pwr == MPU_SENSOR_PWR_OFF)) {
#else
        if (dev->conf.accel_pwr == MPU_SENSOR_PWR_OFF) {
#endif
            ...
            i2c_write_reg(dev->i2c_dev, dev->hw_addr,
                    MPU_PWR_MGMT_1_REG, BIT_PWR_MGMT1_SLEEP);
        }
        else {
            /* Reset clock to internal oscillator */
            i2c_write_reg(dev->i2c_dev, dev->hw_addr,
                    MPU_PWR_MGMT_1_REG, MPU_PWR_WAKEUP);
        }
        pwr_2_setting |= MPU_PWR_GYRO;
    }
...
int mpu_set_compass_power(mpu_dev_t *dev, mpu_pwr_t pwr_conf)
{
    ...
    i2c_read_reg(dev->i2c_dev, dev->hw_addr, MPU_USER_CTRL_REG, &usr_ctrl_setting);
    /* Prepare power register settings */
    if (pwr_conf == MPU_SENSOR_PWR_ON) {
        pwr_1_setting = MPU_PWR_WAKEUP;
        s1_do_setting = MPU_COMP_SINGLE_MEASURE;
        usr_ctrl_setting |= BIT_I2C_MST_EN;
    }
    else {
        pwr_1_setting = BIT_PWR_MGMT1_SLEEP;
        s1_do_setting = MPU_COMP_POWER_DOWN;
        usr_ctrl_setting &= ~(BIT_I2C_MST_EN);
    }
    /* Configure power management 1 register if needed */
    if ((dev->conf.gyro_pwr == MPU_SENSOR_PWR_OFF)
            && (dev->conf.accel_pwr == MPU_SENSOR_PWR_OFF)) {
        i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_PWR_MGMT_1_REG, pwr_1_setting);
    }
    /* Configure mode writing by slave line 1 */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE1_DATA_OUT_REG, s1_do_setting);
    /* Enable/disable I2C master mode */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_USER_CTRL_REG, usr_ctrl_setting);
...
int mpu_read_gyro(mpu_dev_t *dev, mpu_results_t *output)
{
    ...
    /* Read raw data */
    i2c_read_regs(dev->i2c_dev, dev->hw_addr, MPU_GYRO_START_REG, data, 6);
    /* Release the bus */
    i2c_release(dev->i2c_dev);
...
int mpu_read_accel(mpu_dev_t *dev, mpu_results_t *output)
{
    ...
    switch (dev->conf.accel_fsr) {
        case MPU_ACCEL_FSR_2G:
            fsr = 2000.0;
            break;
        case MPU_ACCEL_FSR_4G:
            fsr = 4000.0;
            break;
        case MPU_ACCEL_FSR_8G:
            fsr = 8000.0;
            break;
        case MPU_ACCEL_FSR_16G:
            fsr = 16000.0;
            break;
        default:
            return -2;
    ...
    /* Read raw data */
    i2c_read_regs(dev->i2c_dev, dev->hw_addr, MPU_ACCEL_START_REG, data, 6);
    /* Release the bus */
    i2c_release(dev->i2c_dev);
...
int mpu_read_compass(mpu_dev_t *dev, mpu_results_t *output)
{
    ...
    /* Read raw data */
    i2c_read_regs(dev->i2c_dev, dev->hw_addr, MPU_EXT_SENS_DATA_START_REG, data, 6);
    /* Release the bus */
    i2c_release(dev->i2c_dev);
...
int mpu_read_temperature(mpu_dev_t *dev, int32_t *output)
{
    ...
    i2c_read_regs(dev->i2c_dev, dev->hw_addr, MPU_TEMP_START_REG, data, 2);
    /* Release the bus */
    i2c_release(dev->i2c_dev);
...
int mpu_set_sample_rate(mpu_dev_t *dev, uint16_t rate)
{
    ...
    if ((rate < MPU_MIN_SAMPLE_RATE) || (rate > MPU_MAX_SAMPLE_RATE)) {
        return -2;
    }
    ...
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_RATE_DIV_REG, (char) divider);
...
int mpu_set_compass_sample_rate(mpu_dev_t *dev, uint8_t rate)
{
    ...
    if ((rate < MPU_MIN_COMP_SMPL_RATE) || (rate > MPU_MAX_COMP_SMPL_RATE)
    ...
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE4_CTRL_REG, (char) divider);
    i2c_release(dev->i2c_dev);
...
static int compass_init(mpu_dev_t *dev)
{
    ...
    if (data[0] != MPU_COMP_WHOAMI_ANSWER) {
        DEBUG("[Error] Wrong answer from compass\n");
        return -1;
    }

    /* Configure Power Down mode */
    i2c_write_reg(dev->i2c_dev, dev->comp_addr, COMPASS_CNTL_REG, MPU_COMP_POWER_DOWN);
    xtimer_usleep(MPU_COMP_MODE_SLEEP_US);
    /* Configure Fuse ROM access */
    i2c_write_reg(dev->i2c_dev, dev->comp_addr, COMPASS_CNTL_REG, MPU_COMP_FUSE_ROM);
    xtimer_usleep(MPU_COMP_MODE_SLEEP_US);
    ...
    /* Configure MPU_WITH_COMPASS for single master mode */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_I2C_MST_REG, BIT_WAIT_FOR_ES);

    /* Set up slave line 0 */
    /* Slave line 0 reads the compass data */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr,
            MPU_SLAVE0_ADDR_REG, (BIT_SLAVE_RW | dev->comp_addr));
    /* Slave line 0 read starts at compass data register */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE0_REG_REG, COMPASS_DATA_START_REG);
    /* Enable slave line 0 and configure read length to 6 consecutive registers */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE0_CTRL_REG, (BIT_SLAVE_EN | 0x06));

    /* Set up slave line 1 */
    /* Slave line 1 writes to the compass */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE1_ADDR_REG, dev->comp_addr);
    /* Slave line 1 write starts at compass control register */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE1_REG_REG, COMPASS_CNTL_REG);
    /* Enable slave line 1 and configure write length to 1 register */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_SLAVE1_CTRL_REG, (BIT_SLAVE_EN | 0x01));
    /* Configure data which is written by slave line 1 to compass control */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr,
            MPU_SLAVE1_DATA_OUT_REG, MPU_COMP_SINGLE_MEASURE);

    /* Slave line 0 and 1 operate at each sample */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr,
            MPU_I2C_DELAY_CTRL_REG, (BIT_SLV0_DELAY_EN | BIT_SLV1_DELAY_EN));
    /* Set I2C bus to VDD */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_YG_OFFS_TC_REG, BIT_I2C_MST_VDDIO);
...
{
   char data;
   i2c_read_reg(dev->i2c_dev, dev->hw_addr, MPU_USER_CTRL_REG, &data);

   if (bypass_enable) {
       data &= ~(BIT_I2C_MST_EN);
       i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_USER_CTRL_REG, data);
       xtimer_usleep(MPU_BYPASS_SLEEP_US);
       i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_INT_PIN_CFG_REG, BIT_I2C_BYPASS_EN);
   }
   else {
       data |= BIT_I2C_MST_EN;
       i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_USER_CTRL_REG, data);
       xtimer_usleep(MPU_BYPASS_SLEEP_US);
       i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_INT_PIN_CFG_REG, REG_RESET);
   }
}
#endif
/**
 ...
 *          Acquisation and release is supposed to be handled by the calling function.
 */
static void conf_lpf(mpu_dev_t *dev, uint16_t half_rate)
{
    mpu_lpf_t lpf_setting;

    /* Get target LPF configuration setting */
    if (half_rate >= 188) {
        lpf_setting = MPU_FILTER_188HZ;
    }
    else if (half_rate >= 98) {
        lpf_setting = MPU_FILTER_98HZ;
    }
    else if (half_rate >= 42) {
        lpf_setting = MPU_FILTER_42HZ;
    }
    else if (half_rate >= 20) {
        lpf_setting = MPU_FILTER_20HZ;
    }
    else if (half_rate >= 10) {
        lpf_setting = MPU_FILTER_10HZ;
    }
    else {
        lpf_setting = MPU_FILTER_5HZ;
    }

    /* Write LPF setting to configuration register */
    i2c_write_reg(dev->i2c_dev, dev->hw_addr, MPU_LPF_REG, (char)lpf_setting);
}
