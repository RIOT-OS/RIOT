...
netdev_802154_tx_status_t at86rf231_send_direct(
        ...
        unsigned int len)
{
    ...
    if (!at86rf231_is_on()) {
        DEBUG("at86rf231: cannot send a packet when transceiver is off!\n");
    ...
    /*** START THE TRANSMISSION, BY PUTTING TRANSCEIVER IN TX STATE ***/
...
#if ENABLE_DEBUG
    DEBUG("Frame to send is %d bytes long:\n", size);
        ...
        unsigned int len)
{
    ...
    /* change into transmission (PLL_ON) state */
    netdev_802154_tx_status_t res =
           at86rf231_change_mode(AT86RF231_TRX_STATE__FORCE_PLL_ON);
    if (res != NETDEV_802154_TX_STATUS_OK) {
        core_panic(0x0231,
                   "at86rf231 (TX) : ERROR : could not enter PLL_ON mode");
    }
...
netdev_802154_tx_status_t at86rf231_transmit_tx_buf(netdev_t *dev)
{
    ...
    /* Start TX */
    netdev_802154_tx_status_t res =
           at86rf231_change_mode(AT86RF231_TRX_STATE__TX_START);
    if (res != NETDEV_802154_TX_STATUS_OK) {
        core_panic(0x0231,
                   "at86rf231 (TX) : ERROR : could not start TX");
    }
