...
void at86rfa1_sleep(at86rfa1_t *dev)
{
    if (dev->base.state != AT86RF2XX_STATE_SLEEP) {
        ...
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__IRQ_STATUS));
...
void at86rfa1_assert_awake(at86rfa1_t *dev)
{
    if (dev->base.state == AT86RF2XX_STATE_SLEEP) {
        *AT86RFA1_REG__TRXPR &= ~(AT86RF2XX_TRXPR_MASK__SLPTR);
        xtimer_usleep(AT86RFA1_WAKEUP_DELAY);
 ...
 */
static
int16_t at86rfa1_tx_pow_to_dbm(const at86rfa1_t *dev, uint8_t txpower)
{
    (void)dev;
 ...
 */
static
uint8_t at86rfa1_dbm_to_tx_pow(const at86rfa1_t *dev, int16_t dbm)
{
    (void)dev;
    assert(dbm >= AT86RFA1_TXPOWER_DBM_MIN &&
           dbm <= AT86RFA1_TXPOWER_DBM_MAX);
 ...
 *          in dbm for AT86RFA1 transceivers
 */
static
int16_t at86rfa1_rx_sens_to_dbm(const at86rfa1_t *dev, uint8_t rxsens)
{
    (void)dev;
 ...
 */
static
uint8_t at86rfa1_dbm_to_rxsens(const at86rfa1_t *dev, int16_t dbm)
{
    (void)dev;
    assert(dbm >= AT86RFA1_RX_SENSITIVITY_DBM_MIN &&
           dbm <= AT86RFA1_RX_SENSITIVITY_DBM_MAX);
    dbm -= AT86RFA1_RX_SENSITIVITY_DBM_MIN;
...
void at86rfa1_hardware_reset(at86rfa1_t *dev)
{
    *AT86RFA1_REG__TRXPR |= AT86RF2XX_TRXPR_MASK__TRXRST;
    ...
    assert(at86rfa1_get_status(dev) == AT86RF2XX_STATE_TRX_OFF);
...
void at86rfa1_setup(at86rfa1_t *dev)
{
    ...
    at86rf2xx_periph_reg_write(AT86RFA1_REG(AT86RF2XX_REG__IRQ_MASK), 0x00);
}
...
int at86rfa1_validate(const at86rfa1_t *dev)
{
    (void)dev;
    uint8_t partn =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__PART_NUM));
    ...
    DEBUG("manufactorer: 0x%02X%02X\n",
          at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__MAN_ID_1)),
          at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__MAN_ID_0)));
    DEBUG("version: 0x%02x\n",
          at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__VERSION_NUM)));
    return 0;
}
...
uint8_t at86rfa1_set_state(at86rfa1_t *dev, uint8_t state)
{
    ...
    /* make sure there is no ongoing transmission, or state transmission already
       in progress */
    do {
        old_state = at86rfa1_get_status(dev);
    ...
    if (state != old_state) {
        if (old_state == AT86RF2XX_STATE_SLEEP) {
            DEBUG("at86rfa1: waking up from sleep mode\n");
            at86rfa1_assert_awake(dev);
        }
    ...
    do {
        trx_status =
            at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__TRX_STATUS));
...
{
    /* hardware reset */
    at86rfa1_hardware_reset(dev);

    /* soft reset */
    at86rf2xx_reset((at86rf2xx_t *)dev);

    /* reconfigure channel */
    at86rfa1_set_channel(dev, AT86RFA1_DEFAULT_CHANNEL);

    /* reconfigure TX power */
    at86rfa1_set_txpower(dev, AT86RF2XX_DEFAULT_TXPOWER);

    /* enable safe mode (protect RX FIFO until reading data starts) */
    at86rfa1_set_frame_buffer_protection(dev, true);

    /* enable and clear interrupts */
    at86rf2xx_periph_reg_write(AT86RFA1_REG(AT86RF2XX_REG__IRQ_MASK),
    ...
    DEBUG("[at86rfa1] reset complete.\n");
}

int16_t at86rfa1_get_txpower(const at86rfa1_t *dev)
{
    uint8_t phy_tx_pwr =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__PHY_TX_PWR));

    phy_tx_pwr &= AT86RFA1_PHY_TX_PWR_MASK__TX_PWR;
    DEBUG("[at86rfa1] txpower value: %x\n", phy_tx_pwr);
    return at86rfa1_tx_pow_to_dbm(dev, phy_tx_pwr);
}

void at86rfa1_set_txpower(at86rfa1_t *dev, int16_t dbm)
{
    if (dbm < AT86RFA1_TXPOWER_DBM_MIN) {
        dbm = AT86RFA1_TXPOWER_DBM_MIN;
    }
    else if (dbm > AT86RFA1_TXPOWER_DBM_MAX) {
        dbm = AT86RFA1_TXPOWER_DBM_MAX;
    }
    uint8_t txpower = at86rfa1_dbm_to_tx_pow(dev, dbm);
    DEBUG("[at86rfa1] txpower value: %x\n", txpower);
    uint8_t phy_tx_pwr =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__PHY_TX_PWR));
    phy_tx_pwr &= ~(AT86RFA1_PHY_TX_PWR_MASK__TX_PWR);
    phy_tx_pwr |= (txpower & AT86RFA1_PHY_TX_PWR_MASK__TX_PWR);
    at86rf2xx_periph_reg_write(AT86RFA1_REG(
...
{
    uint8_t rx_syn =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__RX_SYN))
        & AT86RF2XX_RX_SYN_MASK__RX_PDT_LEVEL;

    return at86rfa1_rx_sens_to_dbm(dev, rx_syn);
}

void at86rfa1_set_rxsensitivity(const at86rfa1_t *dev, int16_t dbm)
{
    if (dbm < AT86RFA1_RX_SENSITIVITY_DBM_MIN) {
        dbm = AT86RFA1_RX_SENSITIVITY_DBM_MIN;
    }
    else if (dbm > AT86RFA1_RX_SENSITIVITY_DBM_MAX) {
        dbm = AT86RFA1_RX_SENSITIVITY_DBM_MAX;
    }
    uint8_t rxsens = at86rfa1_dbm_to_rxsens(dev, dbm);
    DEBUG("[at86rfa1] rxsens value: %x\n", rxsens);
    uint8_t rx_syn =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__RX_SYN));
    rx_syn &= ~(AT86RF2XX_RX_SYN_MASK__RX_PDT_LEVEL);
    rx_syn |= (rxsens & AT86RF2XX_RX_SYN_MASK__RX_PDT_LEVEL);
    at86rf2xx_periph_reg_write(AT86RFA1_REG(AT86RF2XX_REG__RX_SYN), rx_syn);
}
...
bool at86rfa1_cca(at86rfa1_t *dev)
{
    ...
    at86rf2xx_periph_reg_write(AT86RFA1_REG(AT86RF2XX_REG__RX_SYN), tmp);
...
int8_t at86rfa1_get_cca_threshold(const at86rfa1_t *dev)
{
    ...
    return AT86RFA1_RSSI_BASE_VAL + (thresh << 1);
}
...
int8_t at86rfa1_get_ed_level(const at86rfa1_t *dev)
{
    ...
    uint8_t phy_ed_level =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__PHY_ED_LEVEL));
    return (int8_t)(phy_ed_level + AT86RFA1_RSSI_BASE_VAL);
}

void at86rfa1_set_channel(at86rfa1_t *dev, uint8_t channel)
{
    if (channel < AT86RFA1_CHANNEL_MIN) {
        channel = AT86RFA1_CHANNEL_MIN;
    }
    else if (channel > AT86RFA1_CHANNEL_MAX) {
        channel = AT86RFA1_CHANNEL_MAX;
    }

    uint8_t prev_state = at86rfa1_set_state(dev, AT86RF2XX_STATE_TRX_OFF);

    at86rf2xx_set_channel((at86rf2xx_t *)dev, channel);

    /* Return to the state we had before reconfiguring */
    at86rfa1_set_state(dev, prev_state);
}
...
void at86rfa1_configure_phy(const at86rfa1_t *dev, at86rfa1_phy_mode_t mode)
{
    (void)dev;
    uint8_t trx_ctrl_2 =
        at86rf2xx_periph_reg_read(AT86RFA1_REG(AT86RF2XX_REG__TRX_CTRL_2));

    uint8_t mode_mask = AT86RFA1_TRX_CTRL_2_MASK__OQPSK_DATA_RATE;
                         ...
                         void *buf, size_t buf_size)
{
    ...
    if (payload_length > 0) {
        ...
        at86rf2xx_periph_fb_read(buf, AT86RFA1_REG__TRXFBST,
                                 (size_t)payload_length);
        fb->payload = buf;
        at86rf2xx_periph_fb_read(&fb->fcs,
                                 AT86RFA1_REG__TRXFBST + payload_length, 2);
        at86rf2xx_periph_fb_read(&fb->lqi,
                                 AT86RFA1_REG__TRXFBST + payload_length + 2, 1);
    }
    return payload_length;
}
#endif /* IS_USED(MODULE_AT86RFA1) */
