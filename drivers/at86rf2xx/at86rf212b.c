...
int at86rf212b_validate(const at86rf212b_t *dev)
{
    ...
    if (partn != AT86RF212B_PARTNUM) {
        DEBUG("[at86rf212b] error: unable to read correct part number\n");
        return -ENOTSUP;
    }
    DEBUG("AT86RF212B 0x%02X\n", partn);
...
{
    uint8_t old_state = at86rf212b_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
    /* Disable RX path */
    uint8_t rx_syn = at86rf2xx_spi_reg_read((at86rf2xx_t *)dev,
                                            AT86RF2XX_REG__RX_SYN);
    ...
    at86rf212b_set_state(dev, AT86RF2XX_STATE_RX_ON);
    bool cca =  at86rf2xx_cca((at86rf2xx_t *)dev);
    /* re-enable RX */
    at86rf2xx_spi_reg_write((at86rf2xx_t *)dev, AT86RF2XX_REG__RX_SYN, rx_syn);
    /* Step back to the old state */
    at86rf212b_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
    at86rf212b_set_state(dev, old_state);
    return cca;
}
