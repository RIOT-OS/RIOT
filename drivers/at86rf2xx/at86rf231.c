...
int at86rf231_validate(const at86rf231_t *dev)
{
    ...
    if (partn != AT86RF231_PARTNUM) {
        DEBUG("[at86rf231] error: unable to read correct part number\n");
        return -ENODEV;
    }
    DEBUG("AT86RF231 0x%02X\n", partn);
...
uint8_t at86rf231_set_state(at86rf231_t *dev, uint8_t state)
{
    ...
    /* make sure there is no ongoing transmission, or state transmission already
       in progress */
    do {
        old_state = at86rf231_get_status(dev);
    ...
    if (state != old_state) {
        if (old_state == AT86RF2XX_STATE_SLEEP) {
            DEBUG("at86rf231: waking up from sleep mode\n");
            at86rf231_assert_awake(dev);
        }
...
{
    uint8_t old_state = at86rf231_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
    /* Disable RX path */
    uint8_t rx_syn = at86rf2xx_spi_reg_read((at86rf2xx_t *)dev,
                                            AT86RF2XX_REG__RX_SYN);
    ...
    at86rf231_set_state(dev, AT86RF2XX_STATE_RX_ON);
    bool cca =  at86rf2xx_cca((at86rf2xx_t *)dev);
    /* re-enable RX */
    at86rf2xx_spi_reg_write((at86rf2xx_t *)dev, AT86RF2XX_REG__RX_SYN, rx_syn);
    /* Step back to the old state */
    at86rf231_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
    at86rf231_set_state(dev, old_state);
    return cca;
}
