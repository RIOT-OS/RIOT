 ...
 *
 * @file
 * @brief       Implementation of AT86RF232 internal functions
 *
...
void at86rf232_sleep(at86rf232_t *dev)
{
    if (dev->base.state != AT86RF2XX_STATE_SLEEP) {
        ...
        if (dev->base.state != AT86RF2XX_STATE_TRX_OFF) {
            at86rf232_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
        }
...
static
void at86rf232_assert_awake(at86rf232_t *dev)
{
    if (dev->base.state == AT86RF2XX_STATE_SLEEP) {
        gpio_clear(dev->params.base_params.sleep_pin);
        xtimer_usleep(AT86RF232_WAKEUP_DELAY);
 ...
 */
static
int16_t at86rf232_tx_pow_to_dbm(const at86rf232_t *dev, uint8_t txpower)
{
    (void)dev;
 ...
 */
static
uint8_t at86rf232_dbm_to_tx_pow(const at86rf232_t *dev, int16_t dbm)
{
    (void)dev;
    assert(dbm >= AT86RF232_TXPOWER_DBM_MIN &&
           dbm <= AT86RF232_TXPOWER_DBM_MAX);
 ...
 */
static
int16_t at86rf232_rx_sens_to_dbm(const at86rf232_t *dev, uint8_t rxsens)
{
    (void)dev;
 ...
 */
static
uint8_t at86rf232_dbm_to_rxsens(const at86rf232_t *dev, int16_t dbm)
{
    (void)dev;
    assert(dbm >= AT86RF232_RX_SENSITIVITY_DBM_MIN &&
           dbm <= AT86RF232_RX_SENSITIVITY_DBM_MAX);
    dbm -= AT86RF232_RX_SENSITIVITY_DBM_MIN;
...
uint8_t at86rf232_set_state(at86rf232_t *dev, uint8_t state)
{
    ...
    /* make sure there is no ongoing transmission, or state transmission already
       in progress */
    do {
        old_state = at86rf232_get_status(dev);
    ...
    if (state != old_state) {
        if (old_state == AT86RF2XX_STATE_SLEEP) {
            DEBUG("at86rf232: waking up from sleep mode\n");
            at86rf232_assert_awake(dev);
        }
...
void at86rf232_hardware_reset(at86rf232_t *dev)
{
    ...
    assert(at86rf232_get_status(dev) == AT86RF2XX_STATE_TRX_OFF);
...
void at86rf232_reset(at86rf232_t *dev)
{
    ...
    /* reconfigure channel */
    at86rf232_set_channel(dev, AT86RF232_DEFAULT_CHANNEL);

    /* configure phy */
    at86rf232_configure_phy(dev);

    /* reconfigure TX power */
    at86rf232_set_txpower(dev, AT86RF2XX_DEFAULT_TXPOWER);

    /* disable clock output to save power */
    at86rf232_set_clock_output(dev, false, AT86RF2XX_TRX_CTRL_0_CLKM_CTRL__OFF);

    /* enable safe mode (protect RX FIFO until reading data starts) */
    at86rf232_set_frame_buffer_protection(dev, true);

    /* enable and clear interrupts */
    at86rf2xx_spi_reg_write((at86rf2xx_t *)dev, AT86RF2XX_REG__IRQ_MASK,
                            AT86RF2XX_IRQ_MASK_MASK__TRX_END);
    at86rf232_set_irq_mode(dev, true, false);
    at86rf2xx_spi_reg_read((at86rf2xx_t *)dev, AT86RF2XX_REG__IRQ_STATUS);

    /* State to return after receiving or transmitting */
    dev->base.idle_state = AT86RF2XX_STATE_RX_AACK_ON;

    DEBUG("[at86rf232] reset complete.\n");
}
...
int16_t at86rf232_get_txpower(const at86rf232_t *dev)
{
    ...
    phy_tx_pwr &= AT86RF232_PHY_TX_PWR_MASK__TX_PWR;
    DEBUG("[at86rf232] txpower value: %x\n", phy_tx_pwr);
    return at86rf232_tx_pow_to_dbm(dev, phy_tx_pwr);
}

void at86rf232_set_txpower(at86rf232_t *dev, int16_t dbm)
{
    if (dbm < AT86RF232_TXPOWER_DBM_MIN) {
        dbm = AT86RF232_TXPOWER_DBM_MIN;
    }
    else if (dbm > AT86RF232_TXPOWER_DBM_MAX) {
        dbm = AT86RF232_TXPOWER_DBM_MAX;
    }
    uint8_t txpower = at86rf232_dbm_to_tx_pow(dev, dbm);
    DEBUG("[at86rf232] txpower value: %x\n", txpower);
    uint8_t phy_tx_pwr = at86rf2xx_spi_reg_read((at86rf2xx_t *)dev,
                                                AT86RF2XX_REG__PHY_TX_PWR);
    phy_tx_pwr &= ~(AT86RF232_PHY_TX_PWR_MASK__TX_PWR);
    phy_tx_pwr |= (txpower & AT86RF232_PHY_TX_PWR_MASK__TX_PWR);
...
int16_t at86rf232_get_rxsensitivity(const at86rf232_t *dev)
{
    ...
    return at86rf232_rx_sens_to_dbm(dev, rx_syn);
}

void at86rf232_set_rxsensitivity(const at86rf232_t *dev, int16_t dbm)
{
    if (dbm < AT86RF232_RX_SENSITIVITY_DBM_MIN) {
        dbm = AT86RF232_RX_SENSITIVITY_DBM_MIN;
    }
    else if (dbm > AT86RF232_RX_SENSITIVITY_DBM_MAX) {
        dbm = AT86RF232_RX_SENSITIVITY_DBM_MAX;
    }
    uint8_t rxsens = at86rf232_dbm_to_rxsens(dev, dbm);
...
{
    uint8_t old_state = at86rf232_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
    /* Disable RX path */
    uint8_t rx_syn = at86rf2xx_spi_reg_read((at86rf2xx_t *)dev,
                                            AT86RF2XX_REG__RX_SYN);
    ...
    at86rf232_set_state(dev, AT86RF2XX_STATE_RX_ON);
    bool cca =  at86rf2xx_cca((at86rf2xx_t *)dev);
    /* re-enable RX */
    at86rf2xx_spi_reg_write((at86rf2xx_t *)dev, AT86RF2XX_REG__RX_SYN, rx_syn);
    /* Step back to the old state */
    at86rf232_set_state(dev, AT86RF2XX_STATE_TRX_OFF);
    at86rf232_set_state(dev, old_state);
    return cca;
}
...
int8_t at86rf232_get_cca_threshold(const at86rf232_t *dev)
{
    ...
    return AT86RF232_RSSI_BASE_VAL + (thresh << 1);
}
...
void at86rf232_set_cca_threshold(const at86rf232_t *dev, int8_t thresh)
{
    ...
    thresh -= AT86RF232_RSSI_BASE_VAL;
...
void at86rf232_set_rx_tx_timestamping(const at86rf232_t *dev, bool timestamping)
{
...
{
    if (channel < AT86RF232_CHANNEL_MIN) {
        channel = AT86RF232_CHANNEL_MIN;
    }
    else if (channel > AT86RF232_CHANNEL_MAX) {
        channel = AT86RF232_CHANNEL_MAX;
    }
    uint8_t prev_state = at86rf232_set_state(dev, AT86RF2XX_STATE_TRX_OFF);

    at86rf2xx_set_channel((at86rf2xx_t *)dev, channel);

    /* Return to the state we had before reconfiguring */
    at86rf232_set_state(dev, prev_state);
}
