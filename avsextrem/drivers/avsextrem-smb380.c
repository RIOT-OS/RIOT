...
void SMB380_enableNewDataInt(void)
{
    unsigned short uReg;
    unsigned long cpsr = disableIRQ();
    //prevent deep sleep, reason: 400ï¿½s wake-up time is to long for 3kHz interrupts
...
void SMB380_disableNewDataInt(void)
{
    ...
    SMB380_ssp_read();
    SMB380_Unprepare();
    //enable deep sleep, reason: 400ï¿½s wake-up time was to long for 3kHz interrupts
