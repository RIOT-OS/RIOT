/*
 * msba2acc-smb380.c - implementation of the Driver for the SMB380 acceleration sensor.
 ...
 * @file
 * @internal
 * @brief	SMB380 acceleration sensor drivers for the LPC2387
 *
...
uint8_t SMB380_init(uint8_t (*func)(int16_t*))
{
	...
	if((ringBuff_X == NULL) | (ringBuff_Y == NULL) | (ringBuff_Z == NULL) | (ringBuff_T == NULL)){
		if(ringBuff_X != NULL) free((int16_t*)ringBuff_X);
		if(ringBuff_Y != NULL) free((int16_t*)ringBuff_Y);
		if(ringBuff_Z != NULL) free((int16_t*)ringBuff_Z);
		if(ringBuff_T != NULL) free((int16_t*)ringBuff_T);
		return 0;
	}
...
{
	switch(activate){
		case 0: dynRange=false ; break;
		case 1: dynRange=true ; break;
	}
}
...
unsigned char SMB380_getRange(void)
{
	...
	switch(ur){
		case SMB380_RANGE_2G: return 2;
		case SMB380_RANGE_4G: return 4;
		case SMB380_RANGE_8G: return 8;
		default: return 4;
	}
}
...
int16_t SMB380_getBandWidthAbs(void)
{
	...
	switch(uBand){
		case SMB380_BAND_WIDTH_25HZ: 	return 25;
		case SMB380_BAND_WIDTH_50HZ: 	return 50;
		case SMB380_BAND_WIDTH_100HZ: 	return 100;
		case SMB380_BAND_WIDTH_190HZ: 	return 190;
		case SMB380_BAND_WIDTH_375HZ: 	return 375;
		case SMB380_BAND_WIDTH_750HZ: 	return 750;
		case SMB380_BAND_WIDTH_1500HZ: 	return 1500;
		default: return uBand;
	}
}
