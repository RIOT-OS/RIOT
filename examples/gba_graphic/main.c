...
void move_ball(struct Ball *ball) {
    ...
    if (ball->x <= 0 || ball->x >= SCREEN_WIDTH - ball->width) ball->vx *= -1;
    if (ball->y <= 0 || ball->y >= SCREEN_HEIGHT - ball->height) ball->vy *= -1;
}
...
int main(void)
{
    ...
    while(1) {
        ...
        *bg0_x_scroll = 8;
        ...
        dma_memcpy16(3, (unsigned short*) 0x6010000, (unsigned short*) (uintptr_t)board_data, (board_width * board_height) / 2);
        dma_memcpy16(3, (unsigned short*) (0x6010000 + (board_width * board_height)), (unsigned short*) (uintptr_t)ball_data, (ball_width * ball_height) / 2);

        sprite_clear();

        GBA_DISPCNT |= 0x1000;
        GBA_DISPCNT |= 0x40;
        dma_memcpy16(3, (unsigned short*) 0x7000000, (unsigned short*) (uintptr_t) sprites, NUM_SPRITES * 4);
