 ...
 *
 * @author      Rane Balslev <ranebalslev@gmail.com> & Oliver Hahm <oliver.hahm@inria.fr>
 */
...
void sixlowapp_send_ping(int argc, char **argv)
{
    ...
    //DEBUG("sixlowapp_send_ping - afer icmpv6_send_echo_request!\n");
    _waiting_for_pong = 1;
...
void send_packet(int argc, char **argv)
{
    ...
    result = ipv6_sendto(&dest, /*IPV6_PROTO_NUM_NONE*/ IPV6_PROTO_NUM_UDP, (uint8_t*)msg, msg_len);
    //result = sixlowpan_lowpan_sendto(TRANSCEIVER_TYPE, &r_addr, 2, (uint8_t*)msg, msg_len);
    if(result == (msg_len+40)) // 40 bytes of payload
    {
...
void send_adc_value(int argc, char **argv)
{
    ...
    value = adc_sample(dev, 0); // get ADC value!
    printf("ADC value is: %d\n", value );
    char msg [6];
     // convert int to char*
    uint16_t msg_len = sprintf(msg, "%d", value);//= (uint16_t)strlen(msg); // Message length
    ...
    result = ipv6_sendto(&dest, IPV6_PROTO_NUM_NONE, (uint8_t*)msg, msg_len);
    ...
    while (1) {
        ...
        if (m.type == IPV6_PACKET_RECEIVED) {
            ...
            if (ipv6_buf->nextheader == IPV6_PROTO_NUM_UDP) {
                ...
                // printf("sixlowpan monitor payload:\n");
                // for(int i = 0; i < ipv6_buf->length; i++)
                // {                    
                //     printf("%c", (char)m.content.ptr[i] );
                // }
