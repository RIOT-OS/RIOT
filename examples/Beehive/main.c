...
const shell_command_t shell_commands[] = {
    ...
    {"adc", "Send the current ADC value over 6LoWPAN. Usage: adc <ADR>", send_adc_value},
    {NULL, NULL, NULL}
};
...
int main(void)
{
    ...
    puts("RIOT 6LoWPAN and ADC example v"APP_VERSION);
    ...
    ipv6_addr_init(&tmp, 0xabcd, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, net_if_get_hardware_address(0));
        ...
        if (!ipv6_net_if_add_addr(IF_ID, &tmp, NDP_ADDR_STATE_PREFERRED,
                              NDP_OPT_PI_VLIFETIME_INFINITE,
                              NDP_OPT_PI_PLIFETIME_INFINITE, 0)) {
        printf("Can not add global address to interface %d\n", IF_ID);
        return 0;
    }
