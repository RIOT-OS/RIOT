                 ...
                 unsigned int delay)
{
    ...
    // set source address
    gnrc_netif_ipv6_addrs_get(netif, &src, sizeof(ipv6_addr_t));
    ...
    for (unsigned int i = 0; i < num; i++) {
        gnrc_pktsnip_t *payload, *udp, *ip; //, *schc;
        ...
        ip_hdr->len = byteorder_htons(payload_size + udp->size);
        ip_hdr->src = ipv6_addr_link_local_prefix;

        udp_hdr_t *udp_hdr = (udp_hdr_t *)udp->data;
        udp_hdr->length = byteorder_htons(payload_size + udp->size);
        ...
        /* send packet */
        if (!gnrc_netapi_dispatch_send(GNRC_NETTYPE_SCHC, GNRC_NETREG_DEMUX_CTX_ALL, ip)) {
            puts("Error: unable to locate SCHC thread");
