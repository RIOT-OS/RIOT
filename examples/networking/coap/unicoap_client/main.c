...
static int _on_response(
    ...
    if ((unicoap_options_get_content_format(response->options, &format) >= 0
         && format == UNICOAP_FORMAT_TEXT)) {
        printf("text respose: '%.*s'\n", (int)response->payload_size, (char*)response->payload);
    }
...
static int _set_flags(char* opt, unicoap_client_flags_t* flags) {
    if (strcmp(opt, "-r") == 0) {
